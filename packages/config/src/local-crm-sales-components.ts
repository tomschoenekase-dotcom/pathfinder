...
export function assertLocalCrmSalesComponentsEnvironment(
  ...
  if (
    ...
    env.APP_ENV === 'production' || env.DEPLOYMENT_ENV === 'production'
  ...
  if (
    !script || !isAbsolute(script) || basename(script) !== 'component_bridge.py' ||
    !env.TORCHIKO_CRM_VAULT || !isAbsolute(env.TORCHIKO_CRM_VAULT)
...
export function assertAuthenticatedCrmSalesComponentsEnvironment(
  ...
  if (env.TORCHIKO_AUTHENTICATED_CRM_SALES_ENABLED !== '1' ||
      env.TORCHIKO_LOCAL_CRM_REHEARSAL === '1' ||
      env.TORCHIKO_LOCAL_CRM_RESEARCH_ENABLED === '1' ||
      env.TORCHIKO_LOCAL_CRM_SALES_ENABLED === '1')
    throw new Error('Authenticated private CRM sales components are not configured')
  const script = env.TORCHIKO_CRM_SALES_BRIDGE
  if (!script || !isAbsolute(script) || basename(script) !== 'component_bridge.py' ||
      !env.TORCHIKO_CRM_VAULT || !isAbsolute(env.TORCHIKO_CRM_VAULT))
...
const guideRelative = join('95 AI Staging', 'Torchiko Sales Writing Reference 2026-09-21',
...
export async function inspectAuthenticatedCrmSalesComponents(
  ...
  let script: string
  try { script = assertAuthenticatedCrmSalesComponentsEnvironment(env) }
  catch {
    return { state: 'unconfigured' as const, reason: 'PRIVATE_COMPONENT_CONFIGURATION_REQUIRED' as const,
  ...
  try {
    ...
    return { state: 'unavailable' as const, reason: 'PRIVATE_COMPONENT_OWNER_NOT_INSTALLED' as const,
  ...
  } catch { /* Only a safe status leaves this boundary. */ }
  return { state: 'paths-present-runtime-unverified' as const,
    reason: 'PRIVATE_COMPONENT_PATHS_PRESENT_RUNTIME_UNVERIFIED' as const, guide }
}
...
async function readExactOwnerWritingGuide(env: Record<string, string | undefined>) {
  ...
  try {
    ...
    if (relative(root, selected) !== guideRelative)
      throw new Error('Guide outside owner root')
    handle = await open(selected, 'r')
    const file = await handle.stat()
    if (!file.isFile() || file.size > 32_000)
      throw new Error('Guide outside bounded file contract')
    ...
    return { label: 'Torchiko sales writing reference v0.2',
      sourceRef: TORCHIKO_SAVED_WRITING_GUIDE_SOURCE, text,
...
export async function invokeLocalCrmSalesComponents<T extends { snapshotHash: string }>(
  ...
  return new Promise((resolve, reject) => {
    const child = spawn('python', ['-B', script], {
      shell: false, cwd: dirname(script), windowsHide: true,
      env: {
        ...
        TORCHIKO_LOCAL_CRM_REHEARSAL: runtime === 'local'
          ? process.env.TORCHIKO_LOCAL_CRM_REHEARSAL : undefined,
      },
    ...
    const timer = setTimeout(() => fail('Local component preparation exceeded its bounded runtime'), 15_000)
    ...
    child.stderr.on('data', (part: string) => { stderr = (stderr + part).slice(-8_000) })
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        fail(stderr ? 'Local component returned an invalid result; inspect the local adapter test' : 'Invalid component result')
      }
    })
