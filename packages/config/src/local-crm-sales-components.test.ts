...
import { assertAuthenticatedCrmSalesComponentsEnvironment,
  inspectAuthenticatedCrmSalesComponents, readAuthenticatedTorchikoWritingGuide,
  TORCHIKO_SAVED_WRITING_GUIDE_SOURCE } from './local-crm-sales-components'
...
describe('authenticated private CRM component boundary', () => {
  ...
  it('requires an explicit installed owner and never inherits fixture authority', () => {
    ...
    expect(() => assertAuthenticatedCrmSalesComponentsEnvironment({
      ...configured, TORCHIKO_LOCAL_CRM_RESEARCH_ENABLED: '1',
    })).toThrow('not configured')
    expect(() => assertAuthenticatedCrmSalesComponentsEnvironment({
      ...configured, TORCHIKO_LOCAL_CRM_REHEARSAL: '1',
    })).toThrow('not configured')
    expect(() => assertAuthenticatedCrmSalesComponentsEnvironment({
      ...configured, TORCHIKO_LOCAL_CRM_SALES_ENABLED: '1',
    })).toThrow('not configured')
    expect(() => assertAuthenticatedCrmSalesComponentsEnvironment({
      ...configured, TORCHIKO_CRM_SALES_BRIDGE: 'component_bridge.py',
    })).toThrow('not configured')
  ...
  it('distinguishes unconfigured, missing owner and readable exact guide without revealing paths', async () => {
    expect(await inspectAuthenticatedCrmSalesComponents({})).toMatchObject({
      state: 'unconfigured', guide: { state: 'unconfigured', sha256: null },
    })
    expect(await inspectAuthenticatedCrmSalesComponents(configured)).toMatchObject({
      state: 'unavailable', guide: { state: 'unavailable', sha256: null },
    })
    const root = await mkdtemp(join(tmpdir(), 'torchiko-guide-test-'))
    try {
      const installed = join(root, 'installed')
      const privateVault = join(root, 'vault')
      const selected = join(privateVault, '95 AI Staging',
        'Torchiko Sales Writing Reference 2026-09-21', 'v0.2-r001')
      ...
      const env = { ...configured, TORCHIKO_CRM_SALES_BRIDGE: join(installed, 'component_bridge.py'),
      ...
      expect(readiness).toMatchObject({ state: 'paths-present-runtime-unverified',
      ...
      expect(await readAuthenticatedTorchikoWritingGuide(env)).toEqual({
        ...
        sourceRef: TORCHIKO_SAVED_WRITING_GUIDE_SOURCE, text, sha256: expected,
      })
