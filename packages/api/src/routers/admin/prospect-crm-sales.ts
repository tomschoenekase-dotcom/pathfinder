  ...
  try {
    const accounts = await withTenantIsolationBypass(() =>
      db.correspondenceProviderAccount.findMany({
        where: {
          provider: 'GMAIL',
          mailboxAddress: { equals: companyMailbox, mode: 'insensitive' },
        },
        select: {
          id: true,
          mailboxAddress: true,
    ...
    if (accounts.length === 0)
      return {
        identity: companyMailbox,
        state: 'unconfigured' as const,
        accountId: null,
        connectionStatus: null,
        lastSuccessfulSyncAt: null,
        updatedAt: null,
        syncEvidence: 'NO_SUCCESSFUL_SYNC_RECORDED' as const,
    ...
    if (accounts.length !== 1)
      return {
        ...
        lastSuccessfulSyncAt: null,
        updatedAt: null,
        syncEvidence: 'NOT_ESTABLISHED' as const,
        queueState: 'NOT_INSPECTED' as const,
    ...
    return {
      ...
      syncEvidence: account.lastSuccessfulSyncAt
        ? ('LAST_SUCCESSFUL_SYNC_RECORDED' as const)
        : ('NO_SUCCESSFUL_SYNC_RECORDED' as const),
      queueState: 'NOT_INSPECTED' as const,
      nextAction:
        account.connectionStatus === 'CONNECTED'
          ? account.lastSuccessfulSyncAt
            ? ('REVIEW_CURRENT_MAILBOX_SNAPSHOT' as const)
...
export const adminProspectCrmSalesRouter = router({
  getProspectSalesReadiness: adminProcedure.query(async () => {
    const [sales, mailbox] = await Promise.all([
      readAuthenticatedSalesReadiness(),
      readCompanyMailboxStatus(),
    ])
    return { ...sales, authentication: { scope: 'AUTHENTICATED_PLATFORM_ADMIN' as const }, mailbox }
  }),
  ...
  prepareReviewProspectSales: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        return await applyNativeSalesAction(
          input,
          {
            type: 'HUMAN',
            role: 'PLATFORM_ADMIN',
