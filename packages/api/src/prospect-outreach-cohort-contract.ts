...
export const outreachCohortPreviewInput = z
  .object({
...
export const outreachCohortReserveInput = z
  .object({
    requestKey: id,
    name: z.string().trim().min(3).max(191),
    preview: outreachCohortPreviewInput,
    expectedPreviewHash: hash,
  })
  .strict()
export const outreachCohortReadInput = z.object({ cohortId: id }).strict()
export const outreachCohortListInput = z
  .object({ limit: z.number().int().min(1).max(50).default(25), cursor: id.optional() })
  .strict()
export const outreachCohortWindowInput = z
  .object({
    cohortId: id,
    requestKey: id,
    limit: z.number().int().min(1).max(OUTREACH_WINDOW_LIMIT).default(5),
...
export const outreachCohortControlInput = z
  .object({
    cohortId: id,
    requestKey: id,
  ...
  .strict()
export const outreachCohortControlReceipt = z
  .object({
    cohortId: id,
    receiptId: id,
    status: z.enum(['DRAFT', 'PAUSED', 'CANCELLED']),
    reason: z.string(),
    retainedMembers: z.number().int().min(1).max(50),
    priorGroupExclusionRetained: z.literal(true),
    draftsDeleted: z.literal(false),
    deliveryChanged: z.literal(false),
    replayed: z.literal(false),
    SEND_AUTHORIZED: z.literal(false),
  })
  .strict()
export const outreachCohortAcknowledgeInput = z
  .object({
    cohortId: id,
    expectedReviewHash: hash,
    expectedCount: z.number().int().min(1).max(50),
    acknowledgement: z.literal(
      'I reviewed these exact messages and holds. This is not sending approval.',
    ),
  })
  .strict()
...
export type CohortCandidate = {
  ...
  priorContactReservation: boolean
  contactSelected: boolean
  contactVerified: boolean
}
export type CohortPreviewRow = CohortCandidate & {
  disposition: 'ELIGIBLE' | 'HELD' | 'EXCLUDED'
  reasons: string[]
  selected: boolean
}
...
export function planOutreachCohort(
  input: z.infer<typeof outreachCohortPreviewInput>,
  candidates: CohortCandidate[],
) {
  if (
    candidates.length !== input.candidates.length ||
    candidates.some((v, i) => v.venueId !== input.candidates[i]!.venueId)
  )
    throw new Error('Exact complete candidate order is required; no missing or replacement rows.')
  const seenOrganizations = new Set<string>(),
    seenRecipients = new Set<string>()
  const rows: CohortPreviewRow[] = candidates.map((candidate) => {
    const reasons: string[] = []
    const state: { disposition: CohortPreviewRow['disposition'] } = { disposition: 'ELIGIBLE' }
    const exclude = (reason: string) => {
      state.disposition = 'EXCLUDED'
      reasons.push(reason)
    }
    const hold = (reason: string) => {
      if (state.disposition !== 'EXCLUDED') state.disposition = 'HELD'
      reasons.push(reason)
    }
    ...
    if (!['chicago', 'evanston'].includes(city ?? '') || candidate.region?.toUpperCase() !== 'IL')
      exclude('OUTSIDE_CHICAGO_EVANSTON')
    if (candidate.relationshipTier !== 'STANDARD')
      exclude('LARGE_OR_STRATEGIC_REQUIRES_SEPARATE_SCOPE')
    if (candidate.sizeVerified && ['large', 'very large', 'enterprise'].includes(size ?? ''))
      exclude('NOT_SMALL_OR_MID_SIZED')
    else if (
      !candidate.sizeVerified ||
      !['small', 'medium', 'mid-sized', 'mid sized'].includes(size ?? '')
    )
      hold('SIZE_NEEDS_SOURCE_REVIEW')
    if (!candidate.countyGeoid || candidate.geographyStatus !== 'ASSIGNED')
      hold('PHYSICAL_GEOGRAPHY_REVIEW_REQUIRED')
    if (candidate.suppressed) exclude('NATIVE_SUPPRESSION')
    if (candidate.priorGroupIds.length || candidate.priorContactReservation)
      exclude('ALREADY_IN_PRIOR_GROUP')
    if (candidate.currentDraftId) exclude('EXISTING_DRAFT_REOPEN_INSTEAD_OF_REDRAFT')
    if (
      ['CONTACTED', 'REPLIED', 'CONVERSATION', 'QUALIFIED', 'PROPOSAL_DECISION', 'WON'].includes(
        candidate.opportunityStage ?? '',
      ) ||
      candidate.history === 'RETAINED_HISTORY'
    )
      exclude('EXISTING_RELATIONSHIP_USE_NATIVE_REPLY_WORKFLOW')
    if (candidate.history === 'INCOMPLETE_OR_UNAVAILABLE')
      hold('HISTORY_UNAVAILABLE_NOT_NO_HISTORY')
    ...
    if (seenOrganizations.has(candidate.organizationId))
      exclude('SAME_NATIVE_ORGANIZATION_IN_THIS_GROUP')
    ...
    if (state.disposition !== 'EXCLUDED') {
      seenOrganizations.add(candidate.organizationId)
      if (recipient) seenRecipients.add(recipient)
    }
    return { ...candidate, disposition: state.disposition, reasons, selected: false }
  })
  const ordered = [
    ...rows.filter((r) => r.disposition === 'ELIGIBLE'),
    ...rows.filter((r) => r.disposition === 'HELD'),
  ]
  ...
  const value = {
    schema: 'torchiko.outreach-cohort-preview/1',
    ...
    shortfall: input.count - selected.size,
    rows,
    notice:
      'Preparation-only cohort. Every individual still requires the existing native research/writer gate and human message review.',
...
export function readCohortMemberState(value: unknown): CohortMemberState {
  ...
  const selection = cohortObject(raw.selection),
    lease = cohortObject(raw.lease)
  if (
    raw.schema !== 'torchiko.outreach-cohort-member/1' ||
    ...
    !id.safeParse(selection.venueId).success ||
    !id.safeParse(selection.organizationId).success ||
    !hash.safeParse(selection.nativeSnapshotHash).success ||
    !Array.isArray(raw.reasons) ||
    raw.reasons.some((reason) => typeof reason !== 'string') ||
    (raw.lease !== null &&
      (!z.string().uuid().safeParse(lease.token).success ||
        !id.safeParse(lease.actorId).success ||
        !id.safeParse(lease.runId).success ||
        typeof lease.expiresAt !== 'string' ||
        !Number.isFinite(Date.parse(lease.expiresAt)))) ||
    (raw.task !== null &&
      (!id.safeParse(cohortObject(raw.task).id).success ||
        !id.safeParse(cohortObject(raw.task).preparationId).success)) ||
    (raw.draft !== null &&
      (!id.safeParse(cohortObject(raw.draft).id).success ||
        !id.safeParse(cohortObject(raw.draft).receiptId).success ||
        !hash.safeParse(cohortObject(raw.draft).contentHash).success ||
        !Number.isInteger(cohortObject(raw.draft).version)))
  )
