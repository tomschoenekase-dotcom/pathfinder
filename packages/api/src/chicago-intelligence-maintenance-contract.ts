...
export const chicagoLifecycleInput = z.discriminatedUnion('action', [
  ...
  z
    .object({
      ...
      supersededByVenueId: id,
      expectedTargetVersion: revision,
    })
    .strict(),
])
...
export const chicagoRankingOverrideInput = z.discriminatedUnion('mode', [
  z
    .object({
      ...override,
      mode: z.literal('set'),
      dimension: z.enum(['productFit', 'attainability', 'contactability']),
...
export const chicagoRankingRefreshInput = z
  .object({
    idempotencyKey: id,
    rationale,
    targets: z
      .array(z.object({ venueId: id, expectedVersion: revision }).strict())
      .min(1)
      .max(100),
  })
  .strict()
  .superRefine((input, context) => {
    if (new Set(input.targets.map((target) => target.venueId)).size !== input.targets.length) {
      context.addIssue({
        ...
        path: ['targets'],
        message: 'Provide each explicit venue ID only once',
      })
    }
  })
