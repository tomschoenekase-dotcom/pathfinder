...
import { readProspectReplyContent, readProspectReplyContentForAgent, retainProspectReplyContent } from './prospect-reply-content'

const selection = {
  messageId: 'message-1', threadId: 'thread-1', organizationId: 'organization-1',
}
...
describe('selected prospect reply content route owner', () => {
  beforeEach(() => {
    ...
    mocks.exactRead.mockResolvedValue({
      rawBodySha256, rawText: 'Raw provider text', replyText: 'Selected reply',
      omittedQuotedText: true, projectionScope: 'CONSERVATIVE_DISPLAY_PROJECTION_NOT_RAW_SOURCE',
      retention: 'TRANSIENT_SOURCE_READ', trust: 'UNTRUSTED_EXTERNAL_CONTENT',
  ...
  it('resolves the exact canonical message and native mailbox before reading one selected source', async () => {
    ...
    expect(mocks.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { id: selection.messageId } }))
    expect(mocks.exactRead).toHaveBeenCalledWith({
      provider: factory.mock.results[0]!.value,
      mailbox: {
        provider: 'GMAIL', providerAccountId: expected.providerAccountId,
        mailboxId: 'native-mailbox', mailboxAddress: 'tomschoenekase@torchiko.com',
        credentialRef: 'native-credential-reference',
      },
      selected: expect.objectContaining({
        canonicalMessageId: selection.messageId, canonicalThreadId: selection.threadId,
        organizationId: selection.organizationId, providerMessageId: expected.providerMessageId,
        providerThreadId: expected.providerThreadId, sourceReference,
        direction: 'INBOUND', bodyRetentionState: 'NOT_STORED',
      }),
    })
  ...
  it.each<[string, Record<string, unknown>]>([
    ...
    ['personal account', { providerAccount: { ...canonicalRow().providerAccount, mailboxAddress: 'personal@example.test' } }],
    ['disconnected account', { providerAccount: { ...canonicalRow().providerAccount, connectionStatus: 'DISCONNECTED' } }],
    ['ambiguous mapping', { thread: { providerMappings: [
      { providerAccountId: 'native-account', providerThreadId: expected.providerThreadId },
      { providerAccountId: 'native-account', providerThreadId: 'another-thread' },
    ] } }],
    ['cross-account extra mapping', { thread: { providerMappings: [
      { providerAccountId: 'native-account', providerThreadId: expected.providerThreadId },
  ...
  it('requires an actor and an unretained body before provider construction', async () => {
    await expect(readProspectReplyContent(selection, ' ', factory as never)).rejects.toMatchObject({ code: 'UNAUTHORIZED' })
    mocks.findUnique.mockResolvedValue(canonicalRow({ bodyRetentionState: 'TEMPORARY' }))
    await expect(readProspectReplyContent(selection, 'admin-1', factory as never)).rejects.toMatchObject({ code: 'CONFLICT' })
    expect(factory).not.toHaveBeenCalled()
    expect(mocks.exactRead).not.toHaveBeenCalled()
  })

  it('rebinds retention to the current canonical mailbox and leaves CAS to the retention owner', async () => {
    const result = await retainProspectReplyContent({ expected, retentionDays: 2 }, 'admin-1', factory as never)
    expect(mocks.retain).toHaveBeenCalledWith({
      provider: factory.mock.results[0]!.value,
      mailbox: expect.objectContaining({
        providerAccountId: expected.providerAccountId,
        mailboxId: 'native-mailbox', credentialRef: 'native-credential-reference',
      }),
      expected, retentionDays: 2, actorId: 'admin-1',
    })
  ...
  it('refuses retention when the selected canonical row disappears or changes account', async () => {
    ...
    await expect(retainProspectReplyContent({ expected, retentionDays: 2 }, 'admin-1', factory as never)).rejects.toMatchObject({ code: 'NOT_FOUND' })
    mocks.findUnique.mockResolvedValueOnce(canonicalRow({ providerAccount: {
      ...canonicalRow().providerAccount, mailboxAddress: 'personal@example.test',
    } }))
    await expect(retainProspectReplyContent({ expected, retentionDays: 2 }, 'admin-1', factory as never)).rejects.toMatchObject({ code: 'PRECONDITION_FAILED' })
    expect(factory).not.toHaveBeenCalled()
    expect(mocks.retain).not.toHaveBeenCalled()
  })

  it('never delegates retention without an authenticated actor', async () => {
    await expect(retainProspectReplyContent({ expected, retentionDays: 2 }, ' ', factory as never))
  ...
  it('binds an agent read to the canonical message and organization territory in one query', async () => {
    ...
    const result = await readProspectReplyContentForAgent(selection, 'agent-1', scope, factory as never)
    ...
    expect(mocks.findFirst).toHaveBeenCalledWith(expect.objectContaining({ where: {
      id: selection.messageId, organizationId: selection.organizationId, threadId: selection.threadId,
  ...
  it('denies an out-of-territory agent read before provider access', async () => {
    ...
    await expect(readProspectReplyContentForAgent(selection, 'agent-1',
      { mode: 'TERRITORIES', territoryIds: ['territory-1'] }, factory as never))
  ...
  it('withholds fetched plaintext if territory or source moves during the provider read', async () => {
    ...
    await expect(readProspectReplyContentForAgent(selection, 'agent-1', scope, factory as never))
      .rejects.toMatchObject({ code: 'NOT_FOUND' })
    expect(mocks.exactRead).toHaveBeenCalledOnce()

    mocks.findFirst.mockResolvedValueOnce(canonicalRow())
