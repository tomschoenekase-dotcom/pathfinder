...
vi.mock('./chicago-intelligence-service', () => ({
  ChicagoIntelligenceError: class extends Error {
    constructor(
  ...
  intelligenceMutation: mocks.mutation,
  saveChicagoRanking: mocks.ranking,
  scopedVenue: mocks.scoped,
}))

import {
  maintainChicagoVenue,
  overrideChicagoVenueRanking,
...
const actor: ChicagoActor = {
  id: 'human-1',
  type: 'HUMAN',
  runId: 'run-1',
  scope: { mode: 'TERRITORIES', territoryIds: ['chicago'] },
...
const common = {
  ...
  venueId: 'venue',
  expectedVersion: 1,
  rationale: 'Reviewed current venue identity and operational status.',
}
const seed = (id: string, territoryId = 'chicago') => ({
  ...
  organization: { id: `org-${id}`, archivedAt: null as Date | null },
  contacts: [] as Array<Record<string, unknown>>,
  intelligenceReviews: [] as Array<{ reason: string }>,
  intelligence: {
    venueId: id,
    revision: 1,
    fields: { original: { value: 'preserved' } } as Record<string, unknown>,
    rankingVersion: 'chicago-venue-ranking/1.0.0',
    rankingInput: {
      venueId: id,
      asOf: '2026-09-21',
      territory: 'Chicago Metro',
      venueType: 'museum',
      exclusionReason: 'Original independent exclusion',
    } as Record<string, unknown>,
    rankingSnapshot: { version: 'chicago-venue-ranking/1.0.0' },
  },
})
...
const tx = {
  prospectVenue: { update: mocks.updateVenue },
  prospectVenueIntelligence: { update: mocks.updateProfile, updateMany: mocks.advance },
}
...
describe('reversible Chicago intelligence maintenance', () => {
  beforeEach(() => {
    ...
    mocks.scoped.mockImplementation(async (id: string, scope: ChicagoActor['scope']) => {
      ...
      if (!venue || (scope.mode !== 'ALL' && !scope.territoryIds.includes(venue.territoryId)))
        throw Object.assign(new Error('Missing or outside scope'), { code: 'NOT_FOUND' })
      return structuredClone(venue)
    })
    mocks.advance.mockImplementation(
      async ({ where }: { where: { venueId: string; revision: number } }) => {
    ...
    mocks.updateVenue.mockImplementation(
      async ({ where, data }: { where: { id: string }; data: Record<string, unknown> }) =>
        Object.assign(venues.get(where.id)!, data),
    )
    mocks.updateProfile.mockImplementation(
      async ({ where, data }: { where: { venueId: string }; data: Record<string, unknown> }) =>
        Object.assign(venues.get(where.venueId)!.intelligence, data),
    )
    mocks.ranking.mockImplementation(async (_tx, venueId, input) => {
      const value = rankChicagoVenue(input)
      snapshots.push({ venueId, version: value.version })
      return value
    })
    ...
    mocks.mutation.mockImplementation(async (operation, input, who, apply) => {
      const key = `${who.id}:${who.runId}:${input.idempotencyKey}`,
        payload = JSON.stringify({ operation, input })
      const prior = receipts.get(key)
      if (prior) {
        if (prior.payload !== payload)
          throw Object.assign(new Error('Different payload'), { code: 'CONFLICT' })
        return { ...prior.result, replayed: true }
      }
      const changed = await apply(tx, `receipt-${input.idempotencyKey}`)
      const result = {
        receiptId: `receipt-${input.idempotencyKey}`,
        replayed: false,
        ...changed.result,
      }
  ...
  it('archives and restores the exact location while preserving identity and the prior exclusion', async () => {
    ...
    const restored = await maintainChicagoVenue(
      { ...common, idempotencyKey: 'restore-1', expectedVersion: 2, action: 'restore' },
      actor,
    )
    expect(restored).toMatchObject({ revision: 3, lifecycle: 'active' })
    expect(venues.get('venue')?.archivedAt).toBeNull()
    expect(venues.get('venue')?.intelligence.rankingInput.exclusionReason).toBe(
      'Original independent exclusion',
    )
    ...
    expect(
      mocks.updateVenue.mock.calls.every(([call]) =>
        Object.keys(call.data).every((key) => ['archivedAt', 'updatedBy'].includes(key)),
      ),
    ).toBe(true)
  })
  ...
  it('supersedes only toward an exact active scoped target without merging either identity', async () => {
    ...
    const result = await maintainChicagoVenue(
      { ...common, action: 'supersede', supersededByVenueId: 'target', expectedTargetVersion: 1 },
      actor,
    )
    expect(result).toMatchObject({
      lifecycle: 'superseded',
      supersededByVenueId: 'target',
      deleted: false,
    })
    expect(venues.get('target')).toEqual(targetBefore)
    expect(venues.get('venue')?.intelligence.rankingInput.exclusionReason).toContain(
      'Superseded by target',
    )
    await maintainChicagoVenue(
      { ...common, action: 'restore', expectedVersion: 2, idempotencyKey: 'undo-supersede' },
      actor,
    )
    expect(venues.get('venue')?.archivedAt).toBeNull()
    expect(venues.get('venue')?.intelligence.rankingInput.exclusionReason).toBe(
  ...
  it('rejects self-supersede, target scope/state/version errors and stale source before writing', async () => {
    const supersede = {
      ...common,
      action: 'supersede',
      supersededByVenueId: 'venue',
      expectedTargetVersion: 1,
    }
    await expect(maintainChicagoVenue(supersede, actor)).rejects.toMatchObject({
      code: 'INVALID_INPUT',
    })
    await expect(
      maintainChicagoVenue({ ...supersede, supersededByVenueId: 'outside' }, actor),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' })
    await expect(
      maintainChicagoVenue(
        { ...supersede, supersededByVenueId: 'target', expectedTargetVersion: 2 },
        actor,
      ),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
    venues.get('target')!.archivedAt = new Date()
    await expect(
      maintainChicagoVenue({ ...supersede, supersededByVenueId: 'target' }, actor),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
    await expect(
      maintainChicagoVenue({ ...common, action: 'archive', expectedVersion: 2 }, actor),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
  ...
  it('denies agent/system overrides and derives human identity/date from trusted context', async () => {
    const input = {
      ...common,
      mode: 'set',
      expectedRankingVersion: CHICAGO_RANKING_VERSION,
      dimension: 'productFit',
      value: 94,
    }
    for (const type of ['AGENT', 'SYSTEM'] as const)
      await expect(overrideChicagoVenueRanking(input, { ...actor, type })).rejects.toMatchObject({
        code: 'FORBIDDEN',
      })
  ...
  it('rejects changed ranking rules, injected actor/date and CAS contention', async () => {
    const input = {
      ...common,
      mode: 'set',
      expectedRankingVersion: 'chicago-venue-ranking/1.0.0',
      dimension: 'productFit',
      value: 94,
    }
    await expect(overrideChicagoVenueRanking(input, actor)).rejects.toMatchObject({
      code: 'CONFLICT',
    })
    await expect(
      overrideChicagoVenueRanking(
    ...
    await expect(
      overrideChicagoVenueRanking(
        { ...input, expectedRankingVersion: CHICAGO_RANKING_VERSION },
        actor,
      ),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
  ...
  it('refreshes bounded explicit IDs, records previous rules and preserves old snapshots and suppression', async () => {
    venues.get('venue')!.contacts = [
      { archivedAt: null, doNotContact: true, permissionState: 'UNKNOWN' },
    ]
    venues.get('venue')!.intelligence.rankingInput.contacts = [
      {
        kind: 'email',
        verified: true,
        suppressed: false,
        roleRelevant: true,
        sourceUrls: ['https://museum.example.org/contact'],
    ...
    const input = {
      ...
      rationale: common.rationale,
      targets: [
        { venueId: 'venue', expectedVersion: 1 },
        { venueId: 'target', expectedVersion: 1 },
      ],
    }
    ...
    expect(await refreshChicagoVenueRankings(input, actor)).toMatchObject({
      replayed: true,
      count: 2,
    })
    expect(mocks.ranking).toHaveBeenCalledTimes(2)
    venues.get('target')!.territoryId = 'outside'
    await expect(refreshChicagoVenueRankings(input, actor)).rejects.toMatchObject({
      code: 'NOT_FOUND',
    })
  })
      ...
      refreshChicagoVenueRankings(
        {
          ...
          targets: [
            { venueId: 'venue', expectedVersion: 1 },
            { venueId: 'target', expectedVersion: 2 },
          ],
        },
        actor,
      ),
  ...
  it('recomputes native suppression after it is cleared without rewriting observed contacts or prior snapshots', async () => {
    ...
    const refresh = (idempotencyKey: string, expectedVersion: number) =>
      refreshChicagoVenueRankings(
        {
          idempotencyKey,
          rationale: common.rationale,
          targets: [{ venueId: 'venue', expectedVersion }],
        },
        actor,
      )
  ...
  it('excludes an organization-archived location during refresh and override without changing location lifecycle', async () => {
    ...
    await refreshChicagoVenueRankings(
      {
        idempotencyKey: 'organization-archive',
        rationale: common.rationale,
        targets: [{ venueId: 'venue', expectedVersion: 1 }],
      },
      actor,
    )
    ...
    await overrideChicagoVenueRanking(
      {
        ...common,
        idempotencyKey: 'organization-override',
        expectedVersion: 2,
        expectedRankingVersion: CHICAGO_RANKING_VERSION,
        mode: 'set',
        dimension: 'productFit',
  ...
  it('enforces strict schemas, bounded batches and maintenance permission before reads', async () => {
    expect(
      chicagoLifecycleInput.safeParse({ ...common, action: 'archive', hardDelete: true }).success,
    ).toBe(false)
    expect(
      chicagoRankingOverrideInput.safeParse({
        ...common,
        mode: 'set',
        expectedRankingVersion: CHICAGO_RANKING_VERSION,
        dimension: 'productFit',
    ...
    expect(
      chicagoRankingRefreshInput.safeParse({
        ...refresh,
        targets: [...refresh.targets, ...refresh.targets],
      }).success,
    ).toBe(false)
    expect(
      chicagoRankingRefreshInput.safeParse({
    ...
    await expect(
      maintainChicagoVenue({ ...common, action: 'archive' }, { ...actor, capabilities: [] }),
