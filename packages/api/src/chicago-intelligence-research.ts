...
import {
  ...
  type ChicagoResearchCandidate,
  type ChicagoResearchGap,
  type ChicagoResearchPreview,
  type ChicagoResearchQueuedJob,
  type ChicagoResearchQueueResult,
  type ChicagoResearchClaimResult,
  type ChicagoResearchCompleteResult,
  type ChicagoResearchReleaseResult,
...
const conflict = (message: string): never => {
  throw new ChicagoIntelligenceError('CONFLICT', message)
}
...
export async function previewChicagoResearch(
  raw: unknown,
  scope: ChicagoScope,
): Promise<ChicagoResearchPreview> {
  const input = chicagoResearchPreviewInput.parse(raw)
  const result = await queryChicagoVenueRows(
    {
      ...input.filters,
      lifecycle: 'active',
      page: 1,
      pageSize: 100,
      sorts: [
        { field: 'researchPriority', direction: 'desc' },
        { field: 'name', direction: 'asc' },
      ],
    },
    scope,
  )
  const all = result.items.slice(0, 10000),
    total = result.total
  ...
  for (const row of all) {
    ...
    if (
      row.revision > 0 &&
      row.ranking.researchGaps.length &&
      !['excluded', 'intentionally-unranked'].includes(row.ranking.state)
    )
      cell.eligible.push(row)
    cells.set(key, cell)
  }
  const eligible = [...cells.values()].reduce((n, cell) => n + cell.eligible.length, 0)
  const groups = [...cells.values()]
    .filter((cell) => cell.eligible.length)
    .sort((a, b) => {
      // Give sparsely represented cells an opportunity before selecting the same famous category repeatedly.
      const score = (cell: typeof a) =>
        Math.max(...cell.eligible.map((row) => row.ranking.researchPriority.value ?? 100))
      return (
        score(b) - score(a) ||
        a.rows.length - b.rows.length ||
        a.eligible[0]!.venueId.localeCompare(b.eligible[0]!.venueId)
      )
    })
  ...
  for (let round = 0; candidates.length < input.limit && round < 10; round++) {
    ...
    for (const cell of groups) {
      ...
      candidates.push({
        ...
        stale: row.stale,
        gaps: uniqueGaps(row.ranking.researchGaps),
        priority: row.ranking.researchPriority.value ?? 100,
        coverageCell: {
          city: row.city,
          state: row.state,
          category: row.category,
          venues: cell.rows.length,
          needingResearch: cell.eligible.length,
        },
      })
      added = true
...
async function activeVenue(venueId: string, actor: ChicagoActor, tx: ChicagoTransaction) {
  ...
  if (venue.archivedAt || venue.organization.archivedAt)
    throw new ChicagoIntelligenceError(
      'CONFLICT',
      'Archived locations or organizations cannot receive new research work',
    )
  return venue
}
...
async function latestRequest(tx: ChicagoTransaction, jobId: string): Promise<QueuedJob | null> {
  const receipts = await tx.prospectIntelligenceReceipt.findMany({
    where: { operation: 'research-queue', result: { path: ['jobs'], array_contains: [{ jobId }] } },
      ...
      ? result.jobs
          ...
          .filter(
            (job) =>
              ...
              typeof job.requestReceiptId === 'string',
          )
      : []
...
export async function queueChicagoResearch(raw: unknown, actor: ChicagoActor) {
  ...
  for (const selection of input.selections)
    details.set(selection.venueId, await getChicagoVenue(selection.venueId, actor.scope))
  return intelligenceMutation('research-queue', input, actor, async (tx, receiptId) => {
    ...
    for (const selection of input.selections) {
      ...
      if (
        venue.intelligence?.revision !== selection.expectedVersion ||
        detail.revision !== selection.expectedVersion
      )
        conflict('Venue evidence changed; refresh research gaps before queuing')
      if (seenOrganizations.has(venue.organizationId))
        throw new ChicagoIntelligenceError(
          'INVALID_INPUT',
          'Select one location per organization in a bounded batch; the native research job is organization-owned',
        )
      seenOrganizations.add(venue.organizationId)
      const gaps = selection.gapKeys.map((key) =>
        detail.researchGaps.find((gap) => gap.key === key),
      )
      if (gaps.some((gap) => !gap))
        conflict('A selected gap is no longer present; refresh the venue before queuing')
      const exactGaps = gaps as ChicagoResearchGap[]
      const existing = await tx.prospectResearchJob.findUnique({
        where: { organizationId: venue.organizationId },
      })
      const previousRequest = existing ? await latestRequest(tx, existing.id) : null
      const keepsGeneration = Boolean(
        existing && ['QUEUED', 'CLAIMED'].includes(existing.status) && previousRequest,
      )
      if (
        keepsGeneration &&
        (previousRequest!.venueId !== venue.id ||
          intelligenceHash(previousRequest!.gaps) !== intelligenceHash(exactGaps))
      )
        conflict(
          'This organization already has a different bounded research request; finish it with an honest complete or incomplete outcome before changing its scope',
        )
      before.push({ venueId: venue.id, job: existing })
      let job = existing
      const priority = Math.min(100, Math.max(...exactGaps.map((gap) => gap.priority)))
      if (!job)
        job = await tx.prospectResearchJob.create({
          data: {
            id: `chrj_${intelligenceHash(venue.organizationId).slice(0, 32)}`,
        ...
        const updated = await tx.prospectResearchJob.updateMany({
        ...
        if (updated.count !== 1)
          conflict('Research job changed while requeueing; refresh its state')
      ...
      jobs.push({
        ...
        generation: keepsGeneration
          ? previousRequest!.generation
          : (previousRequest?.generation ?? 0) + 1,
    ...
    return {
      venueId: null,
      before,
      after: { schema: SCHEMA, jobs },
      result: { schema: SCHEMA, jobs, outreachAuthorized: false },
    }
  }) as Promise<ChicagoResearchQueueResult>
}

async function queueBinding(
  tx: ChicagoTransaction,
  ...
  if (!receipt || receipt.operation !== 'research-queue' || result.schema !== SCHEMA || !binding)
    throw new ChicagoIntelligenceError(
      'INVALID_INPUT',
      'Claim requires the exact queued venue and durable Chicago research receipt',
    )
  const latest = await latestRequest(tx, jobId)
  if (
    !latest ||
    latest.generation !== binding.generation ||
    latest.requestReceiptId !== binding.requestReceiptId
  )
    conflict(
      'This queue receipt was superseded by a newer request generation; use the current research receipt',
    )
...
export async function claimChicagoResearch(raw: unknown, actor: ChicagoActor) {
  ...
  return intelligenceMutation('research-claim', input, actor, async (tx, receiptId) => {
    ...
    if (binding.organizationId !== venue.organizationId)
      conflict('Venue organization changed after the research request')
    const job = await tx.prospectResearchJob.findUnique({ where: { id: input.jobId } })
    if (!job || job.organizationId !== venue.organizationId)
      throw new ChicagoIntelligenceError(
        'NOT_FOUND',
        'Research job is not owned by the selected Chicago venue organization',
      )
    const now = new Date(),
    ...
    const claimable =
      job.status === 'QUEUED' ||
      (job.status === 'CLAIMED' && job.claimExpiresAt && job.claimExpiresAt <= now)
    if (!claimable)
      conflict(
        'Research job is leased or terminal; inspect the current job before requesting more work',
      )
    const claimed = await tx.prospectResearchJob.updateMany({
      ...
      data: {
        status: 'CLAIMED',
        claimToken,
        claimOwnerId: actor.id,
        claimAgentRunId: actor.runId,
    ...
    if (job.claimToken)
      await tx.prospectResearchAttempt.updateMany({
        where: { claimToken: job.claimToken, status: 'CLAIMED' },
        data: {
          status: 'EXPIRED',
          completedAt: now,
          outcomeReason: 'Lease expired; history retained before a new bounded claim',
        },
      })
    const attemptId = `chra_${receiptId}`
    await tx.prospectResearchAttempt.create({
      data: {
        id: attemptId,
        jobId: job.id,
        claimToken,
        agentRunId: actor.runId,
        agentIdentityId: actor.id,
        promptIdentity: SCHEMA,
        leaseExpiresAt,
        usage: intelligenceJson({
          chicagoResearch: {
            schema: SCHEMA,
            venueId: venue.id,
            queueReceiptId: input.queueReceiptId,
            gaps: binding.gaps,
          },
        }),
      },
...
async function ownedLease(
  tx: ChicagoTransaction,
  input: { venueId: string; jobId: string; claimToken: string },
  actor: ChicagoActor,
  ...
  if (!job || job.organizationId !== venue.organizationId)
    throw new ChicagoIntelligenceError(
      'NOT_FOUND',
      'Research job is outside the selected venue organization',
    )
  if (
    job.status !== 'CLAIMED' ||
    job.claimToken !== input.claimToken ||
    job.claimOwnerId !== actor.id ||
    job.claimAgentRunId !== actor.runId ||
    !job.claimExpiresAt ||
    (!allowExpired && job.claimExpiresAt <= now)
  )
    conflict('Research lease is expired, replaced or belongs to another actor/run')
  const attempt = await tx.prospectResearchAttempt.findUnique({
  ...
  if (
    ...
    attempt.agentIdentityId !== actor.id ||
    attempt.agentRunId !== actor.runId ||
    binding.schema !== SCHEMA ||
    binding.venueId !== venue.id
  )
    conflict('Attempt is not bound to this exact Chicago research lease')
...
export async function completeChicagoResearch(raw: unknown, actor: ChicagoActor) {
  ...
  return intelligenceMutation('research-complete', input, actor, async (tx, receiptId) => {
    ...
    if (input.unknowns.some((unknown) => !gaps.some((gap) => gap.key === unknown.gapKey)))
      throw new ChicagoIntelligenceError(
        'INVALID_INPUT',
        'Unknown outcomes must identify a gap from the exact claimed request',
      )
    const updated = await tx.prospectResearchJob.updateMany({
      where: {
        id: job.id,
      ...
      data: {
        ...
        claimAgentRunId: null,
        claimExpiresAt: null,
        terminalReason: input.summary,
        completedAt: now,
      },
    })
    if (updated.count !== 1)
      conflict('Lease changed or expired while completing; no completion evidence committed')
    ...
    for (const [index, evidence] of input.evidence.entries()) {
      ...
      await tx.prospectSourceEvidence.create({
        data: {
          ...
          organizationId: venue.organizationId,
          venueId: venue.id,
          sourceType: 'FIRST_PARTY_RESEARCH_OBSERVATION',
          sourceUrl: evidence.url,
          sourceLabel: evidence.statement,
          researchedAt: new Date(`${evidence.researchedAt}T00:00:00Z`),
          createdBy: actor.id,
          capturedValue: intelligenceJson({
            ...
            meaning:
              'Research observation only; use the versioned evidence/field mutation to apply canonical facts or rankings',
          }),
        },
      })
      evidenceIds.push(id)
    ...
    const finished = await tx.prospectResearchAttempt.updateMany({
      where: { id: attempt.id, claimToken: input.claimToken, status: 'CLAIMED' },
      data: {
        status: 'COMPLETED',
        outcome: input.outcome,
        outcomeReason: input.summary,
        completedAt: now,
        usage: intelligenceJson({ ...object(attempt.usage), chicagoCompletion: completion }),
      },
    })
    if (finished.count !== 1)
      conflict('Attempt changed while completing; original history preserved')
    const result = {
      jobId: job.id,
      venueId: venue.id,
      attemptId: attempt.id,
...
export async function releaseChicagoResearch(raw: unknown, actor: ChicagoActor) {
  ...
  return intelligenceMutation('research-release', input, actor, async (tx) => {
    ...
    const released = await tx.prospectResearchJob.updateMany({
      where: {
        id: job.id,
        status: 'CLAIMED',
        claimToken: input.claimToken,
    ...
    const changed = await tx.prospectResearchAttempt.updateMany({
      where: { id: attempt.id, status: 'CLAIMED', claimToken: input.claimToken },
      data: {
