...
const evidence = {
  url: 'https://www.chicagoparkdistrict.com/parks',
  researchedAt: '2026-09-22',
  statement: 'Official venue page identifies the park and its visitor facilities.',
  firstParty: true,
}
const add = {
  ...
  evidence,
  territoryRationale: 'The official venue address is in Chicago.',
}
const change = {
  ...
  expectedVersion: 1,
  field: 'website',
  value: 'https://www.chicagoparkdistrict.com',
  evidence,
}
...
describe('Chicago public source admission', () => {
  ...
  it.each([
    ...
    'http://museum.example.org/visit',
    'HTTPS://MUSEUM.EXAMPLE.ORG/visit',
    'https://www.museum.example.org./visit',
    'https://museé.example.org/about',
  ...
  it.each([
    'javascript:alert(1)',
    'file:///C:/private.txt',
    'ftp://museum.example.org',
    'data:text/html,Hello',
    '//museum.example.org/visit',
    ...
    'http://localhost.',
    'http://museum.local',
    'http://museum.internal',
    'http://museum.intranet',
    'http://museum.lan',
    'http://museum.home',
    'http://museum.invalid',
    'http://museum.test',
    ...
    'http://-museum.org',
    'http://museum-.org',
    'http://museum_site.org',
    `https://${'a'.repeat(64)}.org`,
    'https://user:secret@museum.example.org',
    'https://user@museum.example.org',
    'https://@museum.example.org',
    'https://museum.example.org\\@127.0.0.1',
    'https://museum.example.org\n/visit',
  ...
  it('does not turn ordinary non-website field values into URL validation', () => {
    expect(
      chicagoChangeInput.safeParse({ ...change, field: 'venueType', value: 'History museum' })
        .success,
    ).toBe(true)
    expect(
      chicagoChangeInput.safeParse({
        ...change,
        field: 'description',
        value: 'Visitor information and history.',
      }).success,
    ).toBe(true)
  })

  it.each(['2024-02-29', '2026-02-28', '2026-09-22', '2000-02-29'])(
    'accepts exact nonfuture calendar dates: %s',
    (researchedAt) => {
  ...
  it.each([
    ...
    ' 2026-09-22',
    'not-a-date',
  ])('rejects rollover, malformed and future dates: %s', (researchedAt) => {
    expect(chicagoEvidence.safeParse({ ...evidence, researchedAt }).success).toBe(false)
    expect(
      chicagoAddInput.safeParse({ ...add, evidence: { ...evidence, researchedAt } }).success,
    ).toBe(false)
    expect(
      chicagoChangeInput.safeParse({ ...change, evidence: { ...evidence, researchedAt } }).success,
    ).toBe(false)
  })
  ...
  it('admits distinct supported observation kinds with no override or authority fields', () => {
    ...
    const observations = [
      {
        ...
        value: 80,
        reason: 'Official collection pages provide substantial interpretive content.',
      },
      {
        kind: 'attainability',
        key: 'pilotScope',
        value: 60,
        reason: 'A single documented visitor center provides a bounded pilot.',
      },
      { kind: 'contact', channel: 'email', value: 'info@museum.example.org', roleRelevant: null },
      {
        kind: 'contact',
        channel: 'phone',
        value: '+1 (312) 555-0123 ext. 42',
        roleRelevant: false,
      },
      {
        kind: 'contact',
        channel: 'form',
        value: 'https://museum.example.org/contact',
        roleRelevant: true,
      },
      {
        kind: 'contact',
        channel: 'website',
        value: 'https://museum.example.org',
        roleRelevant: null,
      },
      { kind: 'source' },
    ]
    for (const observation of observations)
      expect(chicagoAppendEvidenceInput.safeParse({ ...envelope, observation }).success).toBe(true)
    for (const observation of [
      { kind: 'source', override: 100 },
      { ...observations[0], value: 101 },
      { ...observations[0], value: NaN },
      { ...observations[0], value: Infinity },
      { ...observations[0], value: -1 },
      { ...observations[0], key: 'emailAvailable' },
      { ...observations[1], key: 'knowledgeRichness' },
      { ...observations[0], reason: 'guess' },
      { ...observations[0], actor: 'tom' },
    ...
    expect(
      chicagoAppendEvidenceInput.safeParse({
        ...envelope,
  ...
  it.each([
    ['email', 'not-an-email'],
    ['email', 'mailto:info@museum.example.org'],
    ['email', 'Name <info@museum.example.org>'],
    ['email', 'info@museum.local'],
    ['phone', '911'],
    ['phone', 'call the owner'],
    ['phone', '1'.repeat(16)],
    ['form', 'javascript:alert(1)'],
    ['website', 'file:///secret'],
    ['form', 'http://172.16.0.1'],
    ...
    expect(
      chicagoAppendEvidenceInput.safeParse({
        ...
        expectedVersion: 1,
        evidence,
        observation: { kind: 'contact', channel, value, roleRelevant: null },
      }).success,
    ).toBe(false)
  })
})
