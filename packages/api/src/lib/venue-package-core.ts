...
import { buildNotRunSemanticScan, sortVenuePackageIssues } from './venue-package-semantic-analysis'
...
export type VenuePackageApprovalCommand = VenuePackageLifecycleCommand & {
...
export async function approveVenuePackageLifecycle(input: {
  db: DbClient
  tenantId: string
  venueId?: string
  actor: VenuePackageLifecycleActor
  command: VenuePackageApprovalCommand
}) {
    ...
    return await approveVenuePackageAction(
      {
        ...
        actor: input.actor,
        load: async (tx, scope) => {
          const pkg = await findPackage(tx as DbClient, scope.tenantId, scope.id)
          return !pkg || (input.venueId !== undefined && pkg.venueId !== input.venueId) ? null : pkg
        },
        auditState,
        validate: async (tx, existing) => {
          ...
          if (evidence.payloadHash !== input.command.acknowledgedPayloadHash) {
            throw new TRPCError({
              code: 'BAD_REQUEST',
              message: 'The acknowledged venue-package payload does not match this immutable draft',
            })
          }
          if (evidence.warningDigest !== input.command.acknowledgedWarningDigest) {
            throw new TRPCError({
              code: 'BAD_REQUEST',
...
export async function applyVenuePackageLifecycle(request: {
  db: DbClient
  tenantId: string
  venueId?: string
  actor: VenuePackageLifecycleActor
  ...
  if (request.venueId !== undefined) {
    const scoped = await request.db.venuePackage.findFirst({
      where: { id: request.command.id, tenantId: request.tenantId, venueId: request.venueId },
      select: { id: true },
    })
    if (!scoped) throw new TRPCError({ code: 'NOT_FOUND', message: 'Venue package not found' })
  }
  return applyVenuePackageLifecycleImplementation(request)
}

export async function revertVenuePackageLifecycle(request: {
  db: DbClient
  tenantId: string
  venueId?: string
  actor: VenuePackageLifecycleActor
  ...
  return revertVenuePackageLifecycleImplementation(request)
}

async function applyVenuePackageLifecycleImplementation(request: {
  db: DbClient
  tenantId: string
  venueId?: string
  actor: VenuePackageLifecycleActor
  ...
  await lockVenueContentMutation(request.db, { tenantId, venueId: existing.venueId })
  existing = await findPackage(request.db, tenantId, request.command.id)
  if (!existing) throw new TRPCError({ code: 'NOT_FOUND', message: 'Venue package not found' })
  ...
  try {
    if (payload.schemaVersion === 3) {
      ...
      return finalizePackageApply({
        ...
        expectedUpdatedAt: request.command.expectedUpdatedAt,
        commandKey: request.command.commandKey,
        actor: request.actor,
        appliedEntities,
      })
    }
    ...
    if (payload.schemaVersion === 2) {
      const before = venueSnapshot(await assertVenue(request.db, tenantId, existing.venueId))
      const venueData = changedVenuePatchData(payload, before)
      if (Object.keys(venueData).length > 0) {
        const changedVenue = await request.db.venue.updateMany({
          where: { id: existing.venueId, tenantId },
          data: venueData,
        })
        if (changedVenue.count !== 1) conflict('Venue changed during package application')
        const after = venueSnapshot(await assertVenue(request.db, tenantId, existing.venueId))
    ...
    const places =
      payload.places.length === 0
        ? []
        : await request.db.place.createManyAndReturn({
    ...
    const knowledgeEntries =
      payload.knowledgeEntries.length === 0
        ? []
        : await request.db.venueKnowledgeEntry.createManyAndReturn({
...
async function revertVenuePackageLifecycleImplementation(request: {
  db: DbClient
  tenantId: string
  venueId?: string
  actor: VenuePackageLifecycleActor
  ...
  const current = await contentState(request.db, tenantId, existing.venueId)
  if (
  ...
  if (
    (await packageStateDigest(request.db, tenantId, existing.venueId, payload.schemaVersion)) !==
  ...
  const removedKnowledge = await request.db.venueKnowledgeEntry.deleteMany({
    where: {
      tenantId,
      venueId: existing.venueId,
  ...
  const removedPlaces = await request.db.place.deleteMany({
    where: {
      tenantId,
      venueId: existing.venueId,
  ...
  if ('schemaVersion' in manifest && manifest.venue) {
    const restoredVenue = await request.db.venue.updateMany({
      where: { id: existing.venueId, tenantId },
