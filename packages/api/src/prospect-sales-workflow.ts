...
import { assertLocalCrmSalesComponentsEnvironment, assertAuthenticatedCrmSalesComponentsEnvironment, inspectAuthenticatedCrmSalesComponents, invokeLocalCrmSalesComponents, readAuthenticatedTorchikoWritingGuide, readLocalTorchikoWritingGuide, TORCHIKO_SAVED_WRITING_GUIDE, TORCHIKO_SAVED_WRITING_GUIDE_SOURCE } from '@pathfinder/config/local-crm-sales-components'
...
export function assertLocalSalesEnvironment(env: Record<string, string | undefined> = process.env) {
  ...
  try { assertLocalCrmSalesComponentsEnvironment(env) }
  catch { throw new ProspectSalesError('FORBIDDEN', 'Local sales component owner is unavailable') }
}
...
export async function readAuthenticatedSalesReadiness(
  ...
  const deployment = env.DEPLOYMENT_ENV === 'production' || env.APP_ENV === 'production'
    ? 'hosted-production' as const
    : env.NODE_ENV === 'development' ? 'development-instance' as const
  ...
  return {
    ...
    component: { state: component.state, reason: component.reason,
      executionVerified: false },
    writingGuide: { id: TORCHIKO_SAVED_WRITING_GUIDE,
      sourceRef: TORCHIKO_SAVED_WRITING_GUIDE_SOURCE, ...component.guide },
    writer: { preparationSource: SALES_PREPARATION_SOURCE,
      taskSchema: 'torchiko.native-writer-task/1' as const,
      resultSchema: 'torchiko.native-writer-result/1' as const,
      automaticGeneration: false, SEND_AUTHORIZED: false },
    nextAction: component.state === 'unconfigured'
...
export async function selectedWritingGuideCurrent(
  reference: Record<string, unknown>, runtime: SalesRuntime,
  readGuide: typeof readAuthenticatedTorchikoWritingGuide = runtime === 'local' ? readLocalTorchikoWritingGuide : readAuthenticatedTorchikoWritingGuide,
): Promise<boolean> {
...
async function requireSelectedWritingGuideCurrent(component: ComponentOutput,
  runtime: SalesRuntime) {
  if (!await selectedWritingGuideCurrent(obj(component.writingReference), runtime))
    throw new ProspectSalesError('CONFLICT',
...
export function assertSavedGuideSelection(input: PrepareInput, runtime: SalesRuntime) {
  if ((input.savedWritingGuide &&
      (input.writingReference || runtime !== 'authenticated-admin' ||
    ...
    throw new ProspectSalesError('INVALID_INPUT',
...
export async function resolveSelectedWritingReference(
  input: PrepareInput, runtime: SalesRuntime,
  ...
  try { guide = await readGuide() }
  catch { throw new ProspectSalesError('CONFLICT',
...
function assertSalesRuntime(runtime: SalesRuntime) {
  ...
  try { assertAuthenticatedCrmSalesComponentsEnvironment() }
  catch { throw new ProspectSalesError('FORBIDDEN', 'Authenticated private sales component owner is unavailable') }
}

export async function invokeSalesComponents(payload: {
...
}, runtime: SalesRuntime = 'local'): Promise<ComponentOutput> {
  assertSalesRuntime(runtime)
  try {
    return await invokeLocalCrmSalesComponents({
      ...payload, native: nativeWithReplyProjections(payload.native),
    }, runtime)
  } catch (error) {
    throw new ProspectSalesError(
      ...
      runtime === 'authenticated-admin'
        ...
        : error instanceof Error ? error.message : 'Local component failed without a usable result',
    )
  }
}
...
export async function getNativeSalesWorkflow(
  venueId: string, runtime: SalesRuntime = 'local',
  ...
  return withTenantIsolationBypass(async () => {
    ...
    const savedGuideCurrent = await selectedWritingGuideCurrent(
      obj(preparedComponent.writingReference), runtime)
    let launchAttachmentsCurrent = true
    try { await requireCurrentProspectLaunchAttachments(venueId, launchAttachmentsFromSnapshot(preparedComponent)) }
    catch { launchAttachmentsCurrent = false }
    ...
    try {
      component = await invokeSalesComponents({ action: 'evaluate', native,
        ...(typeof selectedThreadId === 'string' ? { selectedThreadId } : {}),
      }, runtime)
      if (temporaryRecipe) {
    ...
    const displayPreparation = temporaryRecipe && temporaryCurrent
      ? obj(component.preparation) : prepared
    ...
    const draftGuideCurrent = draftSource?.id === source?.id ? savedGuideCurrent
    ...
    return {
      ...
      writerTask,
      writerHold,
      operational: await readNativeOperationalView(venueId,
        (current, stored) => verifyNativeOriginRuntime(current, stored, runtime)),
      ...
      gate: {
        ...
        notices: [
          ...
          ...(route.kind === 'email' && typeof route.recipient === 'string' &&
      ...
      correspondence: native.threads.length && (native.threads.length === 1 || typeof selectedThreadId === 'string')
        ? {
      ...
      threadCandidates: native.threads.map((thread) => ({
        ...
        sourceComplete: native.threadCoverage.find((item) => item.threadId === thread.id)?.complete === true,
        sourceIssues: native.threadCoverage.find((item) => item.threadId === thread.id)?.issues ?? [],
      })),
      preparation: source
        ? {
            id: source.id,
            ...
            launchAttachments: launchAttachmentsFromSnapshot(preparedComponent).map(venueLaunchAssetDescriptor),
            writingReference:
      ...
      draft: currentDraft
        ? {
            ...
            launchAttachments: launchAttachmentsFromSnapshot(grounding).map(venueLaunchAssetDescriptor),
            ...
            writerAttribution: grounding.writerProvenance &&
...
export async function viewOrImmutableImportReceipt(
  ...
  if (typeof evidence.draftId !== 'string' || !evidence.draftId.trim() ||
      (evidence.meaningReviewId !== null && evidence.meaningReviewId !== undefined &&
  ...
  try {
    ...
    const currentViewFailure = error instanceof ProspectSalesError
      ? error.code === 'NOT_FOUND' ? 'RECORD_NOT_FOUND' as const
        : error.code === 'CONFLICT' || error.code === 'INVALID_INPUT' ? 'STATE_CONFLICT' as const
          : 'ACCESS_OR_POLICY_HOLD' as const
      : 'READ_FAILED' as const
    return {
      schema: 'torchiko.native-writer-import-receipt-only/1',
      venueId, originalSnapshotHash, writerImportReceipt,
      currentViewAvailable: false, currentViewFailure, SEND_AUTHORIZED: false, senderAvailable: false,
    }
  }
}

export async function applyNativeSalesAction(
  action: NativeSalesAction, actor: SalesActor,
  runtime: SalesRuntime = 'local',
): Promise<SalesActionResponse> {
  assertSalesRuntime(runtime)
  ...
  if (actor.type === 'AGENT') {
    if (runtime !== 'authenticated-admin' ||
  ...
  if (parsed.action === 'importWriterResult') {
    ...
    if (receipt) {
      return viewOrImmutableImportReceipt(
        parsed.input.venueId, parsed.input.result.binding.nativeSnapshotHash, receipt, true,
  ...
  const importedReceipt = await withTenantIsolationBypass(async () => {
    ...
    if (parsed.action !== 'prepare' && parsed.action !== 'admitEvidence')
      await requireSelectedWritingGuideCurrent(
        decodeSalesComponent(selectedSource?.capturedValue), runtime)
    const storedThreadId = obj(obj(decodeSalesComponent(selectedSource?.capturedValue).correspondence).projection).thread_id
    const selectedThreadId = parsed.action === 'prepare'
      ? parsed.input.selectedThreadId : storedThreadId
    const threadSelection = typeof selectedThreadId === 'string' ? { selectedThreadId } : {}
    const evaluated = await invokeSalesComponents({ action: 'evaluate', native, ...threadSelection }, runtime)
    ...
    if (
      ...
      const component = await invokeSalesComponents({ action: 'admission', native, admission }, runtime)
      ...
      if (writingReference &&
          !await selectedWritingGuideCurrent(writingReference, runtime))
        throw new ProspectSalesError('CONFLICT',
          'STALE_SELECTED_WRITING_GUIDE: reload the guide descriptor before preparing')
  ...
  if (parsed.action === 'importWriterResult' && importedReceipt) return viewOrImmutableImportReceipt(
    parsed.input.venueId, parsed.input.result.binding.nativeSnapshotHash,
    importedReceipt, false, (venueId) => getNativeSalesWorkflow(venueId, runtime),
  )
