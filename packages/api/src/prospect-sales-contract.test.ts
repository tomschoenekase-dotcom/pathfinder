...
describe('optional selected writing reference boundary', () => {
  ...
  it('requires actual selected bytes; a source locator alone cannot fetch an unavailable file', () => {
    ...
    const reference = {
      label: 'Test guidance',
      sourceRef: 'file:owner-selected',
      text,
      sha256: sha(text),
    }
    expect(
      salesPrepareInput.parse({ ...base, writingReference: reference }).writingReference,
    ).toEqual(reference)
    expect(
      salesPrepareInput.safeParse({ ...base, writingReference: { ...reference, text: undefined } })
        .success,
    ).toBe(false)
    expect(
      salesPrepareInput.safeParse({ ...base, writingReference: { ...reference, sha256: 'bad' } })
        .success,
    ).toBe(false)
  })
})
