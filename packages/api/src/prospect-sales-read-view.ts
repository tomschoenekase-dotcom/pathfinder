...
export function projectNativeCrmRead(native: NativeSalesSnapshot) {
  return {
    ...
    venue: {
      ...
      city: native.venue.city,
      region: native.venue.region,
    ...
    importLineage: native.importRecords.map((row) => ({
      ...
      sourceWorkbookHash: row.sourceWorkbookHash,
      recordHash: row.recordHash,
      venueId: row.canonicalVenueId,
      contactId: row.canonicalContactId,
      evidenceId: row.canonicalEvidenceId,
      locator: obj(obj(row.rawPayload)._source),
    ...
    correspondence: native.threads.map((thread) => ({
      ...
      omittedFromThisRead: Math.max(
        0,
        thread._count.messages - Math.min(thread.messages.length, 20),
      ),
      ...
      messages: thread.messages.slice(-20).map((message) => {
        ...
        return {
          ...
          bodyRetentionState: message.bodyRetentionState,
    ...
    notice:
      'Bounded read projection only. Raw bodies/source references remain with their original owner. Unknown quotation formats are not certified clean. Use the exact writer task, not this excerpt, for versioned submission.',
