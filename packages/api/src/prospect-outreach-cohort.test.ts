...
vi.mock('@pathfinder/db', () => ({
  db: {},
  readNativeSalesSnapshot: vi.fn(),
  withTenantIsolationBypass: async (fn: () => unknown) => fn(),
}))
...
const actor: OutreachCohortActor = {
  id: 'synthetic-actor',
  type: 'HUMAN',
  runId: 'synthetic-run',
  scope: { mode: 'ALL' },
...
function fixture() {
  const group = {
    ...
    cohortSnapshot: {
      schema: 'torchiko.outreach-preparation-cohort/1',
      question: 'A bounded synthetic test.',
    },
    members: [1, 2].map((i) => ({
      id: `m${i}`,
      organizationId: `o${i}`,
  ...
  const client: any = {
    prospectOutreachCampaign: {
      findUnique: vi.fn(async () => structuredClone(group)),
      findFirst: vi.fn(async () => ({ id: 'cursor' })),
      findMany: vi.fn(),
      updateMany: vi.fn(async () => ({ count: 1 })),
    },
    prospectOrganization: { count: vi.fn(async () => 2) },
    prospectCampaignMember: { updateMany: vi.fn(async () => ({ count: 1 })) },
    prospectActivity: { create: vi.fn(), findUnique: vi.fn() },
    prospectOutreachDraft: { findUnique: vi.fn() },
  }
  ...
  const readView = vi.fn(
    async (venueId: string) =>
      ({
        venueId,
        snapshotHash: hash,
        routing: { kind: 'email', value: `${venueId}@example.invalid` },
  ...
  return {
    ...
    client,
    readView,
    service: createOutreachCohortService({
      client,
      readView,
...
describe('native cohort service integration', () => {
  ...
  it('exports actual list, with exact bounded cursor pagination and complete organization predicates', async () => {
    ...
    client.prospectOutreachCampaign.findMany.mockResolvedValue(
    ...
    const result = await service.list(
      ...
      { ...actor, scope: { mode: 'TERRITORIES', territoryIds: ['t1'] } },
    )
    expect(result.items.map((v) => v.cohortId)).toEqual(['one', 'two'])
    ...
    await expect(service.list({ cursor: 'foreign-or-deleted' }, actor)).rejects.toMatchObject({
      code: 'NOT_FOUND',
    })
  })
  it('rejects missing correspondence/maintenance authority and empty scopes before data access', async () => {
    const { client, service } = fixture()
    await expect(
      service.list({}, { ...actor, capabilities: ['prospects.read'] }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' })
    await expect(
      service.list({}, { ...actor, scope: { mode: 'TERRITORIES', territoryIds: [] } }),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' })
    await expect(
      service.reserve(
        {},
        { ...actor, capabilities: ['prospects.read', 'prospects.correspondence.read'] },
      ),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' })
  ...
  it('revalidates a live grant before returning directory content', async () => {
    const { client, readView } = fixture()
    client.prospectOutreachCampaign.findMany.mockResolvedValue([])
    const service = createOutreachCohortService({
      client,
      readView,
      revalidate: async () => {
        throw Error('Grant withdrawn')
      },
    })
  ...
  it('retains every unavailable member as held rather than pretending no draft or history', async () => {
    const { service, readView } = fixture()
    readView.mockRejectedValue(Error('Actual reader unavailable'))
    const review = await service.read({ cohortId: 'group' }, actor)
    expect(review.rows).toHaveLength(2)
    expect(review.readyForHumanReview).toBe(0)
    expect(
      review.rows.every(
        (r) =>
          r.nativeRead === 'UNAVAILABLE' &&
          r.suppression.blocked &&
          r.correspondenceState === 'UNAVAILABLE',
      ),
    ).toBe(true)
    await expect(
      service.acknowledge(
  ...
  it('rejects partial territory access and group state changes during a read', async () => {
    ...
    client.prospectOrganization.count.mockResolvedValueOnce(1)
    await expect(service.read({ cohortId: 'group' }, actor)).rejects.toMatchObject({
      code: 'FORBIDDEN',
    })
    client.prospectOutreachCampaign.findUnique
      .mockResolvedValueOnce(structuredClone(group))
      .mockResolvedValueOnce({ ...structuredClone(group), status: 'PAUSED' })
    await expect(service.read({ cohortId: 'group' }, actor)).rejects.toMatchObject({
      code: 'CONFLICT',
    })
  })
  it('rejects agent acknowledgement/group control and exact stale review hashes', async () => {
    ...
    await expect(service.acknowledge(input, { ...actor, type: 'AGENT' })).rejects.toMatchObject({
      code: 'FORBIDDEN',
    })
  ...
  it('rejects a forged import receipt before it can replace any native checkpoint', async () => {
    const { group, client, service } = fixture()
    Object.assign(group.members[0]!.selection, {
      state: 'PREPARING',
      task: { id: 'task', preparationId: 'preparation' },
      lease: { token, actorId: actor.id, runId: actor.runId, expiresAt: '2026-09-23T21:10:00Z' },
    })
    client.prospectActivity.findUnique.mockResolvedValue({
