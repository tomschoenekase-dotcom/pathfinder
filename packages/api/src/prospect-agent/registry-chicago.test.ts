...
vi.mock('../chicago-intelligence-research', () => ({
  previewChicagoResearch: mocks.preview,
  queueChicagoResearch: mocks.queue,
  claimChicagoResearch: mocks.claim,
  completeChicagoResearch: mocks.complete,
  releaseChicagoResearch: mocks.release,
}))
vi.mock('../chicago-intelligence-maintenance', () => ({
  maintainChicagoVenue: mocks.lifecycle,
  refreshChicagoVenueRankings: mocks.refresh,
}))
vi.mock('@pathfinder/db', () => ({
  db: { agentRun: { findFirst: mocks.run } },
...
vi.mock('../prospect-sales-workflow', () => ({
  applyNativeSalesAction: vi.fn(),
  getNativeSalesWorkflow: vi.fn(),
  readAuthenticatedSalesReadiness: vi.fn(),
}))
vi.mock('../chicago-intelligence-service', () => ({
  listChicagoVenues: mocks.list,
  getChicagoVenue: mocks.get,
  getChicagoHealth: mocks.health,
  addChicagoVenue: mocks.add,
  appendChicagoEvidence: mocks.append,
  changeChicagoVenue: mocks.change,
  proposeChicagoDuplicate: mocks.relationship,
}))
...
const context = (
  capabilities: VerifiedProspectAgentContext['capabilities'] = [
    'prospects.read',
    'prospects.maintain',
  ],
  ...
  modelName: 'fixture',
  promptIdentity: 'venue-research@1',
  requestedOperation: 'venue-maintenance',
  correlationId: invocation.correlationId,
})
const evidence = {
  url: 'https://museum.example.org/visit',
  researchedAt: '2026-09-22',
  statement: 'The official visitor page identifies the physical venue and location.',
  firstParty: true,
}
const add = {
  ...
  city: 'Chicago',
  state: 'IL',
  territoryId: 'chicago-territory',
  website: 'https://museum.example.org',
  evidence,
  territoryRationale: 'The official address places the physical venue in Chicago.',
}
const change = {
...
describe('Chicago venue intelligence agent adapters', () => {
  ...
  it('keeps bounded research queue writes separate from read and legacy research grants', async () => {
    const input = {
      ...
      selections: [{ venueId: 'venue', expectedVersion: 3, gapKeys: ['knowledgeRichness'] }],
    }
    await expect(
      registry(['prospects.read', 'prospects.research']).callTool(
        'torchiko.prospects.queue_venue_research',
        input,
        invocation,
      ),
    ...
    expect(mocks.queue).toHaveBeenCalledWith(
      input,
      expect.objectContaining({ id: 'agent', type: 'AGENT', runId: 'run', scope }),
    )
    await expect(
      registry().callTool(
        'torchiko.prospects.queue_venue_research',
  ...
  it('passes exact leased completion and retry results without granting override or outreach', async () => {
    const input = {
      ...
      outcome: 'BLOCKED',
      summary: 'Requested evidence remains unknown within the research boundary.',
      evidence: [],
      unknowns: [
        {
          gapKey: 'knowledgeRichness',
          reason: 'The first-party source did not establish the requested fact.',
        },
      ],
    }
    ...
    expect(
      await registry().callTool('torchiko.prospects.complete_venue_research', input, invocation),
    ).toBe(receipt)
    expect(mocks.complete).toHaveBeenCalledWith(
      input,
      expect.objectContaining({ scope, type: 'AGENT' }),
    )
    expect(
      registry()
        .listTools()
        .some((tool) => tool.name.includes('override')),
    ).toBe(false)
  })
  ...
  it('routes reversible lifecycle through the trusted actor and validates target versions', async () => {
    const input = {
      ...
      expectedVersion: 3,
      action: 'archive',
      rationale: 'Retain this location as inactive without deleting its history.',
    }
    ...
    await expect(
      registry().callTool(
        'torchiko.prospects.maintain_venue_lifecycle',
        { ...input, action: 'supersede', supersededByVenueId: 'target' },
        invocation,
      ),
    ).rejects.toThrow()
    await expect(
      registry().callTool(
        'torchiko.prospects.refresh_venue_rankings',
        {
          idempotencyKey: 'refresh',
          rationale: 'Materialize the current ranking version with original history retained.',
          targets: [
            { venueId: 'venue', expectedVersion: 3 },
            { venueId: 'venue', expectedVersion: 3 },
          ],
        },
        invocation,
  ...
  it('uses the exact UI directory input and frozen scope, with bounded pagination', async () => {
    const input = {
      ...
      page: 2,
      pageSize: 25,
      sorts: [{ field: 'productFit', direction: 'desc' }],
    }
    const result = { items: [], total: 40, page: 2, pageSize: 25 }
    mocks.list.mockResolvedValue(result)
    expect(await registry().callTool('torchiko.prospects.search_venues', input, invocation)).toBe(
      result,
    )
    expect(mocks.list).toHaveBeenCalledWith(chicagoDirectoryInput.parse(input), scope)
    await expect(
      registry().callTool('torchiko.prospects.search_venues', { pageSize: 101 }, invocation),
    ).rejects.toThrow()
    await expect(
      registry().callTool(
        'torchiko.prospects.search_venues',
        { scope: { mode: 'ALL' } },
        invocation,
      ),
    ).rejects.toThrow()
  ...
  it('requires read capability independently of maintenance and does not grant correspondence access', async () => {
    await expect(
      registry(['prospects.maintain']).callTool(
        'torchiko.prospects.read_venue',
        { venueId: 'venue' },
        invocation,
      ),
    ).rejects.toMatchObject({ code: 'CAPABILITY_REQUIRED' })
    await expect(
      registry(['prospects.maintain']).callTool(
        'torchiko.prospects.read_selected_reply_content',
        { organizationId: 'org', threadId: 'thread', messageId: 'message' },
        invocation,
      ),
    ).rejects.toMatchObject({ code: 'CAPABILITY_REQUIRED' })
  ...
  it('requires maintenance on live identity and frozen run separately', async () => {
    for (const [live, frozen] of [
      [['prospects.read'], ['prospects.maintain']],
      [['prospects.maintain'], ['prospects.read']],
    ]) {
      mocks.run.mockResolvedValue({
        id: 'run',
        tenantId: 'tenant',
        venueId: 'bridge-venue',
        initiatedById: 'operator',
        requestedOperation: 'venue-maintenance',
        modelProvider: null,
        modelName: null,
        scopeSnapshot: {
          accessCapabilities: frozen,
          prospectScope: scope,
          promptIdentity: 'venue-research@1',
        },
        agentIdentity: { id: 'agent', accessCapabilities: live },
      })
      await expect(
        createProspectAgentRegistry().callTool('torchiko.prospects.add_venue', add, invocation),
      ).rejects.toMatchObject({ code: 'CAPABILITY_REQUIRED' })
    }
    ...
    mocks.run.mockResolvedValue({
      ...
      modelName: null,
      scopeSnapshot: {
        accessCapabilities: ['prospects.maintain'],
        prospectScope: scope,
        promptIdentity: 'venue-research@1',
      },
      agentIdentity: { id: 'agent', accessCapabilities: ['prospects.maintain'] },
    })
    expect((await resolveVerifiedProspectAgentContext(invocation)).capabilities).toEqual([
      'prospects.maintain',
    ])
  })
  ...
  it('derives every mutation actor from authenticated context and rejects caller authority', async () => {
    ...
    expect(mocks.add).toHaveBeenCalledWith(add, {
      id: 'agent',
      type: 'AGENT',
      runId: 'run',
      scope,
      capabilities: ['prospects.read', 'prospects.maintain'],
    })
    for (const injected of [
      ...
      await expect(
        registry().callTool('torchiko.prospects.add_venue', { ...add, ...injected }, invocation),
      ).rejects.toThrow()
    }
  ...
  it('propagates service territory denial without bypass or fallback', async () => {
    mocks.add.mockRejectedValue(
      Object.assign(new Error('Outside frozen territory'), { code: 'FORBIDDEN' }),
    )
    await expect(
      registry().callTool(
        'torchiko.prospects.add_venue',
        { ...add, territoryId: 'other-territory' },
        invocation,
      ),
  ...
  it('preserves exact stale version and retry outcomes without manufacturing success', async () => {
    const conflict = Object.assign(new Error('Expected 1, current 2'), {
      code: 'CONFLICT',
      currentVersion: 2,
    })
    mocks.change.mockRejectedValueOnce(conflict)
    await expect(
      registry().callTool('torchiko.prospects.change_venue', change, invocation),
    ...
    expect(await registry().callTool('torchiko.prospects.change_venue', change, invocation)).toBe(
      receipt,
    )
    expect(mocks.change.mock.calls[1]?.[0].idempotencyKey).toBe('research-1')
    await expect(
      registry().callTool(
        'torchiko.prospects.change_venue',
  ...
  it('requires first-party public evidence and preserves unknowns by rejecting invented fields', async () => {
    for (const invalid of [
      { ...evidence, firstParty: false },
      { ...evidence, url: 'http://127.0.0.1/private' },
    ...
    await expect(
      registry().callTool(
        'torchiko.prospects.change_venue',
        { ...change, field: 'privateContactEmail' },
        invocation,
      ),
    ).rejects.toThrow()
    expect(mocks.add).not.toHaveBeenCalled()
  ...
  it('reads detail, explanation and data health through the same scoped service owner', async () => {
    const detail = {
      venueId: 'venue',
      ranking: { version: 'v1', productFit: { value: null } },
      researchGaps: [{ key: 'knowledgeRichness' }],
      sources: [],
    }
    mocks.get.mockResolvedValue(detail)
    expect(
      await registry().callTool('torchiko.prospects.read_venue', { venueId: 'venue' }, invocation),
    ).toBe(detail)
    expect(
      await registry().callTool(
        'torchiko.prospects.explain_venue',
        { venueId: 'venue' },
        invocation,
      ),
    ).toEqual({ venueId: 'venue', ranking: detail.ranking, researchGaps: detail.researchGaps })
    ...
    await expect(
      registry().callTool(
        'torchiko.prospects.read_data_health',
        { scope: { mode: 'ALL' } },
        invocation,
      ),
    ).rejects.toThrow()
  ...
  it('routes duplicate and same-operator proposals to review without merge authority', async () => {
    const input = {
      ...
      expectedVersion: 2,
      otherVenueId: 'other',
      relation: 'same-operator',
      reason: 'The official operator lists both venues.',
      evidence,
    }
    const receipt = { receiptId: 'receipt', reviewId: 'review', replayed: false }
    mocks.relationship.mockResolvedValue(receipt)
    expect(
      await registry().callTool('torchiko.prospects.propose_venue_relationship', input, invocation),
    ).toBe(receipt)
    expect(mocks.relationship).toHaveBeenCalledWith(
      input,
      expect.objectContaining({ scope, type: 'AGENT' }),
    )
    expect(
      registry()
        .listTools()
        .find((tool) => tool.name === 'torchiko.prospects.propose_venue_relationship')
        ?.humanReviewRequired,
    ).toBe(true)
  })
  ...
  it('publishes bounded honest tool contracts and keeps maintenance separate from existing research/writer capabilities', () => {
    ...
    const additions = tools.filter((tool) =>
      [
        'search_venues',
        'read_venue',
        'explain_venue',
        ...
        'add_venue',
        'change_venue',
        'append_venue_evidence',
        'propose_venue_relationship',
      ].some((name) => tool.name === `torchiko.prospects.${name}`),
    )
    ...
    expect(
      additions
        ...
        .every((tool) => tool.capability === 'prospects.maintain' && tool.idempotent),
    ).toBe(true)
  ...
  it('appends supported observations using frozen maintenance authority and preserves retry/conflict results', async () => {
    const input = {
      ...
      observation: {
        kind: 'fit',
        key: 'knowledgeRichness',
        value: 90,
        reason: 'Official collection pages document substantial venue-specific content.',
      },
    }
    await expect(
      registry(['prospects.read', 'prospects.research']).callTool(
        'torchiko.prospects.append_venue_evidence',
        input,
        invocation,
      ),
    ).rejects.toMatchObject({ code: 'CAPABILITY_REQUIRED' })
    ...
    expect(
      await registry().callTool('torchiko.prospects.append_venue_evidence', input, invocation),
    ).toBe(receipt)
    expect(mocks.append).toHaveBeenCalledWith(input, {
      id: 'agent',
      type: 'AGENT',
      runId: 'run',
      scope,
    ...
    await expect(
      registry().callTool('torchiko.prospects.append_venue_evidence', input, invocation),
    ).rejects.toBe(conflict)
    await expect(
      registry().callTool(
        'torchiko.prospects.append_venue_evidence',
    ...
    await expect(
      registry().callTool(
        'torchiko.prospects.append_venue_evidence',
        { ...input, observation: { kind: 'override', value: 100 } },
        invocation,
      ),
    ).rejects.toThrow()
