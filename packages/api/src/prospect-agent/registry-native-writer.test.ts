...
function context(
  ...
  return {
    ...
    modelName: 'synthetic-unit',
    promptIdentity: 'crm-writer@1',
    requestedOperation: 'explicit_native_writer',
...
describe('verified native prospect writer tools', () => {
  beforeEach(() => {
    ...
    mocks.action.mockResolvedValue({
      organizationId: 'org',
      venueId: 'native-venue',
      snapshotHash: hash,
      preparation: { id: 'prep', stale: false },
    })
    mocks.native.mockResolvedValue({
      organization: { id: 'org' },
      venue: { id: 'native-venue' },
      snapshotHash: hash,
    })
  ...
  it('requires distinct live/frozen writer and correspondence capabilities', async () => {
    await expect(
      registry(['prospects.correspondence.read']).callTool(
        'torchiko.prospects.prepare_native_writer',
        { ...scope, expectedSnapshotHash: hash },
        invocation,
      ),
    ).rejects.toMatchObject({ code: 'CAPABILITY_REQUIRED' })
    await expect(
      registry(['prospects.native-writer']).callTool(
        'torchiko.prospects.read_native_writer_task',
        scope,
        invocation,
      ),
    ).rejects.toMatchObject({ code: 'CAPABILITY_REQUIRED' })
  ...
  it('reads first, explicitly prepares, then exports only an exact scoped no-send task', async () => {
    ...
    expect(first).toMatchObject({
      ...
      preparationId: null,
      task: null,
      hold: 'PREPARATION_REQUIRED',
      writingGuide: { state: 'available', sha256: hash },
      SEND_AUTHORIZED: false,
    })
    expect(JSON.stringify(first)).not.toContain('text')
    const prepared = await registry().callTool(
      'torchiko.prospects.prepare_native_writer',
      { ...scope, expectedSnapshotHash: hash },
      invocation,
    )
    ...
    expect(mocks.action).toHaveBeenCalledWith(
      { action: 'prepare', input: { venueId: 'native-venue', expectedSnapshotHash: hash } },
      expect.objectContaining({ type: 'AGENT', id: 'agent' }),
      'authenticated-admin',
    )
    const task = await registry().callTool(
      'torchiko.prospects.read_native_writer_task',
      scope,
      invocation,
    )
  ...
  it('returns a safe hold and exact snapshot when the component is not installed', async () => {
    ...
    expect(first).toMatchObject({
      snapshotHash: hash,
      task: null,
      hold: 'COMPONENT_UNAVAILABLE',
      writingGuide: { state: 'unavailable' },
    })
  ...
  it('does not return a task after the exact organization changes during the read', async () => {
    mocks.native
      ...
      .mockResolvedValueOnce({
        organization: { id: 'moved-org' },
        venue: { id: 'native-venue' },
        snapshotHash: 'b'.repeat(64),
      })
    await expect(
      registry().callTool('torchiko.prospects.read_native_writer_task', scope, invocation),
  ...
  it('retains exact replay receipt after mutable view moves, pending operator review', async () => {
    ...
    const imported = await registry().callTool(
      'torchiko.prospects.import_native_writer_result',
      { ...scope, expectedSnapshotHash: hash, result },
      invocation,
    )
  ...
  it('rejects changed identity, revoked lease and model assessment without importing', async () => {
      ...
      registry().callTool(
        'torchiko.prospects.import_native_writer_result',
        {
          ...
          result: {
            ...result,
            assessment: {
              reviewer: { kind: 'model', identity: 'synthetic unit writer' },
              assessments: [],
              answers: [],
              unsupportedClaims: [],
            },
    ...
    expect(
      registry()
        .listTools()
        .map((tool) => tool.name),
    ).not.toContain('torchiko.prospects.review_native_writer')
  })
})
