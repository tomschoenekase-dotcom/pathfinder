...
export function assertLocalChicagoIntelligenceEnvironment(env: Record<string, string | undefined>) {
  ...
  if (
    ...
    !(target.pathname === '/pathfinder_disposable_chicago_intelligence_20260922' ||
      env.TORCHIKO_CHICAGO_RETAINED_LOCAL_ENABLED === '1' && target.pathname === '/pathfinder_disposable_crm_research_20260919') ||
    target.search ||
    target.hash
  )
    throw new Error('Local Chicago workspace requires its exact disposable database or explicitly enabled retained local CRM')
