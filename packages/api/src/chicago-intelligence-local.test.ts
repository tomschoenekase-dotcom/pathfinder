...
describe('Chicago preview isolated authority', () => {
  it('requires a separate explicit opt-in for the exact retained local CRM and never accepts another database',()=>{
    const retained={...env,DATABASE_URL:env.DATABASE_URL.replace('pathfinder_disposable_chicago_intelligence_20260922','pathfinder_disposable_crm_research_20260919')}
    expect(()=>assertLocalChicagoIntelligenceEnvironment(retained)).toThrow()
    expect(()=>assertLocalChicagoIntelligenceEnvironment({...retained,TORCHIKO_CHICAGO_RETAINED_LOCAL_ENABLED:'1'})).not.toThrow()
    expect(()=>assertLocalChicagoIntelligenceEnvironment({...retained,TORCHIKO_CHICAGO_RETAINED_LOCAL_ENABLED:'1',NODE_ENV:'production'})).toThrow()
    expect(()=>assertLocalChicagoIntelligenceEnvironment({...retained,TORCHIKO_CHICAGO_RETAINED_LOCAL_ENABLED:'1',DATABASE_URL:retained.DATABASE_URL.replace('crm_research_20260919','unrelated')})).toThrow()
  })
