...
const input = {
  providerAccountId: 'SYNTHETIC-account-record',
  expectedUpdatedAt: when,
  actorId: 'SYNTHETIC-unit-admin',
}
const account = () => ({
  id: input.providerAccountId,
  provider: 'GMAIL',
  mailboxAddress: 'tomschoenekase@torchiko.com',
  externalAccountId: 'SYNTHETIC-mailbox',
  credentialReferenceId: 'SYNTHETIC-reference-not-a-secret',
...
describe('existing mailbox reconciliation access without activation', () => {
  ...
  it('never requests all accounts or fabricates a human actor', async () => {
    await expect(
      requestProspectMailboxReconciliation({ ...input, providerAccountId: '*' }),
    ).rejects.toThrow()
