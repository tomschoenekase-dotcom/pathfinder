...
function table(map: Map<string, any>, kind: string) {
  return {
    async findUnique({ where }: any) {
      return [...map.values()].find((row) => matches(row, where)) ?? null
    },
    async create({ data }: any) {
      ...
      const row = {
        ...
        updatedAt: new Date(),
        createdAt: new Date(),
        status: kind === 'job' ? 'QUEUED' : 'CLAIMED',
        ...data,
      }
      map.set(row.id, row)
    ...
    async updateMany({ where, data }: any) {
      if (kind === 'job' && fake.failJobCas) {
        fake.failJobCas = false
        return { count: 0 }
      }
      let count = 0
...
vi.mock('./chicago-intelligence-service', () => {
  ...
  const scoped = async (venueId: string, scope: any) => {
    ...
    if (
      !row ||
      row.territoryId !== 'chi' ||
      (scope.mode === 'TERRITORIES' && !scope.territoryIds.includes('chi'))
    )
      throw new ErrorWithCode('NOT_FOUND', 'out of scope')
    return structuredClone(row)
  }
  return {
    ChicagoIntelligenceError: ErrorWithCode,
    intelligenceHash: (value: unknown) => hash(value),
    ...
    getChicagoVenue: async (venueId: string, scope: any) => {
      ...
      return {
        venueId,
        revision: row.intelligence.revision,
        researchGaps: [
          {
            key: 'firstPartySources',
            reason: 'Verify first-party source ownership.',
            priority: 90,
          },
          {
            key: 'knowledgeRichness',
            reason: 'Knowledge richness remains unknown.',
            priority: 100,
          },
        ],
      }
    ...
    intelligenceMutation: async (operation: string, input: any, actor: any, apply: any) => {
      if (!actor.id || !actor.runId || !actor.capabilities.includes('prospects.maintain'))
        throw new ErrorWithCode('FORBIDDEN', 'missing grant')
      const id = hash([actor.id, actor.runId, input.idempotencyKey]),
      ...
      try {
        const tx = {
          prospectResearchJob: table(fake.jobs, 'job'),
          prospectResearchAttempt: table(fake.attempts, 'attempt'),
          prospectSourceEvidence: table(fake.evidence, 'evidence'),
          prospectIntelligenceReceipt: {
            findUnique: async ({ where }: any) => fake.receipts.get(where.id) ?? null,
            findMany: async ({ where }: any) =>
              [...fake.receipts.values()].filter(
                (receipt) =>
                  receipt.operation === where.operation &&
                  receipt.result.jobs.some(
                    (job: any) => job.jobId === where.result.array_contains[0].jobId,
                  ),
              ),
          },
        }
        ...
        maps.forEach((map, i) => {
          map.clear()
          snapshots[i]!.forEach((row, key) => map.set(key, row))
        })
        throw error
      }
    },
...
const actor: ChicagoActor = {
  ...
  type: 'AGENT',
  runId: 'run-1',
  scope: { mode: 'TERRITORIES', territoryIds: ['chi'] },
...
const evidence = {
  ...
  statement: 'The official museum visitor page describes its public collection.',
  firstParty: true,
}
async function lease() {
  const queue = await queueChicagoResearch(
    { idempotencyKey: 'queue-1', selections: [selection] },
    actor,
  )
  const claimed = await claimChicagoResearch(
    {
      idempotencyKey: 'claim-1',
      venueId: 'v1',
      jobId: queue.jobs[0]!.jobId,
      queueReceiptId: queue.receiptId,
      leaseSeconds: 60,
    },
    actor,
  )
...
beforeEach(() => {
  ...
  for (const map of [fake.venues, fake.jobs, fake.attempts, fake.receipts, fake.evidence])
    map.clear()
  fake.directory = []
  fake.failJobCas = false
...
describe('Chicago bounded research lifecycle', () => {
  it('limits queue/lease and refuses invented authority, invalid dates and unsupported completion', () => {
    expect(
      chicagoResearchQueueInput.safeParse({
        idempotencyKey: 'q',
        selections: Array.from({ length: 11 }, (_, i) => ({ ...selection, venueId: `v${i}` })),
      }).success,
    ).toBe(false)
    expect(
      chicagoResearchQueueInput.safeParse({
    ...
    expect(
      chicagoResearchClaimInput.safeParse({
        idempotencyKey: 'q',
        venueId: 'v1',
        jobId: 'j',
        queueReceiptId: 'r',
    ...
    const completion = {
      ...
      venueId: 'v1',
      jobId: 'j',
      claimToken: '9ab096ed-79cb-4cce-a754-fd123091c9a4',
      outcome: 'RESEARCHED',
      summary: 'Completed the bounded source review.',
      evidence: [evidence],
      unknowns: [],
    }
    expect(chicagoResearchCompleteInput.safeParse(completion).success).toBe(true)
    expect(
      chicagoResearchCompleteInput.safeParse({
        ...completion,
        evidence: [{ ...evidence, researchedAt: '2026-02-30' }],
      }).success,
    ).toBe(false)
    expect(chicagoResearchCompleteInput.safeParse({ ...completion, evidence: [] }).success).toBe(
      false,
    )
    expect(
      chicagoResearchCompleteInput.safeParse({
        ...completion,
        unknowns: [
          { gapKey: 'firstPartySources', reason: 'Source ownership could not be proved.' },
        ],
      }).success,
    ).toBe(false)
    expect(
      chicagoResearchCompleteInput.safeParse({ ...completion, outcome: 'BLOCKED' }).success,
    ).toBe(false)
  })
  it('preview stratifies across coverage cells and never queues work', async () => {
    fake.directory = ['v1', 'v2', 'v3'].map((venueId, i) => ({
      venueId,
      organizationId: `o${i}`,
      name: venueId,
      city: i === 2 ? 'Evanston' : 'Chicago',
      state: 'IL',
      category: 'museum',
      revision: 1,
      stale: true,
      ranking: {
        ...
        researchPriority: { value: 100 },
        researchGaps: [
          { key: 'firstPartySources', reason: 'Missing first-party source.', priority: 90 },
        ],
      },
    }))
  ...
  it('stores exact derived gaps and deduplicates native organization jobs across request keys', async () => {
    ...
    expect(other.jobs[0]).toMatchObject({
      jobId: first.jobs[0]!.jobId,
      deduplicated: true,
      gaps: [
        { key: 'firstPartySources', reason: 'Verify first-party source ownership.', priority: 90 },
      ],
    })
  })
  it('rechecks every bulk venue scope on receipt replay and rejects stale gap snapshots atomically', async () => {
    ...
    await expect(
      queueChicagoResearch(input, { ...actor, scope: { mode: 'TERRITORIES', territoryIds: [] } }),
    ...
    await expect(
      queueChicagoResearch(
        { idempotencyKey: 'invented', selections: [{ ...selection, gapKeys: ['privateEmail'] }] },
        actor,
      ),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
  })
  it('requires maintain capability and exact queue receipt/venue binding', async () => {
    await expect(
      queueChicagoResearch(
        { idempotencyKey: 'x', selections: [selection] },
        { ...actor, capabilities: ['prospects.read'] },
      ),
    ).rejects.toMatchObject({ code: 'FORBIDDEN' })
    const queued = await queueChicagoResearch(
      { idempotencyKey: 'q', selections: [selection] },
      actor,
    )
    await expect(
      claimChicagoResearch(
        {
          idempotencyKey: 'c',
          venueId: 'v2',
          jobId: queued.jobs[0]!.jobId,
          queueReceiptId: queued.receiptId,
        },
        actor,
      ),
  ...
  it('claims through CAS and recovers the exact token after a lost claim response', async () => {
    ...
    const retry = await claimChicagoResearch(
      {
        idempotencyKey: 'claim-1',
        venueId: 'v1',
        jobId: claimed.jobId,
  ...
  it('expiry reclaim retains expired attempt and rejects the stale former owner', async () => {
    ...
    const next = await claimChicagoResearch(
      {
      ...
      completeChicagoResearch(
        {
          ...
          claimToken: claimed.claimToken,
          outcome: 'RESEARCHED',
          summary: 'Completed source-backed research.',
          evidence: [evidence],
          unknowns: [],
        },
        actor,
  ...
  it('exact completion replay preserves evidence and unknowns without needing the consumed lease', async () => {
    ...
    const input = {
      ...
      summary: 'Public page found; operator ownership remains unresolved.',
      evidence: [evidence],
      unknowns: [
        {
          gapKey: 'firstPartySources',
          reason: 'The page does not establish the exact venue operator.',
        },
      ],
    }
    ...
    expect(fake.attempts.get(claimed.attemptId).usage.chicagoCompletion.unknowns).toEqual(
      input.unknowns,
    )
    await expect(
      completeChicagoResearch(
        { ...input, summary: 'Changed result under an old completion key.' },
        actor,
      ),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
  })
  it('expired completion and wrong run cannot commit evidence; own release is replayable', async () => {
    ...
    await expect(
      completeChicagoResearch(input, { ...actor, runId: 'other-run' }),
    ).rejects.toMatchObject({ code: 'CONFLICT' })
    vi.setSystemTime(new Date('2026-09-22T12:01:01Z'))
    ...
    const release = {
      ...
      jobId: claimed.jobId,
      claimToken: claimed.claimToken,
      reason: 'Source unavailable; release the unfinished bounded request.',
    }
    const first = await releaseChicagoResearch(release, actor)
    expect(first).toMatchObject({ status: 'QUEUED', expired: true })
    expect(await releaseChicagoResearch(release, actor)).toMatchObject({
  ...
  it('terminal requeue advances generation and rejects obsolete request receipts despite equal timestamps', async () => {
    ...
    await completeChicagoResearch(
      {
        ...
        claimToken: claimed.claimToken,
        outcome: 'NEEDS_REVIEW',
        summary: 'Source ownership remains unknown after the bounded attempt.',
        evidence: [],
        unknowns: [
          {
            gapKey: 'firstPartySources',
            reason: 'No current first-party ownership source was found.',
          },
        ],
      },
      actor,
    )
    const nextQueue = await queueChicagoResearch(
      { idempotencyKey: 'requeue', selections: [selection] },
      actor,
    )
    ...
    const nextClaim = await claimChicagoResearch(
      {
        idempotencyKey: 'new-claim',
        venueId: 'v1',
        jobId: claimed.jobId,
        queueReceiptId: nextQueue.receiptId,
      },
      actor,
    )
