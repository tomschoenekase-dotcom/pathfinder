...
export function launchMimeBoundary(operationId: string, rfcMessageId: string): string {
  return `pathfinder-venue-${createHash('sha256').update(JSON.stringify([operationId, rfcMessageId])).digest('hex').slice(0, 40)}`
}

export function launchMimeParts(textBody: string, assets: readonly VenueLaunchAsset[], boundary: string): string {
  if (assets.length !== 1) throw new Error('Exactly one venue QR is required for multipart MIME')
  const asset = assets[0]!
  const text = Buffer.from(textBody, 'utf8').toString('base64').match(/.{1,76}/gu)?.join('\r\n') ?? ''
...
export function matchesLaunchAttachments(
  ...
  return expected.every((asset, index) => {
    ...
    if (!candidate || candidate.filename !== asset.filename ||
        candidate.mimeType !== asset.mimeType || candidate.sizeBytes !== asset.sizeBytes) return false
    ...
    if (bytes.toString('base64url') !== candidate.contentBase64Url.replace(/=+$/u, '') ||
        bytes.length !== asset.sizeBytes) return false
