...
import { createGmailCorrespondenceProvider, GmailApiError } from './gmail'
...
describe('createGmailApiClient', () => {
  it('fetches bounded QR bytes only for an explicit SENT recovery lookup', async () => {
    ...
    const full = { ...message('m1'), labelIds: ['SENT'], payload: {
      headers: [
      ...
      parts: [
        { mimeType: 'text/plain', filename: '', body: { data: Buffer.from('hello').toString('base64url'), size: 5 } },
        { mimeType: asset.mimeType, filename: asset.filename, body: { attachmentId: 'a1', size: bytes.length } },
      ], body: { size: 0 },
    } }
    const request = vi.fn().mockImplementation(async (url: string) => {
      ...
      if (url.endsWith('/messages/m1/attachments/a1')) return json({ data: bytes.toString('base64url'), size: bytes.length })
      throw new Error(`unexpected request ${url}`)
    })
    const client = createGmailApiClient({ fetch: request, apiBaseUrl: 'https://gmail.test/v1' })
    await client.getMessage({ accessToken: 'token', mailboxAddress: recoveryMailbox.mailboxAddress, messageId: 'm1' })
    expect(request).toHaveBeenCalledTimes(1)
    const found = await client.findByRfcMessageId({ accessToken: 'token',
      mailboxAddress: recoveryMailbox.mailboxAddress, rfcMessageId: '<send@example.test>',
  ...
  it.each([
    ...
    'passes HTTP normalization metadata into provider recovery and holds a %s',
    async (kind) => {
      ...
      const full = {
        ...
        payload: {
          ...
          headers: [
            { name: 'Message-ID', value: '<send@example.test>' },
            { name: 'From', value: 'outreach@torchiko.com' },
            { name: 'To', value: 'person@example.org' },
            { name: 'Subject', value: 'Subject' },
  ...
  it('hydrates every unique history addition in bounded chunks without losing IDs', async () => {
    const ids = Array.from({ length: 101 }, (_, index) => `m${index + 1}`)
    const request = vi.fn().mockImplementation(async (url: string) => {
      const messageId = /^https:\/\/gmail\.test\/v1\/users\/outreach%40torchiko\.com\/messages\/(.+)\?format=full$/u.exec(url)?.[1]
      if (messageId) return json(message(decodeURIComponent(messageId)))
      return json({
        historyId: '102',
        history: [
          {
            messagesAdded: [
              ...ids.slice(0, 100).map((id) => ({ message: { id } })),
              { message: { id: ids[100] } },
              { message: { id: ids[0] } },
            ],
          },
    ...
    expect(page.historyId).toBe('102')
    expect(page.messages.map(({ id }) => id)).toEqual(ids)
    expect(request).toHaveBeenCalledTimes(1 + ids.length)
  })

  it('returns definitive missing history additions for durable quarantine', async () => {
    const request = vi.fn().mockImplementation(async (url: string) => {
      const messageId = /^https:\/\/gmail\.test\/v1\/users\/outreach%40torchiko\.com\/messages\/(.+)\?format=full$/u.exec(url)?.[1]
      if (messageId === 'missing') return json({ error: 'missing' }, 404)
      if (messageId) return json(message(decodeURIComponent(messageId)))
      return json({
        historyId: '102',
        history: [{ messagesAdded: [{ message: { id: 'present' } }, { message: { id: 'missing' } }] }],
      })
    })
    const client = createGmailApiClient({ fetch: request, apiBaseUrl: 'https://gmail.test/v1' })
    ...
    await expect(
      ...
      historyId: '102',
      messages: [expect.objectContaining({ id: 'present' })],
      unavailableMessageIds: ['missing'],
    })
  })

  it('does not classify authorization or transient hydration failures as missing', async () => {
    for (const status of [401, 503]) {
      const request = vi.fn().mockImplementation(async (url: string) => {
        const messageId = /^https:\/\/gmail\.test\/v1\/users\/outreach%40torchiko\.com\/messages\/(.+)\?format=full$/u.exec(url)?.[1]
        if (messageId) return json({ error: 'unavailable' }, status)
        return json({
          historyId: '102',
          history: [{ messagesAdded: [{ message: { id: 'm1' } }] }],
        })
      ...
      ).rejects.toMatchObject({ kind: status === 401 ? 'AUTHENTICATION' : 'TRANSIENT' })
    }
  })

  it('anchors full reconciliation before enumerating the first page', async () => {
    const calls: string[] = []
    const request = vi.fn().mockImplementation(async (url: string) => {
      calls.push(url)
      if (url.endsWith('/profile')) return json({ emailAddress: 'outreach@torchiko.com', historyId: 'anchor-1' })
      if (url.includes('/messages?')) return json({ messages: [], nextPageToken: 'page-2' })
      throw new Error(`unexpected request ${url}`)
    })
    const client = createGmailApiClient({ fetch: request, apiBaseUrl: 'https://gmail.test/v1' })
    ...
    ).resolves.toMatchObject({ historyId: 'anchor-1', nextPageToken: 'page-2' })
