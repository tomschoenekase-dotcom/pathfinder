...
beforeEach(() => {
  ...
  vi.mocked(recordProspectInboundReplyAction).mockResolvedValue({ activityId: 'reply-activity' } as never)
})
...
describe('Prisma inbound correspondence store', () => {
  it('binds a receipt event ID to one exact message reference', async () => {
    ...
    await expect(store.receiveReceipt({
      provider: 'GMAIL',
      providerAccountId: mailbox.providerAccountId,
  ...
  it('does not let an older failed attempt overwrite a newer processed receipt', async () => {
    const updateMany = vi.spyOn(db.prospectEmailWebhookReceipt, 'updateMany')
    ...
    await expect(store.markReceiptState({
      receiptId: 'receipt-1',
      attemptCount: 1,
  ...
  it('publishes one reply signal across failed publication, retry, and replay', async () => {
    ...
    vi.spyOn(db.correspondenceProviderAccount, 'findUniqueOrThrow')
      .mockResolvedValue({ provider: 'FAKE' } as never)
    const marker = {
      emailMessageId: 'canonical-message-1',
      eventType: 'crm.reply_received.signal',
    }
    const findMarker = vi.spyOn(db.prospectEmailEvent, 'findUnique')
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(marker as never)
    const createMarker = vi.spyOn(db.prospectEmailEvent, 'create')
    ...
    expect(createMarker).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({
  ...
  it('reads back a concurrent receipt insert by exact provider identity', async () => {
    ...
    const findReceipt = vi.spyOn(db.prospectEmailWebhookReceipt, 'findUnique')
      .mockResolvedValueOnce(null)
  ...
  it('commits only if the account still owns the cursor and mailbox', async () => {
    const updateMany = vi.spyOn(db.correspondenceProviderAccount, 'updateMany')
  ...
  it('clears only the expired cursor observed by the current run', async () => {
    ...
    await expect(store.clearExpiredSyncCursor({ mailbox, expectedCursor: 'expired-history' }))
  ...
  it('reuses one quarantine identity for replayed unmatched provider messages', async () => {
    const create = vi.spyOn(db.prospectInboundQuarantine, 'create')
      .mockResolvedValueOnce({ id: 'created' } as never)
      .mockRejectedValueOnce({ code: 'P2002' })
    vi.spyOn(db.correspondenceProviderAccount, 'findUnique').mockResolvedValue({
      provider: 'GMAIL', externalAccountId: mailbox.mailboxId,
    } as never)
    const store = createPrismaInboundCorrespondenceStore()
    const message = {
    ...
    expect(vi.mocked(publishCrmOperationalSignal)).toHaveBeenCalledWith(expect.objectContaining({
      input: expect.objectContaining({
  ...
  it('labels only a scoped FAKE account message as synthetic in quarantine signals', async () => {
    vi.spyOn(db.prospectInboundQuarantine, 'create').mockResolvedValue({ id: 'fake-quarantine' } as never)
    const account = vi.spyOn(db.correspondenceProviderAccount, 'findUnique').mockResolvedValue({
      provider: 'FAKE', externalAccountId: 'SYN-FAKE-mailbox',
    } as never)
    const ref = {
      provider: 'FAKE' as const, providerAccountId: 'fake-account',
      mailboxId: 'SYN-FAKE-mailbox', externalId: 'fake-message',
    }
    const store = createPrismaInboundCorrespondenceStore()
    const input = {
      receiptId: null, reason: 'UNKNOWN_THREAD' as const,
      message: {
        message: ref, thread: { ...ref, externalId: 'fake-thread' },
        rfcMessageId: '<fake@example.invalid>', inReplyTo: null, references: [],
        from: [{ email: 'contact@example.invalid' }], to: [{ email: 'mailbox@example.invalid' }],
        cc: [], bcc: [], subject: 'Synthetic reply',
        internalDate: new Date('2026-09-22T00:00:00.000Z'), direction: 'INBOUND' as const,
        body: normalizeUntrustedCorrespondenceBody({ text: 'Synthetic body' }), attachments: [],
      },
      occurredAt: new Date('2026-09-22T00:00:01.000Z'),
    }
    ...
    account.mockResolvedValue({ provider: 'GMAIL', externalAccountId: 'different-mailbox' } as never)
    await store.quarantine(input)
    expect(vi.mocked(publishCrmOperationalSignal)).toHaveBeenLastCalledWith(expect.objectContaining({
      input: expect.objectContaining({
