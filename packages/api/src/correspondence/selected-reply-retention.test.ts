...
vi.mock('@pathfinder/db', () => ({
  ...
  writeAuditLogStrict: (
    input: unknown,
    tx: { auditLog: { create: (value: unknown) => unknown } },
  ...
  it('rejects stale content and a lost CAS without writing an audit', async () => {
    ...
    await expect(
      retainSelectedSourceOnlyReply({
        provider,
        mailbox,
        expected: { ...expected, rawBodySha256: '0'.repeat(64) },
        retentionDays: 2,
        actorId: 'admin-1',
      }),
  ...
  it('attributes the isolated FAKE proof to SYSTEM and rejects a forged fixture actor', async () => {
    ...
    await retainSelectedSourceOnlyReply({
      ...
      actor: {
        type: 'SYSTEM',
        role: 'PLATFORM_ADMIN',
        id: 'synthetic:crm-meaning:selected-reply-proof',
      },
    })
    expect(mocks.auditCreate.mock.calls[0]![0].data).toMatchObject({
      actorType: 'SYSTEM',
      actorId: 'synthetic:crm-meaning:selected-reply-proof',
