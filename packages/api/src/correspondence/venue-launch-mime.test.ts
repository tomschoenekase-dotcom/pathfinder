...
describe('venue launch MIME', () => {
  it('validates bytes, keeps boundary stable, and emits an exact base64 attachment', () => {
    ...
    const encodedText = mime.split('Content-Transfer-Encoding: base64\r\n\r\n')[1]!.split(`\r\n--${boundary}`)[0]!
  ...
  it('accepts only the exact recovered attachment', () => {
    const recovered = { filename: asset.filename, mimeType: asset.mimeType,
      sizeBytes: asset.sizeBytes, contentBase64Url: bytes.toString('base64url') }
    ...
    expect(matchesLaunchAttachments([asset], [{ ...recovered, contentBase64Url: Buffer.from('changed').toString('base64url') }])).toBe(false)
