...
function invalidSelection(): never {
  throw new CorrespondenceProviderError('INVALID_INPUT', 'Exact canonical reply selection is incomplete or mismatched')
}
...
export async function readExactSourceOnlyReplyContent(input: {
  ...
  if (!provider.capabilities.has('READ_MESSAGE')) {
    throw new CorrespondenceProviderError('NOT_CONFIGURED', 'Provider cannot read a selected reply message')
  }
  ...
  if (
    ...
    throw new CorrespondenceProviderError('INVALID_INPUT', 'Provider message no longer matches the canonical reply source')
  }
  if (
    ...
    throw new CorrespondenceProviderError('INVALID_INPUT', 'Selected reply body is missing or truncated')
  }
  ...
  if (
    ...
    throw new CorrespondenceProviderError('INVALID_INPUT', 'Selected reply text exceeds the bounded preparation limit')
  }
  return {
