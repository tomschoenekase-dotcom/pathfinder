...
export function createInboundCorrespondenceService(input: {
  ...
  async function processReceipt(
    ...
    try {
      if (!sameScope(mailbox, messageRef)) {
        ...
        return {
          state: completed.applied ? ('QUARANTINED' as const) : receiptOutcome(completed.receipt),
  ...
  async function quarantineUnavailableMessage(
    ...
    const received = await store.receiveReceipt({
      ...
      externalReceiptId: `history-message:${createHash('sha256')
        .update(
          JSON.stringify([
            mailbox.provider,
