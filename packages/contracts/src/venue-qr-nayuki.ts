// @ts-nocheck -- vendored third-party QR encoder; typed adapter is venue-qr-svg.ts
// Vendored from qrcode.react 4.2.0 src/third-party/qrcodegen/index.ts.
// Copyright (c) Project Nayuki. SPDX-License-Identifier: MIT. See venue-qr-nayuki.LICENSE.
  ...
  const _QrCode = class _QrCode {
    ...
    getPenaltyScore() {
      ...
      for (let y = 0; y < this.size; y++) {
        ...
        result +=
          this.finderPenaltyTerminateAndCount(runColor, runX, runHistory) * _QrCode.PENALTY_N3
      }
      for (let x = 0; x < this.size; x++) {
        let runColor = false
        let runY = 0
        ...
        result +=
          this.finderPenaltyTerminateAndCount(runColor, runY, runHistory) * _QrCode.PENALTY_N3
      }
...
export const NayukiQrCode = qrcodegen.QrCode
export const NayukiQrSegment = qrcodegen.QrSegment
