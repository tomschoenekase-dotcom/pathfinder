/**
 * QR Code generator library. Copyright (c) Project Nayuki. SPDX-License-Identifier: MIT.
 * Source: qrcode.react v4.2.0 src/third-party/qrcodegen/index.ts.
 * Upstream SHA-256: a34e609b123e592c453d82d23a97bc1a1773162919e8a5ebe61c0e2b5f332370.
 * Local adaptation preserves the encoder algorithm and exposes ordinary ES module classes
 * instead of TypeScript namespaces. Unlike the prior compiled copy, this source is typechecked.
 * Array reads enforce the encoder's populated-array invariants rather than bypassing strict types.
 * See venue-qr-nayuki.LICENSE.
 */
type bit = number
type byte = number
type int = number
...
export class Mode {
  ...
  public numCharCountBits(ver: int): int {
    return qrArrayAt(this.numBitsCharCount, Math.floor((ver + 7) / 17))
  }
}
export class QrCode {
  ...
  public static encodeSegments(
    ...
    bb.forEach(
      (b: bit, i: int) =>
        (dataCodewords[i >>> 3] = qrArrayAt(dataCodewords, i >>> 3) | (b << (7 - (i & 7)))),
    )
  ...
  public getModule(x: int, y: int): boolean {
    return (
      0 <= x && x < this.size && 0 <= y && y < this.size && qrArrayAt(qrArrayAt(this.modules, y), x)
    )
  }
    ...
    for (let i = 0; i < numAlign; i++) {
      for (let j = 0; j < numAlign; j++) {
        ...
        if (!((i == 0 && j == 0) || (i == 0 && j == numAlign - 1) || (i == numAlign - 1 && j == 0)))
          this.drawAlignmentPattern(qrArrayAt(alignPatPos, i), qrArrayAt(alignPatPos, j))
      }
    }
  ...
  private setFunctionModule(x: int, y: int, isDark: boolean): void {
    qrArrayAt(this.modules, y)[x] = isDark
    qrArrayAt(this.isFunction, y)[x] = true
  }
  ...
  private addEccAndInterleave(data: Readonly<Array<byte>>): Array<byte> {
    ...
    const numBlocks: int = qrArrayAt(
      qrArrayAt(QrCode.NUM_ERROR_CORRECTION_BLOCKS, ecl.ordinal),
      ver,
    )
    const blockEccLen: int = qrArrayAt(qrArrayAt(QrCode.ECC_CODEWORDS_PER_BLOCK, ecl.ordinal), ver)
    ...
    for (let i = 0; i < qrArrayAt(blocks, 0).length; i++) {
      blocks.forEach((block, j) => {
        // Skip the padding byte in short blocks
        if (i != shortBlockLen - blockEccLen || j >= numShortBlocks)
          result.push(qrArrayAt(block, i))
      })
    }
    ...
    for (let right = this.size - 1; right >= 1; right -= 2) {
      ...
      for (let vert = 0; vert < this.size; vert++) {
        ...
        for (let j = 0; j < 2; j++) {
          ...
          if (!qrArrayAt(qrArrayAt(this.isFunction, y), x) && i < data.length * 8) {
            qrArrayAt(this.modules, y)[x] = getBit(qrArrayAt(data, i >>> 3), 7 - (i & 7))
            i++
          }
  ...
  private applyMask(mask: int): void {
    ...
    for (let y = 0; y < this.size; y++) {
      for (let x = 0; x < this.size; x++) {
        ...
        if (!qrArrayAt(qrArrayAt(this.isFunction, y), x) && invert)
          qrArrayAt(this.modules, y)[x] = !qrArrayAt(qrArrayAt(this.modules, y), x)
      }
    }
  }
  ...
  private getPenaltyScore(): int {
    ...
    for (let y = 0; y < this.size; y++) {
      ...
      for (let x = 0; x < this.size; x++) {
        if (qrArrayAt(qrArrayAt(this.modules, y), x) == runColor) {
          runX++
          ...
          runColor = qrArrayAt(qrArrayAt(this.modules, y), x)
          runX = 1
        }
      }
    ...
    for (let y = 0; y < this.size - 1; y++) {
      for (let x = 0; x < this.size - 1; x++) {
        const color: boolean = qrArrayAt(qrArrayAt(this.modules, y), x)
        if (
          color == qrArrayAt(qrArrayAt(this.modules, y), x + 1) &&
          color == qrArrayAt(qrArrayAt(this.modules, y + 1), x) &&
          color == qrArrayAt(qrArrayAt(this.modules, y + 1), x + 1)
        )
  ...
  private static getNumDataCodewords(ver: int, ecl: Ecc): int {
    return (
      ...
      qrArrayAt(qrArrayAt(QrCode.ECC_CODEWORDS_PER_BLOCK, ecl.ordinal), ver) *
        qrArrayAt(qrArrayAt(QrCode.NUM_ERROR_CORRECTION_BLOCKS, ecl.ordinal), ver)
    )
  }
  ...
  private static reedSolomonComputeDivisor(degree: int): Array<byte> {
    ...
    for (let i = 0; i < degree; i++) {
      ...
      for (let j = 0; j < result.length; j++) {
        result[j] = QrCode.reedSolomonMultiply(qrArrayAt(result, j), root)
        if (j + 1 < result.length) result[j] = qrArrayAt(result, j) ^ qrArrayAt(result, j + 1)
      }
  ...
  private static reedSolomonComputeRemainder(
    ...
    for (const b of data) {
      ...
      result.push(0)
      divisor.forEach(
        (coef, i) => (result[i] = qrArrayAt(result, i) ^ QrCode.reedSolomonMultiply(coef, factor)),
      )
    }
    return result
  }
  ...
  private finderPenaltyCountPatterns(runHistory: Readonly<Array<int>>): int {
    const n: int = qrArrayAt(runHistory, 1)
    assert(n <= this.size * 3)
    const core: boolean =
      n > 0 &&
      qrArrayAt(runHistory, 2) == n &&
      qrArrayAt(runHistory, 3) == n * 3 &&
      qrArrayAt(runHistory, 4) == n &&
      qrArrayAt(runHistory, 5) == n
    return (
      (core && qrArrayAt(runHistory, 0) >= n * 4 && qrArrayAt(runHistory, 6) >= n ? 1 : 0) +
      (core && qrArrayAt(runHistory, 6) >= n * 4 && qrArrayAt(runHistory, 0) >= n ? 1 : 0)
    )
  }
  ...
  private finderPenaltyAddHistory(currentRunLength: int, runHistory: Array<int>): void {
    if (qrArrayAt(runHistory, 0) == 0) currentRunLength += this.size // Add light border to initial run
    runHistory.pop()
...
export { QrCode as NayukiQrCode, QrSegment as NayukiQrSegment }

/** Internal arrays are fully populated; fail explicitly if an invariant is ever violated. */
function qrArrayAt<T>(values: readonly T[], index: number): T {
  const value = values[index]
  if (value === undefined) throw new RangeError('QR encoder internal array index out of range')
  return value
}
