...
export function parseVenueLaunchAsset(value: unknown): VenueLaunchAsset {
  ...
  if (bytes.toString('base64') !== asset.contentBase64 || bytes.length !== asset.sizeBytes ||
...
export function launchAttachmentsSha256(value: unknown): string {
  return createHash('sha256').update(JSON.stringify(parseVenueLaunchAttachments(value))).digest('hex')
}
