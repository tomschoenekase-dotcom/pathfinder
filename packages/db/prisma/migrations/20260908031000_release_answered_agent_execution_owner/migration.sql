-- The final answered blocker queues a run and releases its interrupted execution owner.
-- Every live-owner replacement and unrelated ownership clear remains rejected.
...
BEGIN
  ...
  IF NEW."execution_bridge_session_id" IS DISTINCT FROM OLD."execution_bridge_session_id" AND (
    (OLD."execution_bridge_session_id" IS NULL AND NEW."execution_bridge_session_id" IS NOT NULL AND NEW."status" = 'RUNNING')
    OR (
      OLD."execution_bridge_session_id" IS NOT NULL AND NEW."execution_bridge_session_id" IS NOT NULL
    ...
    OR (
      OLD."execution_bridge_session_id" IS NOT NULL AND NEW."execution_bridge_session_id" IS NULL
      AND OLD."status" = 'AWAITING_INPUT' AND NEW."status" = 'QUEUED'
      AND NEW."attempt_number" = OLD."attempt_number"
      AND NEW."execution_worker_id" IS NULL
      AND NEW."execution_lease_token" IS NULL
      AND NEW."execution_lease_expires_at" IS NULL
      AND NEW."last_heartbeat_at" IS NULL
    )
  ) IS NOT TRUE THEN
    RAISE EXCEPTION 'agent run bridge ownership can change only at initial claim, fenced takeover, or answered-blocker release' USING ERRCODE = '55000';
  END IF;
  ...
  IF NEW."status" IS DISTINCT FROM OLD."status" AND NOT (
    ...
    RAISE EXCEPTION 'invalid agent run status transition from % to %', OLD."status", NEW."status" USING ERRCODE = '23514';
  END IF;
