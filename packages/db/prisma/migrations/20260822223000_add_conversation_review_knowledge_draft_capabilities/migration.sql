...
-- One unresolved correction path per insight prevents concurrent workers from creating divergent
-- pending truth. Rejected proposals may be replaced; published or still-reviewable proposals may not.
CREATE UNIQUE INDEX "knowledge_change_proposals_active_insight_key"
  ON "knowledge_change_proposals"("tenant_id", "venue_id", "conversation_insight_id")
  WHERE "conversation_insight_id" IS NOT NULL
    AND "status" IN ('DRAFT', 'PENDING_REVIEW', 'APPROVED', 'PUBLISHED');

-- Admit the two narrowly scoped capabilities used by the review-only visitor-answer correction
-- loop. Credential issuance and activation remain separately evidenced and fail closed.
...
BEGIN
  ...
  END IF;
  IF NEW."kind" = 'MCP' AND (NEW."capabilities" <@ ARRAY['accounts:read','agent-runs:execute','agent-runs:read','ai-usage:read','billing:propose','billing:read','clients:read','configuration:read','content:read','conversations:read','conversations:review','delegations:create','deployments:read','evaluations:read','evaluations:request','events:read','feature-flags:read','history:read','integrations:read','jobs:read','knowledge:draft','knowledge:read','meetings:process','meetings:read','outcomes:read','packages:draft','packages:read','questions:ask','questions:read','readiness:read','reports:read','resources:read','support:draft','support:read','updates:draft','updates:read','venues:read','workers:read']::TEXT[]) IS NOT TRUE THEN
