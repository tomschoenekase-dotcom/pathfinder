...
describe('native no-send revision and exact component storage', () => {
  ...
  it('persists only a recipe for a temporary Gmail reply, never its retained body', async () => {
    ...
    const component = {
      ...
      preparation: {
        ...
        request: {
          mode: 'reply',
          purpose: 'Answer the exact point',
          thread_state: { latest_message: { body: privateBody } },
        },
        writerContext: {
          WLT_packet_identity: 'wlt',
          approved_language_snapshot: {},
        ...
        answerText: 'Propose one room',
      },
    }
    await persistNativeSalesPreparation(
      { venueId: 'venue', expectedSnapshotHash: snapshotHash, component, actor },
      client as never,
    )
  ...
  it('retains exact reference text as guidance, not authenticated authorship or approved language', () => {
    ...
    const input = {
      label: 'Test only',
      sourceRef: 'synthetic:reference-v1',
      text,
      sha256: hash(text),
    }
    ...
    expect(() => bindNativeWritingReference({ ...input, text: text + 'changed' }, actor)).toThrow(
      /BYTES_MISMATCH/,
    )
    ...
    expect(() =>
      bindNativeWritingReference({ ...replacement, sha256: input.sha256 }, actor),
    ...
    expect(() =>
      bindNativeWritingReference({ ...input, sourceRef: 'file:missing', text: '' }, actor),
    ).toThrow(/BYTES_MISMATCH/)
  })
  it('does not rewrite manual provenance when a model returns identical message bytes', async () => {
    const h = harness()
    const manual = await saveNativeSalesDraft(
      h.input('NON-SALES check', 'Diagnostic body'),
      h.client as never,
    )
    ...
    const generated = await saveNativeSalesDraft(
      {
        ...
        writerProvenance: {
          ...
          generatedBy: { kind: 'model', identity: 'Synthetic test model' },
          annotations: [],
        },
      },
      h.client as never,
