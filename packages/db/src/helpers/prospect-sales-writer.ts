...
import { venueLaunchAssetDescriptor, type VenueLaunchAssetDescriptor } from '@pathfinder/contracts/venue-launch-asset'
import { requireCurrentProspectLaunchAttachments, type ProspectLaunchReadClient } from './prospect-launch-attachments'
...
import { assertIssuedNativeSalesWriterAgent,
  revalidateNativeSalesWriterAgent } from './prospect-native-writer-agent'
...
export async function readNativeWriterTask(
  ...
  const launchAttachments = await requireCurrentProspectLaunchAttachments(venueId, launchAttachmentsFromSnapshot(stored), client)
  requireCurrentTemporaryReply(stored, current)
  const preparation = obj(stored.preparation),
    context = obj(preparation.writerContext)
  const transient = obj(stored.retentionRecipe).schema === TEMPORARY_REPLY_RECIPE
    ? obj(current.preparation) : preparation
  ...
  const task: NativeWriterTask = {
    ...
    binding,
    ...(launchAttachments.length ? { launchAttachments: launchAttachments.map(venueLaunchAssetDescriptor) } : {}),
...
export async function importNativeWriterResult(
  ...
  try {
      ...
      async (tx) => {
        if (input.actor.type === 'AGENT')
          await revalidateNativeSalesWriterAgent(input.actor,
            r.binding.venueId, r.binding.organizationId, tx)
    ...
    if (['P2034', 'P2002'].includes(String(obj(error).code))) {
      if (input.actor.type === 'AGENT')
        await revalidateNativeSalesWriterAgent(input.actor,
          r.binding.venueId, r.binding.organizationId, client)
