...
import { z } from 'zod'
import { PROSPECT_GEOGRAPHY_HASH, ProspectPhysicalCountyEvidence } from './prospect-territory-registry'
export const AssignProspectGeographyInput=z.object({
  idempotencyKey:z.string().trim().min(1).max(191), venueId:z.string().min(1).max(191),
  expectedVenueUpdatedAt:z.string().datetime(), expectedRevision:z.number().int().nonnegative(),
  expectedRegistryHash:z.literal(PROSPECT_GEOGRAPHY_HASH), evidence:ProspectPhysicalCountyEvidence,
}).strict()
export const InvalidateProspectGeographyInput=z.object({
  idempotencyKey:z.string().trim().min(1).max(191),venueId:z.string().min(1).max(191),
  expectedVenueUpdatedAt:z.string().datetime(),expectedRevision:z.number().int().positive(),
  expectedRegistryHash:z.literal(PROSPECT_GEOGRAPHY_HASH),reason:z.string().trim().min(12).max(2000),
}).strict()
...
export const ProposeProspectGeographyInput = AssignProspectGeographyInput.extend({
  researchClaim:z.object({jobId:z.string().min(1).max(191),claimToken:z.string().uuid()}).strict().optional(),
}).strict()
export const GeographyProposalListInput = z.object({
  venueId: z.string().min(1).max(191),
  status: z.enum(['OPEN', 'RESOLVED', 'ALL']).default('OPEN'),
  page: z.number().int().min(1).max(100000).default(1),
  limit: z.number().int().min(1).max(50).default(20),
}).strict()
export const ResolveProspectGeographyProposalInput = z.object({
