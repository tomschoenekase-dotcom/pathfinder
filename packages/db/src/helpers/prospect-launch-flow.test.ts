...
vi.mock('./prospect-sales-snapshot', async (original) => ({
  ...
  readNativeSalesSnapshot: mock.snapshot,
}))
vi.mock('./venue-launch-source', async (original) => ({
  ...(await original<typeof import('./venue-launch-source')>()),
  resolveVenueLaunchSource: mock.source,
}))
  ...
  it('persists exact selected bytes and exports only a source-bound descriptor', async () => {
    ...
    const tx = {
      prospectLocationConversion: {
        findMany: vi.fn().mockResolvedValue([{ tenantId: 'tenant', venueId: 'venue' }]),
      },
      ...
      prospectActivity: {
        ...
        findUnique: vi.fn(
          async ({ where }: { where: { id: string } }) => activities.get(where.id) ?? null,
        ),
      },
    }
    const client = {
    ...
    const component = {
      ...
      nativeSnapshotHash: 'b'.repeat(64),
      SEND_AUTHORIZED: false,
      senderAvailable: false,
      blocker: null,
      gate: { can_prepare: true },
      componentCodeHashes: { owner: 'hash' },
      crosswalk: {
        nativeVenueId: 'prospect',
      ...
      preparation: {
        ...
        request: { mode: 'cold' },
        writerMarkdown: 'Write about the venue.',
    ...
    const writerComponent = {
      ...component,
      draftCheck: {
        composerDraftSha256: createHash('sha256')
    ...
    const receipt = await importNativeWriterResult(
      { result, component: writerComponent, actor, assess: vi.fn() },
      client as never,
    )
    ...
    expect(draft.groundingSnapshot).toMatchObject({
      launchAttachments: [asset],
    ...
    const legacyContentHash = salesHash({
      series: draft.preparationKey,
      subject,
      body,
      preparationId: grounding.preparationId,
      route: crosswalk.routing,
      nativeSnapshotHash: grounding.nativeSnapshotHash,
    })
    expect(legacyContentHash).not.toBe(draft.contentHash)
    expect(() =>
      nativeMeaningBinding({ ...draft, contentHash: legacyContentHash } as never, persisted),
    ).toThrow(/content\/recipient\/preparation binding/u)
    const changedIdentity = {
      ...asset,
      release: { ...asset.release, revisionSha256: 'e'.repeat(64) },
    }
    expect(() =>
    ...
    expect(() =>
      nativeMeaningBinding(
        {
          ...draft,
          groundingSnapshot: { ...grounding, launchAttachments: [changedIdentity] },
        } as never,
        persisted,
      ),
    ).toThrow(/Launch attachment selection changed after review/u)
    const reviewInput = {
      ...
      contentHash: draft.contentHash as string,
      expectedSnapshotHash: 'b'.repeat(64),
      actor,
    }
    ...
    await expect(
      reviewNativeSalesDraft({ ...reviewInput, contentHash: 'f'.repeat(64) }, client as never),
    ).rejects.toThrow(/exact native no-send revision/u)
    expect(
      prospectOperationalContentHash('to', 'Subject', 'Body', '', { launchAttachments: [asset] }),
    ).not.toBe(prospectOperationalContentHash('to', 'Subject', 'Body', '', {}))
    await expect(
      requireCurrentProspectLaunchAttachments('wrong-prospect', [asset], {
        ...client,
    ...
    await expect(
      requireCurrentProspectLaunchAttachments(
        ...
        [
          {
            ...asset,
            contentBase64: alternateBytes.toString('base64'),
            sizeBytes: alternateBytes.length,
