...
export async function readNativeSalesSnapshot(venueId: string, client: ReadClient = db) {
  ...
  const threads = boundedThreads.map((thread) => ({
    ...
    messages: [...thread.messages].reverse().map((message) => {
      ...
      const expired = message.bodyRetentionState === 'TEMPORARY' &&
  ...
  const threadCoverage = threads.map((thread) => {
    ...
    if (thread._count.messages > thread.messages.length)
      issues.push(`Only the latest ${thread.messages.length} of ${thread._count.messages} messages are in this bounded thread review`)
    for (const message of thread.messages) {
      if (!message.textBody)
        issues.push(`Retained body unavailable for message ${message.id}; source owner must review availability`)
    }
