  ...
  const outcome = await client.$transaction(async (tx) => {
    ...
    return {
      ...
      send: {
        ...
        credentialReferenceId: providerAccount.credentialReferenceId ?? 'SYNTHETIC-NO-CREDENTIAL',
...
export async function claimProspectAmbiguousRecoveryAction(
  ...
  return client.$transaction(async (tx) => {
    const claimed = await tx.prospectSendOutbox.updateMany({
      ...
      data: {
        status: 'CLAIMED', claimOwner: input.workerId,
        claimExpiresAt: new Date(now.getTime() + 120_000), terminalAt: null,
        attemptCount: { increment: 1 },
      },
    })
      ...
      if (operation?.claimOwner === input.workerId) {
        await tx.prospectSendOutbox.updateMany({
          ...
          data: {
            ...
            lastErrorRetryable: false,
            lastErrorMessage: 'Frozen provider/account/envelope identity is malformed; reconciliation is held',
          },
        })
      }
      return null
    ...
    return {
      ...
      credentialReferenceId: operation.providerAccount.credentialReferenceId ?? 'SYNTHETIC-NO-CREDENTIAL',
