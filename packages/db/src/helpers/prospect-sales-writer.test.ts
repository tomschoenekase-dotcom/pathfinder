...
const result: NativeWriterResult = {
  ...
  subject: 'Synthetic receipt check',
    ...
    const client = {
      prospectActivity: {
        findUnique: vi.fn().mockResolvedValue({
          ...receipt,
          evidence: { ...receipt.evidence, resultHash: '0'.repeat(64) },
        }),
      },
    }
    await expect(readNativeWriterImportReceipt(result, client as never)).rejects.toThrow(
      'WRITER_RECEIPT_CONFLICT',
    )
  })
})
