...
describe('native-origin rehearsal is not a permission bypass', () => {
  it('rejects invalid, expired and changed-business preparations without private voice files', async () => {
    ...
    const preparation = {
      ...
      componentCodeHashes: { gate: 'hash' },
      preparation: {
        fileSha256s: { 'writer-context.md': 'hash' },
    ...
    await expect(
      verifyStoredNativeOrigin(native, { ...preparation, schema: 'forged' } as never),
    ).rejects.toThrow('NATIVE_PREPARATION_INVALID')
    await expect(
      verifyStoredNativeOrigin(native, {
        ...preparation,
        preparation: {
          ...preparation.preparation,
          businessFreshnessReviewDueAt: '2000-01-01T00:00:00Z',
        },
      } as never),
    ).rejects.toThrow('NATIVE_PREPARATION_INVALID')
    await expect(
  ...
  it('does not confuse another Gmail account with the confirmed company mailbox', () => {
    ...
    expect(
      isIntendedNativeGmailAccount({
        provider: 'GMAIL',
        mailboxAddress: 'tomschoenekase@torchiko.com.attacker.invalid',
      }),
    ).toBe(false)
  })
  it('defaults off', () => {
