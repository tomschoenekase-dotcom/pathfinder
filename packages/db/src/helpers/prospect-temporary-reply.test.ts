...
const component = () => ({
  ...
  preparation: {
    ...
    request: {
      mode: 'reply',
      purpose: 'Respond to the selected point',
      thread_state: { latest_message: { body: privateBody } },
    },
    writerContext: {
      WLT_packet_identity: 'wlt-hash',
      approved_language_snapshot: { selected_entries: [] },
      research_snapshot: 'research-hash',
      writer_context_sha256: 'context-hash',
...
describe('temporary Gmail reply preparation storage', () => {
  ...
  it('holds changed, expired, or unavailable canonical source before task export', () => {
    ...
    const compact = compactTemporaryReplyPreparation(live)
    expect(() =>
      requireCurrentTemporaryReply(compact, {
        ...live,
        correspondence: {
          ...live.correspondence,
    ...
    expect(() =>
      requireCurrentTemporaryReply(compact, {
        ...live,
        blocker: 'GMAIL_BODY_UNAVAILABLE_OR_EXPIRED',
        preparation: null,
      }),
    ).toThrow('TEMPORARY_REPLY_SOURCE_STALE_OR_EXPIRED')
  })
  ...
  it('keeps permanent meaning findings while removing inbound source quotations', () => {
    const compact = compactTemporaryMeaningCheck({
      ...
      unresolvedHolds: [privateBody],
      operationalHolds: [privateBody],
