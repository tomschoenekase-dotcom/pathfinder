...
import { requireCurrentProspectLaunchAttachments, requireSameLaunchAttachments, prospectOperationalContentHash } from './prospect-launch-attachments'
...
export function isIntendedNativeGmailAccount(account: { provider: string; mailboxAddress: string }) {
  return account.provider === 'GMAIL' && account.mailboxAddress.toLowerCase() === NATIVE_COMPANY_MAILBOX
}
...
export async function readEligibleNativeOrigin(
  ...
  await requireCurrentProspectLaunchAttachments(draft.venueId, launchAttachmentsFromSnapshot(grounding), tx)
  ...
  if (projection.thread_id) {
    ...
    requireMatch(salesHash(aliases) ===
      salesHash(obj(obj(component.crosswalk).correspondenceMessageIds)),
    'CORRESPONDENCE_IDENTITY_CROSSWALK_CHANGED')
    const projectedId = projection.reply_to_message_id
    const nativeId = synthetic && typeof projectedId === 'string'
      ? (aliases[projectedId] ?? projectedId) : projectedId
...
export async function validateOperationalNativeOrigin(
  ...
  requireMatch(
    prospectOperationalContentHash(draft.toEmail!, draft.subject, draft.textBody, '', draft.groundingSnapshot) === draft.contentHash,
