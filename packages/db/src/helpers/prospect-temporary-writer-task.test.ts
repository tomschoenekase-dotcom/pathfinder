...
const live = {
  ...
  correspondence: {
    snapshot: { provider: { name: 'gmail' }, messages: [{ content: { text: privateBody } }] },
    projection: {
      thread_id: 'thread-1',
  ...
  preparation: {
    ...
    request: {
      mode: 'reply',
      purpose: 'Respond to selected point',
...
describe('temporary source writer export', () => {
  it('reconstructs body-bearing task transiently and holds after source expiry', async () => {
    ...
    source.read.mockResolvedValue({
      snapshotHash: live.nativeSnapshotHash,
    ...
    source.read.mockResolvedValue({
      snapshotHash: 'changed-after-expiry',
      suppression: { blocked: false },
      venue: { id: 'venue-1' },
      organization: { id: 'org-1' },
    })
    await expect(
      readNativeWriterTask(
        'venue-1',
        { ...live, blocker: 'GMAIL_BODY_UNAVAILABLE_OR_EXPIRED', preparation: null },
        client as never,
      ),
    ).rejects.toThrow('TEMPORARY_REPLY_SOURCE_STALE_OR_EXPIRED')
  })
})
