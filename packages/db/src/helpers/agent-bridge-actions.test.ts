...
describe('agent bridge actions', () => {
  ...
  it('narrows an explicit foreground claim without falling back to another queued run', async () => {
    mocks.sessionFind.mockResolvedValue({ id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      provider: 'CODEX_SUBSCRIPTION', supportedModels: ['subscription-default'] })
    ...
    expect(await claimAgentBridgeTask({ sessionId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      venueId: 'venue-1', runId: 'selected-run', credential: credential as never }))
      .toEqual({ task: null })
    expect(mocks.runFindMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({ id: 'selected-run', tenantId: 'tenant-1', venueId: 'venue-1' }),
    }))
    expect(mocks.claim).not.toHaveBeenCalled()
    await expect(claimAgentBridgeTask({ sessionId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      venueId: 'venue-1', runId: ' ', credential: credential as never })).rejects.toThrow()
  })
