...
const frozenSchema = z
  .object({
    ...
    prospectScope: scopeSchema,
    promptIdentity: z.string().trim().min(1).max(191),
  })
  .passthrough()
const requiredCapabilities = ['prospects.native-writer', 'prospects.correspondence.read'] as const
...
export function assertIssuedNativeSalesWriterAgent(actor: NativeSalesWriterAgentActor) {
  if (
    !actor ||
    actor.type !== 'AGENT' ||
    actor.role !== 'NATIVE_SALES_WRITER' ||
    !issued.has(actor)
  )
    deny()
}
...
export async function revalidateNativeSalesWriterAgent(
  ...
  if (binding.venueId !== venueId || binding.organizationId !== organizationId) deny()
...
export async function revalidateNativeSalesWriterAgentBound(
  ...
  assertIssuedNativeSalesWriterAgent(actor)
  await revalidateNativeSalesWriterAgent(actor, venueId, issued.get(actor)!.organizationId, client)
}
...
export async function issueNativeSalesWriterAgentActor(input: {
  ...
  await revalidateNativeSalesWriterAgent(verified, input.venueId, input.organizationId)
  return verified
}
