...
async function validateCurrentAttachments(
  venueId: string | null,
  snapshot: unknown,
  tx: SalesTransaction,
  proofs?: readonly VerifiedCurrentProspectPrintAsset[],
) {
  ...
  if (!venueId)
    throw new ProspectOutreachError(
      'CONFLICT',
      'A current prospect venue is required for launch attachments',
    )
  return requireCurrentProspectLaunchAttachments(venueId, attachments, {
    client: tx,
    ...(proofs
      ? {
          verifiedCurrentPrintAssets: proofs
            .filter((proof) => proof.prospectVenueId === venueId)
            .map((proof) => proof.asset),
        }
      : {}),
  })
}
...
export async function saveProspectOutreachDraftActionInTransaction(
  ...
  await validateCurrentAttachments(
    member.venueId,
              ...
              headerSnapshot: {
                ...(launchAttachmentsFromSnapshot(draft.groundingSnapshot).length
                  ? {
                      ...
                      launchAttachmentsSha256: launchAttachmentsSha256(
