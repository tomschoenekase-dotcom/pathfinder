...
const base: ChicagoRankingInput = {
  venueId: 'venue-a',
  asOf: '2026-09-22',
  territory: 'Chicago Metro',
}
const observed = (value: number): RankingObservation => ({
  value,
  reason: 'A published visitor guide supports this assessment.',
  sourceUrls: [source],
  researchedAt: '2026-09-01',
  basis: 'verified',
})
const fullFit = (value: number) =>
  Object.fromEntries(CHICAGO_FIT_KEYS.map((key) => [key, observed(value)]))
const contact = {
  kind: 'email' as const,
  verified: true,
  suppressed: false,
...
describe('Chicago whole-market ranking v1.2', () => {
  it('keeps strong-fit venues without an email above weak-fit email-ready venues', () => {
    ...
    expect([weak, strong].sort(compareChicagoVenueRankings).map((entry) => entry.venueId)).toEqual([
      'venue-a',
      'venue-b',
    ])
    expect(rankChicagoVenue({ ...base, fit: fullFit(95), contacts: [contact] }).productFit).toEqual(
      strong.productFit,
    )
  })
  ...
  it.each([
    ...
    'museum?',
    'unknown museum',
    'event venue museum ' + 'unverified detail '.repeat(20),
  ...
  it('recognizes bounded complete category tokens and preserves explicit evidence independently', () => {
    ...
    expect(
      rankChicagoVenue({ ...base, venueType: 'museum'.padStart(120, ' ') }).productFit.value,
    ).toBe(80)
    expect(
      rankChicagoVenue({ ...base, venueType: 'museum'.padStart(121, ' ') }).productFit.value,
    ).toBeNull()
    const supported = rankChicagoVenue({
      ...base,
      venueType: 'museum '.repeat(100),
      fit: fullFit(90),
    })
  ...
  it('compares a retained 1.0.0 category snapshot with current rules without rewriting history', () => {
    ...
    const before = {
      ...rankChicagoVenue({ ...base, venueType: 'museum' }),
      version: 'chicago-venue-ranking/1.0.0',
    }
  ...
  it('averages only known component values and preserves actual zero evidence', () => {
    const value = rankChicagoVenue({
      ...base,
      fit: { knowledgeRichness: observed(0), interpretiveValue: observed(80) },
    })
  ...
  it('requires supported, dated, complete, current fit observations for evidence-backed state', () => {
    ...
    expect(
      rankChicagoVenue({ ...base, fit: fullFit(80), conflicts: ['Location identity disagrees'] })
        .state,
    ).toBe('provisional-heuristic')
    expect(
      rankChicagoVenue({
        ...base,
        fit: { ...fullFit(80), guideUseCases: { ...observed(80), researchedAt: null } },
      }).state,
    ).toBe('provisional-heuristic')
    expect(
      rankChicagoVenue({
        ...base,
        fit: { ...fullFit(80), guideUseCases: { ...observed(80), researchedAt: '2025-01-01' } },
      }).state,
    ).toBe('provisional-heuristic')
    expect(
      rankChicagoVenue({ ...base, fit: { knowledgeRichness: { ...observed(80), sourceUrls: [] } } })
        .productFit.value,
    ).toBeNull()
  })
  ...
  it('does not count unverified or suppressed email claims as usable routes', () => {
    expect(
      rankChicagoVenue({ ...base, contacts: [{ ...contact, verified: false }] }).contactability
        .value,
    ).toBeNull()
  ...
  it('uses the best route without gaming the score by adding copies or weak routes', () => {
    ...
    expect(rankChicagoVenue({ ...base, contacts: [contact, contact] }).contactability.value).toBe(
      one,
    )
    expect(
      rankChicagoVenue({
        ...base,
        contacts: [contact, { ...contact, kind: 'website', roleRelevant: false }],
  ...
  it('calculates evidence and freshness independently and treats invalid dates as unknown', () => {
    ...
    for (const researchedAt of ['2027-01-01', '2026-02-31', 'yesterday', null]) {
      expect(
        rankChicagoVenue({ ...base, sources: [{ url: source, firstParty: null, researchedAt }] })
  ...
  it('marks source conflicts as research priority without erasing underlying component scores', () => {
    const value = rankChicagoVenue({
      ...base,
      fit: fullFit(90),
      sources: [{ url: source, firstParty: true, conflicted: true }],
    })
    expect(value.researchPriority.value).toBe(100)
    expect(value.state).toBe('provisional-heuristic')
  ...
  it('counts each canonical source page once so repeated fresh evidence cannot hide a stale page', () => {
    const old = {
      url: 'https://venue.example.org/history',
      firstParty: false,
    ...
    const raw = [
      old,
      recent,
      { ...recent, url: 'https://VENUE.example.org:443/visit#top' },
      recent,
      recent,
    ]
  ...
  it('keeps tied ownership unknown and unresolved source conflicts visible despite repetition', () => {
    ...
    expect(result.evidenceFreshness.value).toBe(100)
    const conflicted = rankChicagoVenue({
      ...base,
      sources: [{ ...dated, researchedAt: '2024-01-01', conflicted: true }, dated],
    })
    expect(conflicted.evidenceQuality.value).toBe(0)
    expect(conflicted.researchPriority.value).toBe(100)
    const undated = rankChicagoVenue({
      ...base,
      sources: [
        { url: source, firstParty: true },
        { url: source, firstParty: true, researchedAt: 'invalid' },
      ],
    })
  ...
  it('repeated contact routes cannot improve contactability or reduce research priority', () => {
    ...
    const repeated = rankChicagoVenue({
      ...base,
      contacts: Array.from({ length: 50 }, () => ({ ...contact })),
    })
  ...
  it('compares retained 1.1.0 repeated-source scores against 1.2.0 without relabeling history', () => {
    ...
    const prior = {
      ...result,
      version: 'chicago-venue-ranking/1.1.0',
      evidenceFreshness: { ...result.evidenceFreshness, value: 77.5 },
    }
  ...
  it('supports explicit unranked/excluded states, with exclusions taking priority over overrides', () => {
    ...
    const intentionally = rankChicagoVenue({
      ...base,
      intentionallyUnrankedReason: 'Awaiting identity review',
      fit: fullFit(100),
    })
  ...
  it('retains actor/date/rationale and computed evidence when a human overrides a dimension', () => {
    const override = {
      ...
      rationale: 'Current founder relationship assessment.',
      dimension: 'productFit' as const,
      value: 75,
    }
    ...
    expect(() => rankChicagoVenue({ ...base, override: { ...override, actor: '' } })).toThrow(
      /Override/,
    )
    expect(() =>
      rankChicagoVenue({ ...base, override: { ...override, at: '2027-01-01' } }),
    ).toThrow(/Override/)
  })
  ...
  it('requires explicit date and stable identity, and rejects invalid scoring evidence', () => {
    ...
    for (const value of [NaN, Infinity, -1, 101])
      expect(
        rankChicagoVenue({ ...base, fit: { knowledgeRichness: observed(value) } }).productFit.value,
      ).toBeNull()
    expect(
      rankChicagoVenue({
        ...base,
        fit: { knowledgeRichness: { ...observed(80), sourceUrls: ['javascript:alert(1)'] } },
      }).productFit.value,
    ).toBeNull()
  })
  ...
  it('is reproducible, stable under sorting ties, and compares retained version snapshots', () => {
    ...
    expect(() => compareChicagoRankingVersions(before, { ...after, venueId: 'another' })).toThrow(
      /same venue/,
    )
  })
})
