...
export async function resolveVenueLaunchSource(input: {
  ...
  if (
    ...
    destinations[0]?.id !== input.venueId ||
    destinations[0]?.tenantId !== input.tenantId
  )
    return null
  ...
  if (
    native &&
    snapshot.state!.places.length === 0 &&
    snapshot.state!.knowledgeEntries.length === 0 &&
    snapshot.state!.generalizedModules.length === 0
  )
    return null
  ...
  if (
    (!native && places.length === 0 && knowledge.length === 0) ||
    places.length > 1000 ||
    knowledge.length > 1000
  )
    return null
  ...
  return {
    ...
    release: {
      ...
      id: native
        ? snapshot.releaseId!
        : `legacy:${createHash('sha256').update(`${input.tenantId}:${input.venueId}`).digest('hex')}`,
      revisionSha256,
    },
  }
}
