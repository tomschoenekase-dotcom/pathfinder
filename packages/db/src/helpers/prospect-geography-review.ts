...
import {
  ...
  type GeographyActor,
  type ProspectGeographyTransaction,
} from './prospect-territory-actions'

const KIND = 'PHYSICAL_GEOGRAPHY_PROPOSAL'
type Tx = ProspectGeographyTransaction &
  Pick<typeof db, 'prospectIntelligenceReview' | 'prospectResearchJob' | 'prospectResearchAttempt'>
...
function requireScope(actor: GeographyActor, territoryId: string | null) {
  if (
    ...
    throw new ProspectGeographyError(
      'FORBIDDEN',
      'The current native territory is outside the actor grant',
    )
}
...
async function model(tx: Tx) {
  const value = await tx.prospectGeographyModel.findUnique({
    where: { version: PROSPECT_GEOGRAPHY_VERSION },
  })
  if (!value || value.registryHash !== PROSPECT_GEOGRAPHY_HASH)
    throw new ProspectGeographyError(
      'CONFLICT',
      'The approved registry is not installed with this release hash',
    )
}
...
async function prior(tx: Tx, id: string, hash: string) {
  ...
  if (receipt.inputHash !== hash)
    throw new ProspectGeographyError('CONFLICT', 'Retry key belongs to a different payload')
...
async function receipt(
  ...
  await tx.prospectIntelligenceReceipt.create({
    data: {
      ...
      operation,
      inputHash: geographyHash({ operation, input }),
      venueId,
...
export async function proposeProspectGeography(raw: unknown, actor: GeographyActor) {
  ...
  const input = ProposeProspectGeographyInput.parse(raw),
    operation = 'geography-propose'
  return transaction(async (tx) => {
    await model(tx)
    const venue = await venueFor(tx, input.venueId, actor)
    const recovered = await prior(
      tx,
      receiptIdentity(operation, input.idempotencyKey, actor),
      geographyHash({ operation, input }),
    )
    if (recovered) return recovered
    const job = await tx.prospectResearchJob.findUnique({
      where: { organizationId: venue.organizationId },
    })
    if (job?.status === 'CLAIMED') {
      const binding = input.researchClaim,
        now = new Date()
      if (
        !binding ||
        binding.jobId !== job.id ||
        binding.claimToken !== job.claimToken ||
        ...
        throw new ProspectGeographyError(
          'CONFLICT',
          'Existing organization research is claimed; use its current exact actor/run/token or wait for a fresh claim',
        )
      ...
      if (
        ...
        (typeof object(attempt.usage).venueId === 'string' &&
          object(attempt.usage).venueId !== venue.id)
      )
        throw new ProspectGeographyError(
          'CONFLICT',
          'Research attempt does not cover this current native location',
        )
      const fence = await tx.prospectResearchJob.updateMany({
        where: {
          id: job.id,
          status: 'CLAIMED',
      ...
      if (fence.count !== 1)
        throw new ProspectGeographyError(
          'CONFLICT',
          'Research claim changed before the evidence proposal could be retained',
        )
    } else if (input.researchClaim)
      throw new ProspectGeographyError(
        'CONFLICT',
        'The supplied research claim is no longer current',
      )
    if (
      venue.archivedAt ||
      ...
      throw new ProspectGeographyError(
        'CONFLICT',
        'Venue or geography changed; refresh before proposing evidence',
      )
    ...
    if (
      ...
      (existing.kind !== KIND ||
        ...
        existing.sourceHash !== geographyHash(proposal))
    )
      throw new ProspectGeographyError(
        'CONFLICT',
        'Review identity collision; original evidence preserved',
      )
    ...
    if (!existing) {
      await tx.prospectSourceEvidence.create({
        data: {
          ...
          venueId: venue.id,
          sourceType: 'PROPOSED_PHYSICAL_COUNTY',
      ...
      await tx.prospectIntelligenceReview.create({
        data: {
          ...
          venueId: venue.id,
          kind: KIND,
          sourceHash: geographyHash(proposal),
          reason:
            'Review the physical visitor address and authoritative county evidence before changing canonical research ownership.',
          original: json({
            schema: 'torchiko.geography-proposal/v1',
            proposal,
            evidenceId,
            territoryCode: plan.territoryCode,
            submittedBy: actor.id,
            submittedRunId: actor.runId,
            researchClaim: researchClaim ?? null,
    ...
    return receipt(
      ...
      operation,
      venue.id,
      { geographyRevision: input.expectedRevision },
      {
        venueId: venue.id,
        reviewId,
        evidenceId,
        reviewStatus: existing?.status ?? 'OPEN',
...
export async function listProspectGeographyProposals(raw: unknown, actor: GeographyActor) {
  ...
  const where = {
    ...
    ...(actor.scope.mode === 'ALL'
      ? {}
      : { venue: { territoryId: { in: [...actor.scope.territoryIds] } } }),
  }
  ...
  const items = rows.map((row) => {
    ...
    const parsed = ProposeProspectGeographyInput.safeParse({
      ...object(original.proposal),
    ...
    return {
      ...
      reason: row.reason,
      proposal,
      evidenceId: typeof original.evidenceId === 'string' ? original.evidenceId : null,
      decision: row.decision,
      createdAt: row.createdAt,
      stale:
        !proposal ||
        venue.updatedAt.toISOString() !== proposal.expectedVenueUpdatedAt ||
...
export async function resolveProspectGeographyProposal(raw: unknown, actor: GeographyActor) {
  ...
  if (actor.type !== 'HUMAN')
    throw new ProspectGeographyError(
      'FORBIDDEN',
      'Only an authenticated human operator may decide a geography proposal',
    )
  const input = ResolveProspectGeographyProposalInput.parse(raw),
    operation = 'geography-review'
  return transaction(async (tx) => {
    await model(tx)
    const review = await tx.prospectIntelligenceReview.findUnique({ where: { id: input.reviewId } })
    if (!review || review.kind !== KIND || !review.venueId)
      throw new ProspectGeographyError('NOT_FOUND', 'Geography proposal not found')
    ...
    if (review.status !== 'OPEN' || review.revision !== input.expectedReviewRevision)
      throw new ProspectGeographyError(
        'CONFLICT',
        'Review already changed; reload its current decision',
      )
    const original = object(review.original)
    const proposal = ProposeProspectGeographyInput.parse({
      ...object(original.proposal),
      idempotencyKey: `review-assignment-${geographyHash({ reviewId: review.id, revision: review.revision }).slice(0, 40)}`,
    })
    if (proposal.venueId !== venue.id)
      throw new ProspectGeographyError('CONFLICT', 'Proposal venue identity mismatch')
    const assignment =
      input.decision === 'ACCEPT'
        ? await assignProspectGeographyInTransaction(proposal, actor, tx)
        : null
    const changed = await tx.prospectIntelligenceReview.updateMany({
      ...
      data: {
        ...
        decision: json({
          ...
          assignmentReceiptId: assignment?.receiptId ?? null,
        }),
      },
    })
    if (changed.count !== 1)
      throw new ProspectGeographyError(
        'CONFLICT',
        'Concurrent review decision; no partial assignment committed',
      )
    return receipt(
      tx,
      actor,
      ...
      { reviewRevision: review.revision },
      {
        ...
        decision: input.decision,
        canonicalFieldsApplied: assignment !== null,
        assignment,
        outreachAuthorized: false,
      },
    )
