...
describe('opaque native writer agent authority', () => {
  ...
  it('binds exact live run, lease, bridge and territory to one native venue', async () => {
    ...
    await expect(revalidateNativeSalesWriterAgentBound(actor, 'other-venue')).rejects.toThrow(
      'Live leased native-writer grant',
    )
  })
  it('rejects fabricated actors and revoked live or frozen capability', async () => {
    await expect(
      revalidateNativeSalesWriterAgent(
        { type: 'AGENT', role: 'NATIVE_SALES_WRITER', id: 'agent' },
        'native-venue',
        'org-1',
      ),
    ...
    mocks.run.mockResolvedValue({
      ...live(),
      agentIdentity: { id: 'agent', accessCapabilities: [] },
    })
    await expect(
      issueNativeSalesWriterAgentActor({
        invocation,
    ...
    mocks.run.mockResolvedValue({
      ...live(),
      agentIdentity: { id: 'agent', accessCapabilities: ['prospects.native-writer'] },
    })
    await expect(
      issueNativeSalesWriterAgentActor({
        invocation,
  ...
  it('rejects moved territory or expired lease at a later mutation check', async () => {
    ...
    await expect(revalidateNativeSalesWriterAgentBound(actor, 'native-venue')).rejects.toThrow(
      'Live leased native-writer grant',
    )
  ...
  it('does not return a conflict-recovered immutable receipt after live authority is lost', async () => {
    ...
    const binding = {
      ...
      componentCodeHash: 'c'.repeat(64),
      fileSetHash: 'd'.repeat(64),
      selectionId: null,
      routeHash: 'e'.repeat(64),
      routeKind: 'email',
      recipient: 'fixture@example.invalid',
      formUrl: null,
      threadHash: 'f'.repeat(64),
      libraryHash: '1'.repeat(64),
      wltHash: '2'.repeat(64),
    ...
    mocks.run.mockResolvedValueOnce(null)
    await expect(
      importNativeWriterResult({ result, component: {}, actor, assess: vi.fn() }, client as never),
    ).rejects.toThrow('Live leased native-writer grant')
