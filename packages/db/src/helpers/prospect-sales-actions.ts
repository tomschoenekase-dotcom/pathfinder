import { launchAttachmentsFromSnapshot, parseVenueLaunchAttachments, launchAttachmentsSha256 } from '@pathfinder/contracts/venue-launch-asset-node'
...
import { assertIssuedNativeSalesWriterAgent, revalidateNativeSalesWriterAgent,
  type NativeSalesWriterAgentActor } from './prospect-native-writer-agent'
...
export function bindNativeWritingReference(value: NativeWritingReference, actor: SalesActor) {
  ...
  if (!value || Object.keys(value).sort().join(',') !== 'label,sha256,sourceRef,text' ||
      typeof value.label !== 'string' || !value.label.trim() || value.label.length > 200 ||
      typeof value.sourceRef !== 'string' || !value.sourceRef.trim() || value.sourceRef.length > 1000 ||
      typeof value.text !== 'string' || !value.text.trim() || value.text.length > 20_000 ||
      value.text.includes('\0') || Buffer.byteLength(value.text, 'utf8') > 32_000 ||
      createHash('sha256').update(value.text, 'utf8').digest('hex') !== value.sha256)
    throw new ProspectSalesError('INVALID_INPUT', 'WRITING_REFERENCE_BYTES_MISMATCH: supply bounded exact text and its SHA-256')
  return {
    ...value,
...
export function compactTemporaryReplyPreparation(component: ComponentOutput): ComponentOutput {
  ...
  if (!preparation.fileSha256s || !context.WLT_packet_identity ||
      !projection.thread_id || !projection.reply_to_message_id)
...
export function requireCurrentTemporaryReply(
  stored: ComponentOutput, current: ComponentOutput,
): void {
  ...
  if (current.nativeSnapshotHash !== stored.nativeSnapshotHash || current.blocker ||
...
export function requireSalesOperator(actor: SalesActor): asserts actor is Exclude<SalesActor, NativeSalesWriterAgentActor> {
  ...
  return client.$transaction(
    async (tx) => {
      ...
      if (input.actor.type === 'AGENT')
        await revalidateNativeSalesWriterAgent(input.actor,
          native.venue.id, native.organization.id, tx)
...
export async function saveNativeSalesDraftInTransaction(
  ...
  if (input.actor.type === 'AGENT') {
    if (!input.writerProvenance) throw new ProspectSalesError('FORBIDDEN',
  ...
  const launchAttachments = await requireCurrentProspectLaunchAttachments(input.venueId,
    launchAttachmentsFromSnapshot(previousComponent), tx)
  ...
  if (latest?.contentHash === contentHash) {
    ...
    if (!input.writerProvenance || (previousWriter &&
        salesHash({ ...object(previousWriter), submittedBy: undefined }) ===
        salesHash(input.writerProvenance))) return latest
  }
...
export async function reviewNativeSalesDraft(
  ...
  return client.$transaction(
    async (tx) => {
      ...
      await requireCurrentProspectLaunchAttachments(input.venueId, launchAttachmentsFromSnapshot(draft.groundingSnapshot), tx)
