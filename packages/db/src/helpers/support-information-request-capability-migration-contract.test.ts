...
describe('support information-request capability migration contract', () => {
  const migration = readFileSync(
    resolve(
      process.cwd(),
      'prisma/migrations/20260824210000_add_support_information_request_capability/migration.sql',
    ),
    'utf8',
  )

  it('admits only the approval-gated capability through the existing fail-closed trigger', () => {
    expect(migration).toContain("'support:request-information'")
