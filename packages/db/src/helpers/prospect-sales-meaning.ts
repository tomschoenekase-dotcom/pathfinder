  ...
  return {
    ...
    findings: (check.findings as unknown[]).map((value) => ({
      code: /^[A-Z][A-Z0-9_]{0,80}$/u.test(String(obj(value).code))
        ? String(obj(value).code)
        : 'SOURCE_FINDING',
      detail: 'Review the current canonical message source while retained',
    })),
    unresolvedHolds: (check.unresolvedHolds as unknown[]).map(
      (value) => `SOURCE_HOLD_${salesHash(value).slice(0, 12)}`,
    ),
    operationalHolds: Array.isArray(check.operationalHolds)
      ? check.operationalHolds.map(
          (value: unknown) => `SOURCE_HOLD_${salesHash(value).slice(0, 12)}`,
        )
      : [],
    claimEvidence: [],
  }
