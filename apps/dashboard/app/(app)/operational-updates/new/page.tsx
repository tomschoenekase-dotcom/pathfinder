      ...
      <div className="mx-auto max-w-4xl">
        {venues.length === 0 ? (
          <section className="rounded-[2rem] border border-dashed border-pf-light bg-pf-white px-6 py-12 text-center shadow-sm">
            <h1 className="text-2xl font-semibold text-pf-deep">Create a venue first</h1>
            <p className="mt-3 text-sm leading-6 text-pf-deep/60">
              Operational alerts are scoped to a venue, so you need at least one venue before
              publishing updates.
            </p>
            <Link
              href="/venues/new"
              className="mt-6 inline-flex min-h-11 items-center rounded-full bg-pf-primary px-5 text-sm font-medium text-white transition hover:bg-pf-accent"
            >
              Add a venue
            </Link>
          </section>
          ...
          <OperationalUpdateForm
            venues={venues.map((venue: { id: string; name: string }) => ({
              id: venue.id,
              name: venue.name,
            }))}
          />
