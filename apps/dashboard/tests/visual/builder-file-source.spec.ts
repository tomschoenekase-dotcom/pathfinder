...
test('verified file source remains truthful, readable, and non-authoritative', async ({
  ...
  await expect(page.getByText(/bounded local text extractor/).first()).toBeVisible()
  await expect(page.getByText('c'.repeat(64)).first()).toBeVisible()
  const extract = page.getByRole('button', { name: 'Extract document for review' })
  ...
  await expect(page.getByText('Document text · review required')).toBeVisible()
