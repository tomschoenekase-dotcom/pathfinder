...
async function saveViewportEvidence(page: Page, testInfo: TestInfo) {
  const screenshot = await page.screenshot({
    animations: 'disabled',
    caret: 'hide',
    path: testInfo.outputPath('source-review-answer.png'),
  })
  expect(screenshot.byteLength).toBeGreaterThan(10_000)
}

test('V1 receipt separates extracted files from held and answer-waiting source review', async ({
  page,
}, testInfo) => {
  ...
  await expect(processing.getByText('Torchiko review needs setup', { exact: true })).toBeVisible()
  await processing.scrollIntoViewIfNeeded()
  await page.screenshot({
    path: testInfo.outputPath('source-review-held.png'),
    animations: 'disabled',
  })
  ...
  await expect(processing.getByText('Torchiko review needs setup', { exact: true })).toHaveCount(0)
  await processing.scrollIntoViewIfNeeded()
