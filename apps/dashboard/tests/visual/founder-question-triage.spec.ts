...
test('founder question queue supports fast evidence-backed triage without widening authority', async ({
  ...
  const handbookCover = page.getByText('Handbook cover', { exact: true })
  await expect(handbookCover).toBeHidden()
  await foundational.click()
  await expect(handbookCover).toBeVisible()
  await expect(foundationalCard.getByText(/video timestamp/i)).toBeVisible()
  await expect(foundationalCard.getByText('at 1:34', { exact: true })).toBeVisible()
  await expect(foundationalCard.getByText('58%')).toBeVisible()
  await expect(foundationalCard.getByText('Candidate entities')).toBeVisible()
  await expect(foundationalCard.getByText('What each answer changes')).toBeVisible()
