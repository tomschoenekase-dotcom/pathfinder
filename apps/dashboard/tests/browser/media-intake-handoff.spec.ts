...
test('binds saved items, retains an uncertain request, and links the resulting proposal', async ({
  ...
  await page
    ...
    .selectOption('source:s1')
  await page.getByRole('combobox', { name: 'Arrival assistance' }).selectOption('source:s2')
  await page.getByText('Optional temporal claim review').click()
  await page.getByRole('textbox', { name: 'Temporal claims JSON' }).fill(
    JSON.stringify([
      {
  ...
  await page.getByRole('button', { name: 'Preview temporal claims' }).click()
  await expect(
    page.getByText('1 eligible for the static candidate · 1 retained on local hold'),
  ).toBeVisible()
  await expect(page.getByText(/Held · date-bound \+ no current supporting evidence/)).toBeVisible()
  await page
  ...
  await expect(page.getByRole('textbox', { name: 'Temporal claims JSON' })).toBeDisabled()
  expect((await new AxeBuilder({ page }).include('main').analyze()).violations).toEqual([])
