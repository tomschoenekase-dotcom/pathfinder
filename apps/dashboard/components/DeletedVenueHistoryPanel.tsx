'use client'

import { useEffect, useRef, useState } from 'react'
import { useRouter } from 'next/navigation'

import { currentDeletedVersions } from '../lib/content-history-diff'
import { useTRPCClient } from '../lib/trpc'

type VenueVersion = {
  id: string
  sequence: bigint
  entityType: string
  entityId: string
  beforeState: unknown
  afterState: unknown
  createdAt: Date
}

function venueName(version: VenueVersion): string {
  if (
    typeof version.beforeState === 'object' &&
    version.beforeState !== null &&
    !Array.isArray(version.beforeState)
  ) {
    const name = (version.beforeState as Record<string, unknown>).name
    if (typeof name === 'string') return name
  }
  return version.entityId
}

function errorMessage(error: unknown): string {
  return error instanceof Error && error.message ? error.message : 'History request failed.'
}

function errorCode(error: unknown): string | null {
  if (!error || typeof error !== 'object' || !('data' in error)) return null
  const data = error.data
  if (!data || typeof data !== 'object' || !('code' in data)) return null
  return typeof data.code === 'string' ? data.code : null
}

type PendingAction = { kind: 'load' | 'restore'; versionId?: string }
type Feedback = { kind: 'error' | 'success'; text: string }

function appendVersions(current: VenueVersion[], incoming: VenueVersion[]) {
  const currentIds = new Set(current.map((version) => version.id))
  return [...current, ...incoming.filter((version) => !currentIds.has(version.id))]
}

export function DeletedVenueHistoryPanel() {
  const router = useRouter()
  const client = useTRPCClient()
  const [versions, setVersions] = useState<VenueVersion[]>([])
  const [isOpen, setIsOpen] = useState(false)
  const [hasMore, setHasMore] = useState(false)
  const [pending, setPending] = useState<PendingAction | null>(null)
  const [feedback, setFeedback] = useState<Feedback | null>(null)
  const [requiresReload, setRequiresReload] = useState(false)
  const mounted = useRef(false)
  const actionSequence = useRef(0)
  const activeAction = useRef<number | null>(null)
  const deleted = currentDeletedVersions(versions)

  useEffect(() => {
    mounted.current = true
    return () => {
      mounted.current = false
      activeAction.current = null
    }
  }, [])

  function startAction(next: PendingAction): number | null {
    if (activeAction.current !== null) return null
    const token = ++actionSequence.current
    activeAction.current = token
    setPending(next)
    return token
  }

  function isCurrentAction(token: number) {
    return mounted.current && activeAction.current === token
  }

  function finishAction(token: number) {
    if (!isCurrentAction(token)) return
    activeAction.current = null
    setPending(null)
  }

  async function load(beforeSequence?: bigint) {
    const token = startAction({ kind: 'load' })
    if (token === null) return
    setFeedback(null)
    try {
      const result = await client.contentHistory.listDeletedVenues.query({
        limit: 100,
        ...(beforeSequence !== undefined ? { beforeSequence } : {}),
      })
      if (!isCurrentAction(token)) return
      setVersions((current) =>
        beforeSequence === undefined ? result : appendVersions(current, result),
      )
      setHasMore(result.length === 100)
      setRequiresReload(false)
    } catch (loadError) {
      if (!isCurrentAction(token)) return
      setFeedback({ kind: 'error', text: errorMessage(loadError) })
    } finally {
      finishAction(token)
    }
  }

  async function restore(version: VenueVersion) {
    const token = startAction({ kind: 'restore', versionId: version.id })
    if (token === null) return
    if (!window.confirm(`Restore deleted venue ${venueName(version)}?`)) {
      finishAction(token)
      return
    }
    setFeedback(null)
    try {
      await client.contentHistory.revert.mutate({
        versionId: version.id,
        expectedCurrentVersionId: version.id,
        snapshotSide: 'BEFORE',
      })
      if (!isCurrentAction(token)) return
      setFeedback({ kind: 'success', text: 'Deleted chatbot restored.' })
      router.push(`/venues/${version.entityId}`)
      router.refresh()
    } catch (restoreError) {
      if (!isCurrentAction(token)) return
      setRequiresReload(true)
      setFeedback({
        kind: 'error',
        text:
          errorCode(restoreError) === 'CONFLICT'
            ? 'Deleted chatbot history changed after this recovery view was loaded. Reloading authoritative state; review it before retrying.'
            : 'The restore outcome could not be confirmed. Reloading authoritative state; review deleted chatbots before retrying.',
      })
      router.refresh()
    } finally {
      finishAction(token)
    }
  }

  const isLoading = pending?.kind === 'load'
  const restoringId = pending?.kind === 'restore' ? pending.versionId : null

  return (
    <section
      className="rounded-3xl border border-pf-light bg-pf-white p-6 shadow-sm"
      aria-busy={pending !== null}
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-pf-deep">Deleted chatbots</h2>
          <p className="mt-1 text-sm text-pf-deep/60">
            Owners can restore a previously deleted venue and its saved configuration.
          </p>
        </div>
        <button
          type="button"
          disabled={pending !== null}
          aria-expanded={isOpen}
          onClick={() => {
            if (activeAction.current !== null) return
            const next = !isOpen
            setIsOpen(next)
            if (next) void load()
          }}
          className="inline-flex min-h-10 items-center rounded-full border border-pf-light px-4 text-sm font-medium text-pf-primary"
        >
          {isOpen ? 'Hide deleted chatbots' : 'Review deleted chatbots'}
        </button>
      </div>

      {isOpen ? (
        <div className="mt-5 space-y-3">
          {feedback ? (
            <p
              className={`rounded-xl p-3 text-sm ${
                feedback.kind === 'error'
                  ? 'bg-rose-50 text-rose-700'
                  : 'bg-emerald-50 text-emerald-700'
              }`}
              role={feedback.kind === 'error' ? 'alert' : 'status'}
            >
              {feedback.text}
            </p>
          ) : null}
          {requiresReload && pending === null ? (
            <button
              type="button"
              onClick={() => void load()}
              className="inline-flex min-h-9 items-center rounded-full border border-pf-light px-4 text-sm font-medium text-pf-primary"
            >
              Reload deleted chatbots
            </button>
          ) : null}
          {isLoading && versions.length > 0 ? (
            <p className="text-sm text-pf-deep/50" role="status">
              Refreshing deleted chatbots…
            </p>
          ) : null}
          {isLoading && versions.length === 0 ? (
            <p className="text-sm text-pf-deep/50" role="status">
              Loading…
            </p>
          ) : feedback?.kind === 'error' && versions.length === 0 ? null : deleted.length === 0 ? (
            <p className="text-sm text-pf-deep/50">No deleted chatbots in loaded history.</p>
          ) : (
            deleted.map((version) => (
              <div
                key={version.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-pf-light p-4"
              >
                <div>
                  <p className="font-medium text-pf-deep">{venueName(version)}</p>
                  <p className="mt-1 text-xs text-pf-deep/50">
                    Deleted {version.createdAt.toLocaleString()}
                  </p>
                </div>
                <button
                  type="button"
                  disabled={pending !== null || requiresReload}
                  onClick={() => void restore(version)}
                  className="inline-flex min-h-9 items-center rounded-full bg-pf-primary px-4 text-sm font-medium text-white disabled:opacity-50"
                >
                  {restoringId === version.id ? 'Restoring…' : 'Restore'}
                </button>
              </div>
            ))
          )}
          {hasMore && versions.length > 0 ? (
            <button
              type="button"
              disabled={pending !== null || requiresReload}
              onClick={() => void load(versions.at(-1)!.sequence)}
              className="inline-flex min-h-9 items-center rounded-full border border-pf-light px-4 text-sm font-medium text-pf-primary disabled:opacity-50"
            >
              {isLoading ? 'Loading…' : 'Load older history'}
            </button>
          ) : null}
        </div>
      ) : null}
    </section>
  )
}
