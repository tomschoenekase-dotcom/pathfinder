...
function candidate(venueId = 'v') {
  ...
  return {
    ...
    binding: { venueId, organizationId: 'o', preparationId: 'p', nativeSnapshotHash: h,
      preparationHash: h, componentCodeHash: h, fileSetHash: h, selectionId: null,
      routeHash: h, routeKind: 'email', recipient: 'qa@example.invalid', formUrl: null,
      threadHash: h, libraryHash: h, wltHash: h, expectedDraftId: null,
      expectedVenueDraftId: null, expectedMeaningReviewId: null, expectedReadReviewId: null },
    generatedBy: { kind: 'model', identity: 'Synthetic test' }, subject: 'Hello', body: 'Hi, 🌿',
    annotations: ['subject', 'body'].map((section, index) => ({ annotation_id: section,
      section, start: 0, end: 5, quote: index ? 'Hi, 🌿' : 'Hello', category: 'NONFACTUAL',
      claim_ids: [], reason: 'Synthetic greeting', answers: [] })),
...
describe('bounded file-only writer exchange UI', () => {
  ...
  it('clears the import action only after an explicit receipt confirmation', async () => {
    ...
    await waitFor(() => expect((screen.getByRole('button', { name: 'Import exact AI candidate' }) as HTMLButtonElement).disabled).toBe(false))
  ...
  it('discards a slow file read after navigating to another prospect', async () => {
    ...
    await waitFor(() => expect((screen.getByLabelText('AI result JSON file') as HTMLInputElement).disabled).toBe(false))
    expect(screen.queryByRole('button', { name: 'Import exact AI candidate' })).toBeNull()
    expect(action).not.toHaveBeenCalled()
  })
  it('retains the exact candidate after a lost response and permits an identical retry', async () => {
    const action = vi.fn().mockRejectedValueOnce(new Error('Response lost')).mockResolvedValue(undefined)
    render(<ProspectWriterRoundtrip view={view} enabled exportTask={vi.fn()} onAction={action} />)
    fireEvent.change(screen.getByLabelText('AI result JSON file'), {
      target: { files: [{ size: 1000, text: async () => JSON.stringify(candidate()) }] },
    })
  ...
  it('labels model attribution and makes an older replay receipt visible', () => {
    render(<ProspectWriterRoundtrip view={{ ...view,
      draft: { id: 'new-draft', version: 2, subject: 'New', body: 'Review only',
        contentHash: 'b'.repeat(64), state: 'DRAFT_REVIEW', preparationId: 'prep',
        warnings: [], previousDraftId: 'old-draft',
        writerAttribution: { generatedByKind: 'model', generatedBy: 'Tom',
          submittedBy: 'synthetic:operator', taskId: 'task', resultHash: 'c'.repeat(64) } },
      writerImportReceipt: { id: 'old-receipt', draftId: 'old-draft',
        meaningReviewId: null, replayed: true },
    }} enabled exportTask={vi.fn()} onAction={vi.fn()} />)
