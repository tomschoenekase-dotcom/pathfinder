  ...
  return (
    <div className="space-y-6" aria-label="Operational attention queues">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        >
          <h2 id="agent-runs-heading" className="font-semibold text-slate-950">
            Recent agent runs
          </h2>
          <p className="mt-1 text-xs text-slate-500">
            Lifecycle metadata only; artifacts, inputs, and outputs are omitted.
          </p>
          {data.agents.items.length === 0 ? (
            <div className="mt-4">
              <Empty>No agent runs are recorded.</Empty>
            </div>
          ) : (
            <ul className="mt-4 space-y-3">
              {data.agents.items.map((run) => (
                <li key={run.id} className="rounded-xl border border-slate-200 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                  ...
                  <p className="mt-2 text-xs text-slate-500">{date(run.createdAt)}</p>
                  {run.venueId ? (
                    <Link
                      className="mt-2 inline-block text-sm font-semibold text-sky-700"
          ...
          <More param="agentsCursor" cursor={data.agents.nextCursor} label="Older agent runs" />
        </section>
      </div>
    </div>
  )
