...
3. For code, use [repository onboarding](docs/repository-onboarding.md) and the generated [command index](docs/repository-command-index.md). Before dependencies are installed, find test files with `rg --files | rg '<topic>.*test'`; after the lockfile install, `node scripts/torchiko.mjs tests find <topic> --json` offers structured discovery. Read the affected source and focused tests. Preserve unrelated dirty work.
