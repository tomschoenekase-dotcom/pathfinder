  ...
  return {
    ...
    admission: {
      ...
      requiredActions: [
        ...
        'For a preserved-data migration cutover, pause autodeploy on staging-web, staging-dashboard, and staging-workers without deploying, verify the isolated settings diff and no pending replacement attempts, and freeze the connected owner branch for sequential release.',
        ...
        'Push the prepared exact owner staging revision only after required release variables are staged with --skip-deploys and autodeploy is paused for a preserved-data cutover; require owner CI success before the single web migration attempt. During that preserved-data cutover, do not release dashboard or workers merely because CI passed.',
        'Before the preserved-data migration attempt, drain every known old application writer and external ingress, retain the final release-bound backup and verified restore proof, confirm recovery authority and the effective provider predeploy timeout, and keep dashboard/workers stopped.',
        'Only when the exact ledger has pending migrations and all preservation/recovery gates pass, set PATHFINDER_ALLOW_STAGING_MIGRATIONS=1 and PATHFINDER_STAGING_MIGRATION_ONLY_HOLD=1 on Railway web with --skip-deploys; never arm this attempt during an automatic three-service rollout.',
        ...
        'For the pending preserve-existing cutover only, manually deploy only web at the exact frozen owner SHA through the existing GitHub source path, or the separately reviewed local-upload fallback. The checked-in staging migration predeploy must verify preserved staging data before service startup; hold=1 deliberately exits 2 with migration-verified-application-held, leaving this migration-only deployment FAILED and the application stopped. Retain that intentional failure separately from a migration error; never automatically retry or release after an unresolved cancellation.',
        'For the pending preserve-existing cutover only, after the operator accepts the complete exact ledger and integrity/preservation readback with no unresolved stop alarm, set PATHFINDER_ALLOW_STAGING_MIGRATIONS=0 and PATHFINDER_STAGING_MIGRATION_ONLY_HOLD=0 with --skip-deploys. Manually deploy web again at the same owner SHA for read-only code-only admission; never reopen with migration opt-in still armed.',
        'For the pending preserve-existing cutover only, only after same-SHA code-only web health passes, manually release dashboard and dormant workers at that exact frozen owner SHA. Require three-service topology and runtime admission before restoring reviewed autodeploy/CI settings without deployment and resuming authorized writers.',
        'For an already-complete exact schema code-only candidate, keep PATHFINDER_ALLOW_STAGING_MIGRATIONS=0 and PATHFINDER_STAGING_MIGRATION_ONLY_HOLD=0 or unset; use the normal CI-gated source release with read-only integrity admission. No intentional held FAILED deployment or second web deployment is required for this branch; retain exact release identity, feature defaults, topology and runtime gates.',
