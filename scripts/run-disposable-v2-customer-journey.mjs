...
try {
  process.exitCode = await runDisposableServiceShakedown({ configuration: {
    ...
    proofScope: ['real-postgresql-draft-cas', 'atomic-stale-submit-rollback', 'submission-replay', 'draft-scope-isolation', 'actual-production-retrieval', 'long-tail-fact', 'spanish-holdout', 'private-disabled-cross-tenant-exclusion', 'provider-dark'],
    integration: {
      packageDirectory: 'packages/api',
      testFile: 'src/lib/v2-customer-journey-disposable.integration.test.ts',
      expectedPassed: 1,
      environment: { RUN_V2_CUSTOMER_JOURNEY_DB_INTEGRATION: '1', OUTBOUND_PROVIDER_WORKERS_ENABLED: 'false', WORKER_SCHEDULERS_ENABLED: 'false' },
    },
  } })
} catch (error) {
