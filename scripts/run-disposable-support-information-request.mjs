...
try {
  ...
  if (error instanceof DisposableIntakeVerificationRefusal) {
    console.error(`Disposable support information-request shakedown refused: ${error.message}`)
    process.exitCode = 2
  } else if (error instanceof DisposableIntakeVerificationExecutionError) {
    console.error(`Disposable support information-request shakedown failed: ${error.message}`)
    process.exitCode = 1
  } else if (error instanceof AggregateError) {
    console.error('Disposable support information-request shakedown and cleanup both failed.')
    ...
    console.error(
      error instanceof Error
        ? `Disposable support information-request shakedown failed: ${error.message}`
        : 'Disposable support information-request shakedown failed unexpectedly.',
    )
    process.exitCode = 1
  }
}
