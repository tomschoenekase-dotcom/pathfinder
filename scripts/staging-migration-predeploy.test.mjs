...
test('248 approval preserves exact target, one-run opt-in and release-bound backup gates', () => {
  ...
  const env = {
    ...
    PATHFINDER_STAGING_BACKUP_LEDGER_COUNT: '236',
  ...
  const state = 'agent-question-operations-predecessor'
  const admission = admitPendingStagingMigrations(env, state)
  assert.equal(admission.backupEvidence.ledgerCount, 236)
  assert.doesNotThrow(() => assertBackupEvidenceMatchesLedger(admission, Array(236)))
