...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 423)
  ...
  assert.equal(report.operations.counts.byKind.query, 180)
  assert.equal(report.operations.counts.byKind.mutation, 243)
