...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 426)
  ...
  assert.equal(report.operations.counts.byKind.query, 181)
  assert.equal(report.operations.counts.byKind.mutation, 245)
