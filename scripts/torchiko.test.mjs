...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 441)
  ...
  assert.equal(report.operations.counts.byKind.query, 186)
  assert.equal(report.operations.counts.byKind.mutation, 255)
