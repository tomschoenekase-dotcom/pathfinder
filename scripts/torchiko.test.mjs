...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 524)
  ...
  assert.equal(report.operations.counts.byKind.query, 226)
  assert.equal(report.operations.counts.byKind.mutation, 298)
