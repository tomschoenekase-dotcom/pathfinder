...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 414)
  ...
  assert.equal(report.operations.counts.byKind.query, 176)
  assert.equal(report.operations.counts.byKind.mutation, 238)
