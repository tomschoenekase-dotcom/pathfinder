...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 432)
  ...
  assert.equal(report.operations.counts.byKind.query, 183)
  assert.equal(report.operations.counts.byKind.mutation, 249)
