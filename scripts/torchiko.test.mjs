...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 446)
  ...
  assert.equal(report.operations.counts.byKind.query, 188)
  assert.equal(report.operations.counts.byKind.mutation, 258)
