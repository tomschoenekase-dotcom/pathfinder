...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 510)
  ...
  assert.equal(report.operations.counts.byKind.query, 217)
  assert.equal(report.operations.counts.byKind.mutation, 293)
