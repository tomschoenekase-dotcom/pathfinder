...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 520)
  ...
  assert.equal(report.operations.counts.byKind.query, 224)
  assert.equal(report.operations.counts.byKind.mutation, 296)
