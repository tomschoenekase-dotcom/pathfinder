...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 421)
  ...
  assert.equal(report.operations.counts.byKind.query, 179)
  assert.equal(report.operations.counts.byKind.mutation, 242)
