...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 429)
  ...
  assert.equal(report.operations.counts.byKind.query, 182)
  assert.equal(report.operations.counts.byKind.mutation, 247)
