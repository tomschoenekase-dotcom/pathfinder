...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 417)
  ...
  assert.equal(report.operations.counts.byKind.query, 178)
  assert.equal(report.operations.counts.byKind.mutation, 239)
