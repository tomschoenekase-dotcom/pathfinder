...
test('every mounted router has exactly one explicit agent/developer coverage decision', async () => {
  ...
  assert.equal(report.operations.total, 538)
  ...
  assert.equal(report.operations.counts.byKind.query, 233)
  assert.equal(report.operations.counts.byKind.mutation, 305)
