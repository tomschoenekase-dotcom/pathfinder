...
try {
  ...
  if (error instanceof DisposableIntakeVerificationRefusal) {
    console.error(`Disposable support triage proposal shakedown refused: ${error.message}`)
    process.exitCode = 2
  } else if (error instanceof DisposableIntakeVerificationExecutionError) {
    console.error(`Disposable support triage proposal shakedown failed: ${error.message}`)
    process.exitCode = 1
  } else if (error instanceof AggregateError) {
    console.error('Disposable support triage proposal shakedown and cleanup did not both succeed.')
    ...
    console.error(
      error instanceof Error
        ? `Disposable support triage proposal shakedown failed: ${error.message}`
        : 'Disposable support triage proposal shakedown failed unexpectedly.',
    )
    process.exitCode = 1
  }
}
