...
Every database-backed provider-disabled isolated runtime emits the same bounded operational heartbeat and dependency observation with its exact scheduler state. It probes only the storage/scanner dependencies configured for that mode and never widens provider execution authority. This keeps deliberately dark workers attributable without misrepresenting them as fully provider-ready.
