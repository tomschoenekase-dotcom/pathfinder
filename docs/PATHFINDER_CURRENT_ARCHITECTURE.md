...
- **Fragile / flagged for verification:** the media-ingestion lab's image/synthesis model constant
  defaults to the literal string `'gpt-5.6-luna'` (`apps/workers/src/processors/media-ingestion.ts:94,
255-256, 274-276`) — not a publicly documented OpenAI model ID as of this analysis; if not
  overridden by `MEDIA_ANALYSIS_MODEL`/`MEDIA_SYNTHESIS_MODEL` env vars, calls using it would fail.
  Confirmed present in code and in `.env.example`'s placeholder value; **not confirmed** whether it
  resolves to a real, working model in production — flag for the team.
...
PathFinder import JSON (places, knowledge entries, unresolved questions, coverage). **No Zod
validation anywhere in this pipeline** — parsed JSON is only TypeScript-cast, not runtime-checked.
Per-asset failures are recorded and skipped (fail-open); job-level failures mark the whole project
FAILED (BullMQ retries ×3). Model constants default to `MEDIA_ANALYSIS_MODEL ?? 'gpt-5.6-luna'` for
image/synthesis calls and `MEDIA_TRANSCRIPTION_MODEL ?? 'gpt-4o-mini-transcribe'` for audio — the
`'gpt-5.6-luna'` default is **flagged for verification** (see §1). `estimatedCostCents`/
...
| Media analysis/synthesis                  | OpenAI (`gpt-5.6-luna` default) | background, long-running | n/a       | No (fields exist, unpopulated) |
...
| Media ingestion lab                      | Built, admin-only, experimental                                                                                          | `admin/media-ingestion.ts`, `apps/workers/src/processors/media-ingestion.ts`        | High                           | No Zod validation on model output; unverified model ID default; no cost tracking despite UI fields for it | Verify `gpt-5.6-luna` resolves correctly in prod; add output validation                        |
...
5. **Per-client usage and cost tracking** — capture `response.usage` on every AI call (via the same
   wrapper), persist to a new lightweight `AiUsageEvent` (tenant/venue/feature/model/tokens/cost,
   append-only like `AnalyticsEvent`), and expose a per-tenant cost rollup analogous to the existing
   `DailyRollup` pattern. This is the highest-leverage single addition for the "scaling" goal — it
   currently blocks any usage-based pricing decision.
...
- Unverified media-ingestion model ID default (`'gpt-5.6-luna'`) — confirm with the team whether
  this resolves to a real, working OpenAI model in production before relying on the lab for
  onboarding new venues at scale.
...
- Actual production domain names, DNS, and Railway environment/project layout beyond what's in
  `railway*.json`.
- Whether `'gpt-5.6-luna'` is a real, currently-working OpenAI model ID, or a placeholder/typo that
  happens to be overridden by an env var in production.
