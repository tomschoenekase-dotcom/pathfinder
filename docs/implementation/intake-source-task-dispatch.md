...
## Connected upload-to-worker-to-guide proof

`8c49cad5` integrates the registered HTTP source worker into the existing browser upload/scanner/V1/package/QR fixture. Native `c1b4a29dd599` on `9cdf4814` passed all four cases, 242 migrations / 260 tables and 113 current matching source hashes. Two fresh worker processes read the exact extracted receipt, ask and resume from a bounded human answer, retain an amendment, and the explicit human source review records its digest before the existing OWNER approval/application and capacity-137 public retrieval/QR decode. Five services stopped; phone and desktop handoff screenshots inspected. See `docs/evidence/source-worker-connected-2026-09-10.json`.
...
## Support addition through publication

`45dddf1e` fixes two missing explicit tenant predicates in support proposal replay and request-version evidence reads. Native `6106365a32fe` on `057757c8` proves a support-linked new fact remains conflicted/unpublishable until human review, becomes an ADDITION draft, remains absent from guest retrieval until explicit publication, and replays with exactly one module/revision/publication while unrelated knowledge stays unchanged. Setup bypass ends before the production actions; prior support-native tests wrapped the entire action in bypass and missed this admission defect.

242 migrations / 260 tables, one integrated case and 22 measured source hashes passed; PostgreSQL stopped. DB/API types, changed-source lint and the support proposal migration contract pass. Exact retained failures, hashes and proof limits are in `docs/evidence/support-addition-native-2026-09-10.json`. This closes only the scoped ADDITION proof, not P08-03's full six-class matrix or provider/device/customer acceptance. Next inspect supersession/conflict/duplicate durable outcomes against existing correction and temporal proofs; extend the same canonical services only where evidence is missing. Broad handoff refresh remains deferred.
...
## Answered semantic conflict resolution

`b12e8783` adds the explicit human resolution backend and append-only migration243; `c5b40e66` uses built-in PostgreSQL hashing and `b3eca968` pins the exact replacement target/text/creator. Native `2a3918f077fd` passes both replace-pending-review and keep-current outcomes, concurrency, exact answer binding and append-only SQL enforcement (243 migrations /261 tables /29 stable source hashes). Current-candidate addition/supersession and conflict/duplicate regression runs also pass; all owned databases stopped. 46 API tests,332 tenant-middleware tests, API/DB types and changed-source lint pass.

See `docs/implementation/semantic-conflict-resolution.md` and `docs/evidence/semantic-conflict-resolution-native-2026-09-10.json`. Next implement the operator resolution form in the existing SemanticUpdatePreview surface with actual answer text, explicit keep/replace, editable replacement, required note, frozen exact retry identity and navigation to separate replacement review. Client/admin UI and rendered proof are not claimed by this backend slice. Broad handoff refresh remains deferred; original requirements and external reservations remain intact.
