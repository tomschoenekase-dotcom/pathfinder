...
Remaining completion lanes after version 4 are terminal no-op outcome presentation, same-request historical receipt supersession, and broader exact-candidate release/physical field QA. Temporal outcome evidence is implemented below. An old receipt whose revision is no longer current deliberately blocks completion until a separately designed supersession chain proves the intended outcome. Do not silently discard receipts, infer completion from proposal APPROVED, or publish automatically to satisfy this gate. This slice does not complete all six semantic outcome classes or the original campaign acceptance suite.
...
Native proof `6f8c1bcc29b6` passes temporal lifecycle and locked expiry plus the existing completion replay case on candidate `2fbe88bb` (45 sources). It observes the exact reader backend waiting on the operational advisory lock, crosses expiry using PostgreSQL's clock, releases the writer, and verifies rejection. Addition/supersession plus completion replay also pass in `d7e11ee89f1f` (42 sources, candidate `14879699`); resolved adoption passes `d9cdc5e11a2a` (47 sources, candidate `2fbe88bb`). All measured production sources still match the retained manifests and all PostgreSQL instances stopped. These are four distinct cases across five executions, not the full campaign QA suite. See `docs/evidence/support-temporal-completion-native-2026-09-10.json`.

### Next implementation: same-request receipt supersession

The remaining historical-receipt blocker needs explicit chain evidence, never silently dropping older receipts. Group by exact support request + module + kind. Select universal receipt classification/relation/expectedBaseRevisionId/expectedBaseVersion plus each receipt revision's version/kind. Adoption can be a root. A successor must be a paired CORRECTION/CORRECTS or SUPERSESSION/SUPERSEDES, name the predecessor's exact revision and version, and have consecutive revision version. Retain every old receipt in the digest with an explicit successor link; only the single terminal may satisfy current-public/effective/projection/native observation.

Each endpoint independently retains its own frozen source request version. The normal edge requires a strictly newer request version, allowing later messages in the same request. Equal versions require the successor's explicit immutable conflict replacement link to the predecessor proposal; equality alone is insufficient. Reject backward source order, unrelated bases/revisions, gaps, forks, detached nodes, multiple terminals, cross-module/kind chains, withdrawal/private/expired terminal and native drift. Do not require every receipt to share one frozen request version: the existing uniqueness constraint means ordinary later-message continuations necessarily use newer versions. Preserve the current fail-closed behavior until positive and negative chain proof is integrated.
