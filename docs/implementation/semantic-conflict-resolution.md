...
This query is display state, not approval, a prepared draft or publication permission. The existing mutations still perform their exact semantic preview, provenance, receipt and locked version checks. UI consumption remains the next step; the existing `hasLegacyTarget` list boolean is not upgraded into authoring authority.
