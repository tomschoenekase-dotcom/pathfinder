...
### Next implementation: temporal completion evidence

Reuse `KnowledgeProposalOperationalUpdateHandoff` as immutable draft provenance, not scheduling authority. A read-only temporal fulfillment reader should compose with current completion evidence and reuse bounded direct/one-hop support provenance. The present public chat predicate is in `packages/api/src/routers/chat.ts` around the operational-update query; `buildOperationalUpdatePreview` and the human-only scheduling/expiry actions are in `packages/db/src/helpers/operational-update-actions.ts`. Re-read exact current sources before editing.

A positive temporal receipt must observe PUBLISHED, active, a retained publishedAt, startsAt <= verification time < expiresAt, and either no place target or a PUBLIC place. Freeze receipt/proposal/source request version/update identity, actual guest-visible fields and effective window into a timestamp-independent digest. Only LIVE is eligible: DRAFT, future SCHEDULED, INACTIVE, explicit or natural EXPIRED, missing publication identity and private-place updates must block. Revalidate under the existing venue content lock and compare approved identity. Do not add scheduling, publication, expiry or delivery as a side effect of completion.

Targeted proof must cover direct and immutable replacement lineage, scope/frozen-audit rejection, all ineligible states, a public LIVE positive, and body/window/status drift between proposal and apply. Use real disposable scheduling actions for the persistence proof; do not relabel a fixture as live/provider/customer acceptance. Before removing the current temporal fail-closed guard, compose positive receipt evidence into the same approval digest and prove a temporary update cannot evade either reader through its alternate handoff.
