...
The public guest-chat prompt is independently versioned as `guest-chat-prompt-v8` with contract hash `c4178f9c4200c7e27c959d16f5dcfdbacc1f8172c70b2210fd03159eb03d144d`. Version 8 retains the version 7 response-depth contract and strengthens the instruction/data boundary: cross-venue, hidden-context, private-data, and secret refusals must be generic and must not reflect attacker-supplied names or markers. The seven-dimension onboarding suite is versioned separately as `torchiko-onboarding-evaluation-suite-v4`; its adversarial case checks both cross-tenant canary disclosure and a stable system-prompt heading, and its honest-unknown rubric accepts the bounded disclosure language required by the production prompt. Evaluation identities were deliberately re-pinned when either contract changed.
