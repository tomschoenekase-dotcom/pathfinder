...
`PATHFINDER_STAGING_MIGRATION_APPROVAL=torchiko-staging-lineage-to-249-20260912`. The value must
...
The current source endpoint is 249 migrations and 265 public tables, ending with `20260912080000_add_guest_conversation_disposition`, with LF-normalized manifest SHA-256 `4eff7f0a42a6cce1e695f7bc9bcd5b90a8b7534b1e3f0b11f59bfc6e618f9736`. The exact 248/264 boundary below is now the `guest-disposition-predecessor`; only migration 249 remains. The [populated 248→249 rehearsal](evidence/migration-248-249-admission-2026-09-12.json) preserved 157 synthetic rows across 38 populated tables, all 263 old application-table column projections, both sequences and every old ledger field. One advance succeeded and replay was a no-op. Failed, rolled-back, logged and divergent ledger rows were refused. This is local source and fixture evidence, not a hosted migration or completed guest erasure.

The new approval token is `torchiko-staging-lineage-to-249-20260912`; the prior 248 token does not authorize this endpoint. Keep the existing exact release/target, USD10 ceiling, fresh preserved-data backup/restore, named recovery decision and migration-only hold gates. Migration 249 creates or validates a marked NOLOGIN function-owner role; hosted migration-role privileges and any existing role conflict must be reviewed before execution. The application receives no erase privilege by default. Actual maintenance also requires separately reviewed execution grants, stopped processes and external journal custody/high-water evidence. Do not equate schema installation with maintenance authority or privacy completion.
