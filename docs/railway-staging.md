...
`PATHFINDER_STAGING_MIGRATION_APPROVAL=torchiko-staging-lineage-to-251-20260925`. The value must
...
The current source endpoint is 251 migrations and 268 public tables, ending with `20260925044500_add_prospect_outreach_draft_gmail_links`, with LF-normalized manifest SHA-256 `2a516d26aa0e41703af3a727b4477d67094b20e3d9cd0b53a346e3d81059ba2d`. The exact 250/267 boundary is the `agent-routines-complete-predecessor`, ending with `20260918190000_add_agent_routines` and retaining its frozen LF-normalized manifest SHA-256 `9a8d7747ac94edeb3eb2b60aabbe661e23c3f360d5f48f657591dcff08e837be`. The 249/265 boundary remains frozen at `20260912080000_add_guest_conversation_disposition` with manifest SHA-256 `4eff7f0a42a6cce1e695f7bc9bcd5b90a8b7534b1e3f0b11f59bfc6e618f9736`. The source admission tests retain both predecessor identities, accept only the exact 250→251 suffix, require replay to be a no-op, and refuse divergent, failed, rolled-back, or logged ledger rows. This deterministic source proof is not a populated native rehearsal, hosted migration, backup, restore, or production-readiness claim.

The new approval token is `torchiko-staging-lineage-to-251-20260925`; prior tokens, including the 250 token, do not authorize this endpoint. Migration 251 adds Gmail message links to prospect outreach drafts. Keep the existing exact release/target, USD10 ceiling, fresh preserved-data backup/restore, named recovery decision and migration-only hold gates. A fresh hosted preservation backup and restore readback tied to the exact 250-row ledger remains required before a `preserve-existing` cutover; source admission does not satisfy it.

The retained historical local endpoint has 248 migrations and 264 public tables, ending with `20260911063000_add_native_venue_bot_configuration_effect`, with LF-normalized manifest SHA-256 `73e4424d07324767058e2934a01d32f068b8270b414f30a91e15914fd32891fa`. The [additive enum rehearsal](evidence/migration-247-248-admission-2026-09-11.json) preserves the exact 247 predecessor, its ledger, existing native effect and enum ordering; replay is a no-op. The new enum value is required by the existing native configuration apply/revert implementation. This local evidence does not authorize a hosted migration: retain the exact new candidate, final drained backup/restore, named recovery decision, hold and closed-opt-in reopening gates.

The retained 247 predecessor has 264 public tables, ends with `20260910140000_add_semantic_reviewed_decline`, and has manifest `accc130b682f930408145bf38eb97e27488b183cf54884e8f78753760d5da82c`. The exact-question operation boundary remains an explicitly measured predecessor: 236 migrations, 256 public tables, final migration `20260908160000_add_agent_question_operations`, and manifest `f4aebada18e395975ca24613b86caf3a93428d1f5c661e55ae446527130861a9`. Its eleven-file suffix is accepted only as the complete reviewed transition; an intermediate 237–246 ledger is not automatically admitted. The prior file-extraction candidate (235 migrations, 255 tables, final migration `20260908150000_add_intake_v1_file_extraction_dispatches`) remains an explicitly admitted predecessor with manifest SHA-256 `968270ab6d65dd64e9b3027e3c6d6b906d4805d7312190eecaa3b269cc34d0e5`. The prior 234-migration, 255-table website PDF state remains an explicitly admitted predecessor with manifest SHA-256 `fc4f9c47b4378fdd3abf2d598cdbcba2e3d2d1f0d86997102c9c0b72a5f767ab`.
...
capture the resulting owner revision, then finalize the generated candidate handoff before changing
Railway or pushing:

```bash
pnpm staging:handoff:finalize \
  --manifest artifacts/staging-handoff/<candidate-sha>.json \
