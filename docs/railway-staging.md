...
`PATHFINDER_STAGING_MIGRATION_APPROVAL=torchiko-staging-lineage-to-250-20260918`. The value must
...
The current source endpoint is 250 migrations and 267 public tables, ending with `20260918190000_add_agent_routines`, with LF-normalized manifest SHA-256 `9a8d7747ac94edeb3eb2b60aabbe661e23c3f360d5f48f657591dcff08e837be`. The exact 249/265 boundary is now the `agent-routines-predecessor`, ending with `20260912080000_add_guest_conversation_disposition` and retaining its frozen LF-normalized manifest SHA-256 `4eff7f0a42a6cce1e695f7bc9bcd5b90a8b7534b1e3f0b11f59bfc6e618f9736`; only migration 250 remains. The source admission tests freeze that prefix, accept only the exact 249→250 suffix, require replay to be a no-op, and refuse divergent, failed, rolled-back, or logged ledger rows. This deterministic source proof is not a populated native rehearsal, hosted migration, backup, restore, or production-readiness claim.

The new approval token is `torchiko-staging-lineage-to-250-20260918`; prior tokens, including the 249 token, do not authorize this endpoint. Migration 250 adds the default-disabled `agent_routines` and `agent_routine_dispatches` tables. Keep the existing exact release/target, USD10 ceiling, fresh preserved-data backup/restore, named recovery decision and migration-only hold gates. A fresh hosted preservation backup and restore readback tied to the exact 249-row ledger remains required before a `preserve-existing` cutover; source admission does not satisfy it.
