...
- `candidate`: the static checks plus typecheck, lint, the full test suite, build, client-bundle
  secret scanning, browser-foundation contracts and accessibility contracts. A clean successful
  candidate is `ready-for-staging-review`, not production-approved.
...
deployed revision and core resource identity, not provider quality, OAuth, mail delivery, browser
journeys, billing sandbox behavior or customer acceptance. Record those as separate release evidence
when applicable.
