...
| Visitor   | Location awareness                          | IMPLEMENTED BUT NEEDS POLISH | guest anchor and deterministic route resolvers; guarded admin floor/anchor/connection workspace; `torchiko.locations.propose_draft`; location intelligence schema | Safe resolver and progressive review lifecycle | Active reviewed topology can return bounded shortest-path guidance with strict accessibility filtering. Visitor UI/tool integration, weighted routing, real-venue route QA, and live turn-by-turn navigation remain. |
