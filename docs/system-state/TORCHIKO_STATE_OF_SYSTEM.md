...
| Knowledge | Legacy Place/knowledge                      | LEGACY / SUPERSEDED          | `place.ts`, `knowledge.ts`, `legacy-content-actions.ts`                                                                                                                           | Still functional                               | Semantic retrieval still depends on it; exact convergence and frozen legacy-to-native shadow comparisons are measurable, but no read switch or retirement is authorized.                                   |
| Knowledge | Native module/revision/publication model    | IMPLEMENTED BUT NEEDS POLISH | content models/actions, manifests, native deployments, convergence and shadow-comparison measurement                                                                              | Strong but complex                             | Exact head drift plus same-corpus legacy/native evaluation deltas are available; representative evidence, approved thresholds, and the cutover/rollback contract remain open.                              |
