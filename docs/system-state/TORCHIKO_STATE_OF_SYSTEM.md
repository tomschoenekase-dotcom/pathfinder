...
| Knowledge | Native module/revision/publication model    | IMPLEMENTED BUT NEEDS POLISH | content models/actions, manifests, native deployments, convergence, shadow-comparison measurement, non-executable read-switch contract                                            | Strong but complex                             | Exact head drift and same-corpus legacy/native deltas feed an explicit fail-closed contract. Representative evidence, approved quality thresholds, an implemented/rehearsed executor, and production approval remain open.                |
