...
| Current release evidence   | Local candidate `392a5415...`: 215 migrations and 239 public tables measured on disposable PostgreSQL. Retained September 2 hosted evidence records `a3e66de5...` at 207/207 migrations and 232 tables; this local campaign has not deployed its candidate. |
...
| Technical   | Two content/deployment generations coexist, and 402 approved tenant-isolation bypass calls plus 129 raw-SQL operations increase review burden.    |
...
- `packages/db`: integrated Prisma schema, 215 migrations, tenant middleware, auditable domain actions, raw SQL, lifecycle helpers.
...
The newer local candidate at `392a5415` has a frozen 215-migration manifest ending at `20260907021500_add_media_entity_resolution_revisions`. A fresh disposable PostgreSQL deployment measured 215 completed ledger rows and 239 public tables, then verified cleanup. The 214/238 legacy-adoption candidate remains a preserved predecessor. This is local release-candidate evidence only: no Railway service, hosted database, or production environment was changed, and the September 2 207/232 hosted snapshot remains preserved as dated deployment evidence.
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 402 approved bypass calls in 134 production files and 129 raw-SQL operations (55 reads, 74 writes). Those are inventoried and tests passed, but each expands the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
