...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current-truth overlay      | 2026-09-01, America/Chicago                                                                                                                                                                     |
...
| Current release evidence   | Exact active Railway staging release `723ecc58...`; three Git-identified services; 207/207 migrations; 232 public tables; hosted profile 19/19; admissions closed; production untouched         |
...
Railway has separate dashboard, web, and worker configurations plus Dockerfiles and executable staging-config gates. The bounded 2026-09-01 snapshot proves that all three staging services are `SUCCESS`/`RUNNING` on exact Git revision `723ecc583a57ac79e56489f4a9a5865cb946d505`; guarded pre-deploy applied only the intended 207th migration and a later Git-identified predeploy rechecked the complete 207/207 ledger against 232 public tables. The exact hosted profile passed 19/19, strict topology admitted every immutable image digest, both one-run admissions returned to `0`, and production was not selected or mutated. Railway's separate web pre-deploy runtime requires the exact non-secret approval as a service variable rather than relying on image-only `ENV`; the verifier and runbook enforce that topology. A root Railway/Nixpacks configuration still coexists with the proven service-specific path and must not be removed until every remaining consumer or recovery use is ruled out. See [`TORCHIKO_STAGING_CURRENT_TRUTH_2026-09-01.md`](./TORCHIKO_STAGING_CURRENT_TRUTH_2026-09-01.md).
...
CI provisions disposable pgvector PostgreSQL, Redis, and MinIO and runs migration, integration, bundle-secret, accessibility, type, lint, test, and build gates. Exact local release candidates run a 27-gate assessment and can be projected as immutable platform evidence. GitHub Actions runs `33464160442` and `33466345529` completed successfully for the exact active staging revision on the candidate and staging branches; later branch revisions retain separate run status. Local compose pins all five service images by SHA-256 digest; tag text is retained only as human-readable context.
