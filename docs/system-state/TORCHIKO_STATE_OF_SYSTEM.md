...
Current isolated source candidate, 2026-09-24: 254 migrations end at `20260923014500_prospect_county_research_leases` with manifest SHA-256 `57cb04a4165c78db2be8ea0bd2e9eea72527a1f8fd5bf298efb7a7dbfbf3d112`. This source has 477 approved bypass calls in 167 production files and 252 raw-SQL operations (151 reads, 101 writes). The last hosted staging observation below remains the 250-migration release; this source extension has not been admitted or deployed. Its four added migrations belong to the native prospect workflow integration and require the staging migration hold, backup, restore proof, and release checks before rollout.

Final launch review, 2026-09-19: the current source and admitted staging database contain 250 migrations ending at `20260918190000_add_agent_routines`, with 267 public tables and LF-normalized manifest SHA-256 `9a8d7747ac94edeb3eb2b60aabbe661e23c3f360d5f48f657591dcff08e837be`. Staging revision `43d5dcd5bdc58a32334619a2cede5cf56bd79ec0` completed the release-bound backup and disposable restore, held migration, three-service rollout, public health read-back, and authenticated hosted inspection; web deployment `91cbf9c2-089d-41c4-870c-442de4e5e225`, dashboard `ff87d936-49ba-4b12-be16-96bb4dc4fd66`, and workers `9c82e470-1332-451f-996c-cfbcca39bd5a` all reported `SUCCESS`. The frozen 249/265 predecessor ends at `20260912080000_add_guest_conversation_disposition` with manifest SHA-256 `4eff7f0a42a6cce1e695f7bc9bcd5b90a8b7534b1e3f0b11f59bfc6e618f9736`; its [retained integrity evidence](../evidence/staging-249-integrity-2026-09-18.json) remains historical proof for that exact revision rather than current hosted state. Recurring routines are present but default-disabled, and the local Hermes bridge remains human-gated on an exact-venue one-time credential and live smoke test. Production readiness and publication are not claimed or authorized. Historical snapshots below retain their recorded revision limits.
...
| Retained release evidence  | Local candidate `f510cd38b2f79efdc2b74c940b8b5b7535c7596f`: 227 migrations and 254 public tables measured on native disposable PostgreSQL in UTC. Retained September 2 hosted evidence records `a3e66de5...` at 207/207 migrations and 232 tables; this local campaign has not deployed its candidate. |
...
| `golden-venue-lifecycle`           | The disposable end-to-end lifecycle and eight failure classes are retained and provider-dark locally proven.   |
...
| `agent-workforce-credibility`      | Concurrent heterogeneous workers, same-role instances, and fenced crash takeover are provider-dark proven.     |
...
| Technical   | Two content/deployment generations coexist, and 451 approved tenant-isolation bypass calls plus 252 raw-SQL operations increase review burden.    |
...
| Visitor   | Citations and claim evidence                | PARTIALLY IMPLEMENTED            | citation contract/renderer; deterministic guest citation projection; append-only human/system attribution; default-off evaluator workflow; independent-human agreement evidence                                                | Conservative visitor proof; strong internal evidence integrity | Exact machine review is durable and provider-dark proven but default-off. Semantic correctness, representative calibration, provider-enabled staging QA, thresholds, automatic sampling, and visitor-visible claim UX remain unproven.          |
...
| Visitor   | Realtime voice                              | PARTIALLY IMPLEMENTED            | `packages/api/src/routers/voice.ts`; `packages/ai/src/realtime-voice.ts`; `packages/db/src/helpers/voice-session-recovery.ts`; `apps/workers/src/processors/voice-session-recovery.ts`; `apps/web/components/VoiceControl.tsx` | Serious, recovery-safe foundation                              | WebRTC ephemeral auth, quota, transcripts, usage, and abandoned-session capacity recovery exist; provider-dark lifecycle is disposable-DB proven, while feature/entitlement/provider activation and live browser/provider quality remain gated. |
...
| Client    | Analytics/chat logs/content editing         | LOCALLY IMPLEMENTED              | privacy-bounded visitor pulse; service-led correction request; exact support-to-proposal bridge; legacy authoring routes redirect                                                                                              | Safe and intentionally narrow                                  | Clients see aggregate signals and can request a correction; reviewed evidence can enter a separate proposal queue, while raw conversations and direct publication remain unavailable.                                                           |
...
| Agents    | Durable identities/runs/questions/approvals | IMPLEMENTED / LOCALLY PROVEN     | agent Prisma models; admin agent routers/UI; `agent-run.ts`; provider-dark disposable workforce shakedown                                                                                                                      | Strong state machine                                           | Heterogeneous concurrency, same-role instances, takeover fencing, duplicate prevention, system initiation, and exact costs are locally proven; hosted provider quality remains unverified.                                                      |
| Agents    | Specialist delegation/tool use              | PARTIALLY IMPLEMENTED            | MCP registry/read actions; bridge runner; delegation records; role/capability-bound claims                                                                                                                                     | Conditional                                                    | Authenticated bridge execution is provider-dark disposable-proven; rich live tools still require an admitted external worker and explicit permission mapping.                                                                                   |
...
The retained historical local candidate `f510cd38b2f79efdc2b74c940b8b5b7535c7596f` freezes 227 migrations ending at `20260907022700_add_intake_source_mapping_reviews`, with normalized manifest SHA-256 `821701d8190e35e06c27053b16bc176937015dedc047514cea0d42643ec89c19`. Native disposable PostgreSQL 16.15 in UTC measured 227 completed ledger rows and 254 public tables; the source-mapping fixture passed and its server stopped. [Evidence](../evidence/intake-source-mapping-native-postgres-2026-09-07.json) retains measured source copies, logs, and data. Earlier Docker resource cleanup remains unverified.

The measured V1 package predecessor `55408c4759b2fe9ccfe977e45abbe4800cc05cc5` retains 226 migrations, 253 public tables, and normalized manifest SHA-256 `033233483cdf5c89f5afecd2eb622f93802b228b0b5b9f11400e05f7254b4bbd`. Its 226-row boundary remains explicitly admitted; the retained package-handoff proof is preserved.
...
The preserved predecessor candidate `11d5cabc14b00351c94b110c57e924a15a0f2f53` has a frozen 223-migration manifest ending at `20260907022300_add_agent_workflow_activations`. A fresh disposable PostgreSQL deployment measured 223 completed ledger rows and 248 public tables, recorded normalized manifest SHA-256 `f643a5f40f0e205f8664eb36f42cddad22e66527bc1b47f663cd3f75f5d69be1`, then verified cleanup. The 222/245 promotion-assessment, 221/244 usage-observation and 220/244 workflow-registry states are admitted migration-derived predecessor boundaries; they were not separately queried. The measured 219/243 governed-media, 218/243 prospect-onboarding, 216/240 media-relation, 215/239 media-resolution, and 214/238 legacy-adoption candidates remain preserved predecessors. This is local release-candidate evidence only: no Railway service, hosted database, or production environment was changed, and the September 2 207/232 hosted snapshot remains preserved as dated deployment evidence.
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 451 approved bypass calls in 157 production files and 252 raw-SQL operations (151 reads, 101 writes). The September 11 final review inspected the added scopes and exact repeated-template counts, including active public slug resolution followed by exact tenant/venue publication authority and the shared tenant lock for checkout reservation and finalization. The September 22 client-delivery slice additionally admits one exact platform-admin venue-launch-asset read and its bounded global public-slug uniqueness check. Both executable verifiers passed. Each operation remains part of the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
...
The default unit suite skips integrations needing disposable DB/Redis/storage/ClamAV in the current invocation; CI has explicit jobs for many of them. The suite is strongest at state transitions, authorization, tenant predicates, validation, idempotency, worker retry/lease logic, and rendered component contracts. A bounded authenticated four-worker bridge scenario now covers heterogeneous concurrent claims, multiple instances of one role, and crash takeover locally. The suite remains weaker at true browser automation across authenticated journeys, provider-backed AI quality, voice WebRTC, deliverability, cloud deployments, restoration, sustained high-volume concurrency, and real mixed-media extraction.
...
9. **Accumulate progressive agent authority evidence** — canonical rollback, approval acceptance, policy-violation, and confidence signals now exist without automatically loosening policy; representative reviewed history and any promotion policy remain future work. _Effort M; impact high._
