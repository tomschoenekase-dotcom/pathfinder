...
| Visitor   | Location awareness                          | IMPLEMENTED BUT NEEDS POLISH | `packages/api/src/routers/location.ts`; guarded admin location workspace; `torchiko.locations.propose_draft`; `20260819150000_add_location_intelligence_v1` | Safe resolver and progressive review lifecycle | Agent proposals change no content; human approval and separate application create only inactive drafts; activation remains separate. No floor/connection authoring or turn-by-turn routing. |
