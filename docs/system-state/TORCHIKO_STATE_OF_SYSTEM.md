...
The largest product gaps in the audited baseline were equally concrete. CRM, sales pipeline, outreach sequences, inbound email, general outbound communications, billing collection, and meeting scheduling did not exist at that snapshot; later branch deltas in this report must be consulted before treating those statements as current truth. The client portal intentionally hides most analytics and content-management power, so internal operators carry much of the workload. Since the audit, bounded citation projection now exposes safe provenance for explicitly named retrieved records and persists it across replay, while claim-level semantic support remains unproven. Location V1 resolves known location context, and platform administrators can now create, correct, review, activate, and deactivate exact-venue anchors through a mobile-responsive workspace. It still does not provide turn-by-turn routing. “Agents improve as they work” is not real learning: outcome observations are stored and displayed, but nothing consumes them to change prompts, routing, reputation, or policy. The event center is a strong in-app aggregation surface, while its multi-channel delivery model is largely scaffolded.
...
| Visitor   | Location awareness                          | IMPLEMENTED BUT NEEDS POLISH | guest location resolver; guarded admin floor/anchor/connection workspace; `torchiko.locations.propose_draft`; location intelligence schema | Safe resolver and progressive review lifecycle | Agent proposals change no content; human approval and separate application create only inactive anchor drafts; topology activation remains separate. Floors/connections have guarded admin authoring, but no turn-by-turn routing consumes them. |
