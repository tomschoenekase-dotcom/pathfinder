...
| Visitor   | Location awareness                          | IMPLEMENTED BUT NEEDS POLISH | `packages/api/src/routers/location.ts`; `packages/api/src/routers/admin/location-authoring.ts`; dashboard location workspace; `20260819150000_add_location_intelligence_v1` | Safe resolver and review lifecycle  | Exact-venue anchors are inactive by default, CAS-edited, reason-activated, and audited; no floor/connection authoring or turn-by-turn routing. |
