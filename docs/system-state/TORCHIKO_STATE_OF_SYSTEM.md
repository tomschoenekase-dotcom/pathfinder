...
| Current release evidence   | Local candidate `af5e15559fb6296cfdc676a9f6de0d2c216186f5`: 222 migrations and 245 public tables measured on disposable PostgreSQL. Retained September 2 hosted evidence records `a3e66de5...` at 207/207 migrations and 232 tables; this local campaign has not deployed its candidate. |
...
| Technical   | Two content/deployment generations coexist, and 414 approved tenant-isolation bypass calls plus 147 raw-SQL operations increase review burden.    |
...
- `packages/db`: integrated Prisma schema, 222 migrations, tenant middleware, auditable domain actions, raw SQL, lifecycle helpers.
...
The local candidate `af5e15559fb6296cfdc676a9f6de0d2c216186f5` has a frozen 222-migration manifest ending at `20260907022200_add_agent_workflow_promotion_assessments`. A fresh disposable PostgreSQL deployment measured 222 completed ledger rows and 245 public tables, recorded normalized manifest SHA-256 `ceff51dc605a1d4873c327f9ab667be4eef1523f2c131a5b1462f5ac0a74cd1d`, then verified cleanup. The 221/244 usage-observation and 220/244 workflow-registry states are admitted migration-derived predecessor boundaries; they were not separately queried. The measured 219/243 governed-media, 218/243 prospect-onboarding, 216/240 media-relation, 215/239 media-resolution, and 214/238 legacy-adoption candidates remain preserved predecessors. This is local release-candidate evidence only: no Railway service, hosted database, or production environment was changed, and the September 2 207/232 hosted snapshot remains preserved as dated deployment evidence.
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 414 approved bypass calls in 137 production files and 147 raw-SQL operations (71 reads, 76 writes). Those are inventoried and tests passed, but each expands the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
