...
The largest product gaps are equally concrete. CRM, sales pipeline, outreach sequences, inbound email, general outbound communications, billing collection, and meeting scheduling do not exist. The client portal intentionally hides most analytics and content-management power, so internal operators carry much of the workload. Citations are present in the response contract/UI but are not generated by the chat path. Location V1 resolves known location context but does not route. “Agents improve as they work” is not real learning: outcome observations are stored and displayed, but nothing consumes them to change prompts, routing, reputation, or policy. The event center is a strong in-app aggregation surface, while its multi-channel delivery model is largely scaffolded.

The highest immediate value is not another broad architecture layer. It is to run and preserve one golden, real-data lifecycle: create a client and venue, complete remote intake, verify extraction/review, approve and publish a package, answer grounded guest questions, collect feedback, generate a report, handle a support case, and safely offboard/export. That exercise should drive fixes to the misleading provider-disabled chat error, missing privacy page, notification delivery, client visibility, recovery posture, and observability. Only after this flow is repeatable should Torchiko expand into a CRM or autonomous outreach system.
...
| Visitor   | Citations                                   | SCAFFOLDED                   | citation block in guest contract/renderer; no citation construction in `chat.ts`                                                        | Misleadingly visible in architecture | Do not claim sourced answers until generation and provenance are wired.                                                             |
...
| Visitor   | Location awareness                          | PARTIALLY IMPLEMENTED        | `packages/api/src/routers/location.ts`; `20260819150000_add_location_intelligence_v1`                                                   | Safe resolver                        | Verified anchors/map URLs; no map authoring UI or turn-by-turn routing.                                                             |
...
| Client    | Analytics/chat logs/content editing         | PARTIALLY IMPLEMENTED        | legacy client routes redirect; admin surfaces hold the full tools                                                                       | Safe but limited                     | Client analytics route intentionally returns home; paying customers may perceive low self-service value.                            |
...
| Knowledge | Legacy Place/knowledge                      | LEGACY / SUPERSEDED          | `place.ts`, `knowledge.ts`, `legacy-content-actions.ts`                                                                                 | Still functional                     | Retrieval still depends on it; do not remove without a measured cutover.                                                            |
| Knowledge | Native module/revision/publication model    | IMPLEMENTED BUT NEEDS POLISH | content models/actions, manifests, native deployments                                                                                   | Strong but complex                   | Dual-write/materialization boundaries add cognitive and migration cost.                                                             |
...
A visitor can open a branded venue route or embed, receive suggested questions, choose a language, persist an anonymous scoped session, and send retry-safe chat turns. The API validates venue/experience visibility, rate limits, retrieves public venue knowledge, records analytics, and produces structured responses. Privacy and “verify important details” messaging are visible. The inspected `/august-test/chat` route rendered well, but because the local worker/provider mode was disabled, sending a suggested question ended with “outcome not confirmed” rather than a clear service-unavailable response. Citations, realtime voice, feedback, and location all have code, but only the basic feedback/location contracts are near usable; citations are not generated, voice was not live-tested, and routing does not exist.
...
**No, not in the strong sense.** Humans can append an `AgentOutcomeObservation`, and agents/operators can later read it. Conversation insights and knowledge proposals can surface repeated information gaps. But no implementation was found that aggregates outcomes into a quality score, rewrites agent instructions, changes model routing, selects specialists, updates memory, or promotes a behavior automatically. The present system supports auditable feedback collection; it does not support autonomous learning. Building a small, explicit outcome-to-policy review loop would be safer than calling this “self-improvement.”
...
5. **Schema-ahead behavior:** operational event delivery, outcome learning, citations, billing and some structured response blocks imply capabilities their runtimes do not yet deliver.
...
- Citations are not connected to retrieval/generation.
...
7. **Give clients a small, safe insight/correction loop** — report top questions, content gaps, feedback, and propose corrections without direct publish. _Effort M; impact high._ It increases customer value without abandoning review safety.
...
What looks more complete than it is is everything around the edges of that core. Voice, location, event delivery, agent learning, citations, billing, and communications all have varying amounts of foundation, but they are not equally usable. CRM and outreach basically do not exist. Agents can run and ask questions, but they do not learn automatically, and rich tools depend on an external bridge. Clients see a polished but intentionally narrow portal, leaving your team to do most of the work.
