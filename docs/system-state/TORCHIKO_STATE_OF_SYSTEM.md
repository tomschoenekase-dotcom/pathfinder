...
Final launch review, 2026-09-11: the current local source contains 249 migrations ending at `20260912080000_add_guest_conversation_disposition`; this source count does not claim that the new migration has passed native admission or reached staging. Real disposable native publication previously exposed the missing `VENUE_BOT_CONFIGURATION` enum value; that append-only repair preserves all 247 prior SQL files. The [247-to-248 rehearsal](../evidence/migration-247-248-admission-2026-09-11.json) preserves the predecessor ledger, existing enum values/order and measured deployment effect, with 264 tables and a no-pending replay. The earlier independently reviewed populated 236/245-to-247 rehearsal and actual staging 207 archive local restore/upgrade remain historical proof through 247 only. See [full archive preflight evidence](../evidence/staging-full-archive-restore-preflight-2026-09-10.json) for their exact source, ownership/ACL proof and runtime limits. A new exact candidate admission, fresh drained backup, concrete hosted recovery approval and staging publication remain pending. The last read-only staging evidence reports `ecd7d07f3f9e7af97f3db31990b37849667598ed`, with the expected database/Redis/storage fingerprints and database/queue up; its database remains 207 migrations/232 tables. This is the prior staging deployment, not publication of the current candidate. Historical snapshots below retain their recorded revision limits. Production publication is prohibited for this mission.
...
| Technical   | Two content/deployment generations coexist, and 446 approved tenant-isolation bypass calls plus 248 raw-SQL operations increase review burden.    |
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 448 approved bypass calls in 154 production files and 248 raw-SQL operations (149 reads, 99 writes). The September 11 final review inspected the added scopes and exact repeated-template counts, including active public slug resolution followed by exact tenant/venue publication authority and the shared tenant lock for checkout reservation and finalization. Both executable verifiers passed. Each operation remains part of the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
