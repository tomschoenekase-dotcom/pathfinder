...
| Visitor   | Citations and claim evidence                | PARTIALLY IMPLEMENTED          | citation contract/renderer; deterministic guest citation projection; `docs/guest-answer-attribution.md`; append-only attribution evidence                                                               | Conservative visitor proof; strong internal evidence integrity | Explicitly named retrieved records can expose safe provenance. Internal reviewers can bind exact claims to frozen evidence, but semantic correctness, evaluator calibration, provider-enabled staging QA, and visitor-visible claim UX remain unproven.                                       |
