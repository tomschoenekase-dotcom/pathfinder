...
| Current release evidence   | Local candidate `f6e39375914424e7570d0a8560343a08ebb092dc`: 219 migrations and 243 public tables measured on disposable PostgreSQL. Retained September 2 hosted evidence records `a3e66de5...` at 207/207 migrations and 232 tables; this local campaign has not deployed its candidate. |
...
| Technical   | Two content/deployment generations coexist, and 410 approved tenant-isolation bypass calls plus 146 raw-SQL operations increase review burden.    |
...
- `packages/db`: integrated Prisma schema, 219 migrations, tenant middleware, auditable domain actions, raw SQL, lifecycle helpers.
...
The local candidate `f6e39375914424e7570d0a8560343a08ebb092dc` has a frozen 219-migration manifest ending at `20260907021900_add_governed_guest_place_media_preferences`. A fresh disposable PostgreSQL deployment measured 219 completed ledger rows and 243 public tables, then verified cleanup. The measured 218/243 prospect-onboarding candidate, 216/240 media-relation candidate, 215/239 media-resolution candidate, and 214/238 legacy-adoption candidate remain preserved predecessors. This is local release-candidate evidence only: no Railway service, hosted database, or production environment was changed, and the September 2 207/232 hosted snapshot remains preserved as dated deployment evidence.
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 410 approved bypass calls in 136 production files and 146 raw-SQL operations (70 reads, 76 writes). Those are inventoried and tests passed, but each expands the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
