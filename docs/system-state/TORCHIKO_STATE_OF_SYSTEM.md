...
Final launch review, 2026-09-10: the current local source contains 247 migrations ending at `20260910140000_add_semantic_reviewed_decline`. The independently reviewed populated 236/245-to-247 rehearsal and actual staging 207 archive local restore/upgrade both reach 264 tables, preserving measured predecessor data and boundaries. See [full archive preflight evidence](../evidence/staging-full-archive-restore-preflight-2026-09-10.json) for exact source, ownership/ACL proof and runtime limits. Canonical final candidate admission, a fresh drained backup, concrete hosted recovery approval and staging publication remain pending. Read-only staging health on this date reports `ecd7d07f3f9e7af97f3db31990b37849667598ed`, with the expected database/Redis/storage fingerprints and database/queue up; its database remains 207 migrations/232 tables. This is the prior staging deployment, not publication of the current candidate. Historical snapshots below retain their recorded revision limits. Production remains stopped.
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 443 approved bypass calls in 151 production files and 243 raw-SQL operations (146 reads, 97 writes). The September 10 final review inspected the added scopes and exact repeated-template counts, and both executable verifiers passed. Each operation remains part of the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
