...
| Current release evidence   | Local candidate `7fea5c6b...`: 216 migrations and 240 public tables measured on disposable PostgreSQL. Retained September 2 hosted evidence records `a3e66de5...` at 207/207 migrations and 232 tables; this local campaign has not deployed its candidate. |
...
| Technical   | Two content/deployment generations coexist, and 405 approved tenant-isolation bypass calls plus 143 raw-SQL operations increase review burden.    |
...
- `packages/db`: integrated Prisma schema, 216 migrations, tenant middleware, auditable domain actions, raw SQL, lifecycle helpers.
...
The newer local candidate at `7fea5c6b` has a frozen 216-migration manifest ending at `20260907021600_add_media_relation_applications`. A fresh disposable PostgreSQL deployment measured 216 completed ledger rows and 240 public tables, then verified cleanup. The 215/239 media-resolution candidate and 214/238 legacy-adoption candidate remain preserved predecessors. This is local release-candidate evidence only: no Railway service, hosted database, or production environment was changed, and the September 2 207/232 hosted snapshot remains preserved as dated deployment evidence.
...
Tenant isolation is application-enforced through Prisma middleware, tenant-aware helpers, composite ownership checks, and executable source gates. The current executable inventories contain 405 approved bypass calls in 136 production files and 143 raw-SQL operations (68 reads, 75 writes). Those are inventoried and tests passed, but each expands the review surface. PostgreSQL row-level security was not found, so a missed predicate remains a plausible cross-tenant risk. This is not evidence of a present leak; it is a defense-in-depth gap. Current-truth tests derive these counts from the same boundary verifiers so future drift fails the script suite.
