...
| Realtime voice                     | PARTIALLY IMPLEMENTED        | Feature-gated; browser boundary verified locally | `VoiceControl.tsx`; `middleware.ts`; `widget.js`; `routers/voice.ts`; `packages/ai/src/realtime-voice.ts` | Provider-enabled staging/WebRTC/quota smoke on target Safari/Android. |
