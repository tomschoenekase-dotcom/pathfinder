...
- **Recommended change:** Put the bounded answer-attribution evaluator behind durable default-off request/worker/persistence controls, then use it only on a representative human-reviewed staging corpus to measure agreement before proposing thresholds. Continue describing the visitor feature narrowly as retrieved-record provenance.
...
- **Current automatic-evaluator foundation (2026-08-25):** A separately named provider-backed workload can review one exact answer/evidence identity through central routing, admission, budget, usage, and pre-dispatch fencing. It verifies frozen hashes before I/O and revalidates strict exact-span/source annotations afterward. It is intentionally not wired to an API, queue, schedule, persistence authority, release decision, or visitor UI.
- **Remaining gap:** No activated/persisted automatic evaluator workflow, representative calibrated corpus, approved quality threshold, release gate, or visitor-visible claim-level UX exists. Real human-review history, provider-backed staging calibration, and product review remain required.
