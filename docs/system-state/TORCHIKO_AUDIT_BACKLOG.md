...
### P0.3 — Prove the integrated migration lineage in disposable and staging databases — RESOLVED FOR THE EXACT 2026-09-01 STAGING RELEASE

- **Status:** **EXACT HOSTED STAGING LINEAGE PROVEN 2026-09-01.** The active web, dashboard, and worker services run exact Git revision `a3e66de5a1231aca6df5d150ca7bcd81831dd784`. Guarded pre-deploy proved the frozen 207-file manifest and exact complete 207-row ledger against exactly 232 public tables. The exact hosted profile passed 19/19, the single-use migration admission returned to `0`, and the fail-closed topology verifier proved one `SUCCESS` deployment with one `RUNNING` instance and the exact immutable image digest per application service.
...
- **Evidence:** `docs/system-state/TORCHIKO_STAGING_CURRENT_TRUTH_2026-09-01.md`; `scripts/verify-staging-config.mjs`; `scripts/create-staging-handoff.mjs`; `scripts/staging-migration-predeploy.test.mjs`; exact candidate and staging runtime artifacts.
...
- **Status:** **CENTRAL AND OPERATOR-CLI FAIL-CLOSED REDACTION IMPLEMENTED.** The shared stdout logger now retains only bounded operational metadata, recursively redacts credential/content/free-form fields and raw errors before every local or monitoring sink, bounds depth and collection size, handles circular values, and falls back to a minimal synthetic action if hostile metadata cannot be inspected. Embedding freshness, one-entity claim repair, and terminal-redrive failure paths now use a code-only CLI writer that has no exception-text input and validates every string it can emit.
...
- **Evidence:** `packages/config/src/logger.ts`; `packages/config/src/logger.test.ts`; `apps/workers/src/lib/safe-cli-failure.ts`; `apps/workers/src/lib/safe-cli-failure.test.ts`; `scripts/worker-cli-output-privacy.test.mjs`; `docs/MONITORING_RUNBOOK.md`; complete repository test, lint, typecheck, and production-build proof.
...
- **Evidence:** no RLS policies found; 451 approved bypasses across 157 production files; 252 raw-SQL operations. Current-truth tests derive these counts from the executable boundary verifiers so inventory drift cannot silently leave the audit understated.
