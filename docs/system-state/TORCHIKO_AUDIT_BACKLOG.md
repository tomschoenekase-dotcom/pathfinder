...
- **Status:** **EXACT HOSTED STAGING LINEAGE PROVEN 2026-09-01.** The active web, dashboard, and worker services run exact Git revision `a3e66de5a1231aca6df5d150ca7bcd81831dd784`. Guarded pre-deploy proved the frozen 207-file manifest and exact complete 207-row ledger against exactly 232 public tables. The exact hosted profile passed 19/19, the one-run migration admission returned to `0`, and the fail-closed topology verifier proved one `SUCCESS` deployment with one `RUNNING` instance and the exact immutable image digest per application service.
