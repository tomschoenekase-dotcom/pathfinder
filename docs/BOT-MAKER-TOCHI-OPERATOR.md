...
5. Show Tom the clean static candidates at comparable size against the approved Tochi reference. He can approve the exact static identity tonight without waiting for the animation engineering gate. Static approval is not animation or publication approval.
6. Retain the selected clean master and its provenance. Import it through the existing `CREATE_FROM_IMPORT` path and submit the exact artifact for founder review.
